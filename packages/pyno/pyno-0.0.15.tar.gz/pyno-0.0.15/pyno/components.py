...
H.defaults['bootstrap_dyn'] = {
    'styles': (
        ("https://stackpath.bootstrapcdn.com/bootstrap/4.1.0/css/bootstrap.min.css", "sha384-9gVQ4dYFwwWSjIDZnLEWnxCjeSWFphJiwGPXr1jddIhOegiu1FwO5qRGvFXOdJZ4"),),
    'scripts': (
...
def bootstrap(*content):
    return H.html(
    H.head((H.link(rel="stylesheet", href=href, integrity=integrity, crossorigin="anonomous") for href, integrity in H.defaults['bootstrap_dyn']['styles']), *H.defaults['bootstrap_preemble']),
    H.body(*content, (H.script(src=src, integrity=integrity, crossorigin="anonymous") for src, integrity in H.defaults['bootstrap_dyn']['scripts']), *H.defaults['bootstrap_postemble']))
