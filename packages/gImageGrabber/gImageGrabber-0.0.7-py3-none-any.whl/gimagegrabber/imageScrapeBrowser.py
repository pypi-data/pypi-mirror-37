# Script to Grab the image links froma google search and then downlaoding them
...
import ImageSearchTools as imgTools
import os
...
def get_images(url, test=False):  # get images from url
    # getting source code of the webpage
    html = browser(url, test)
    print("Extracting and Formating the source code")
...
def save_images(data, search, onlyType=' '):
    data = imgTools.invDict(data)
    print("Making Root Directory")
    directory = search.replace(' ', '_')
