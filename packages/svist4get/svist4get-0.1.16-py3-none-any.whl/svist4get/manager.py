...
class Parameters:
    ...
    def parse_command_line(self):
        ...
        parser.add_argument('-t', type=str, dest='transcript_id', default=0,
        ...
        parser.add_argument('-gtf', dest='gtf_file', type=str, default=0,
                            help='<path/file.gtf>   Path to the gtf file with the transcript annotation')
        ...
        parser.add_argument('-scb', dest='bedgraph_scale',
                            choices=['large', 'small', 'auto'], default='auto',
                            help=' large| small  Scaling style for bedGraph tracks (default: %(default)s)')

        parser.add_argument('-nts', dest='nucleotide_sequence', action='store_true',
                            help='Show reference genomic sequence with single-nucleotide resolution (default: for short regions e.g. < 160 nts)')

        parser.add_argument('-sca', dest='aminoacid_sequence_scale',
                            choices=['large', 'small', 'auto'], metavar=None, default='auto',
                            help=' largae|small   Scaling style for aminoacid sequence track: large (for long regions) or small (keep single-aminoacid resolution) (default: auto)')

        parser.add_argument('-it', dest='image_title', type=str, default='',
                            help=' <text>   Image title')

        parser.add_argument('-xs', dest='genome_axis_tics_step', type=int, default=0,
                            help=' <N>  X axis tics step [nt] (default: auto)')

        parser.add_argument('-lb', dest='lvl_of_bedgraph_axis_tics', nargs='*', default=-1, type=float,
        ...
        parser.add_argument('-ys', dest='bedgraph_axis_tics_step', nargs='*', type=float, default=0,
        ...
        parser.add_argument('-bul', dest='bedgraph_upper_limit', nargs='*', default=0,
                            help=' <N1> <N2>...| max   Upper limit for a bedGraph track Y-axis, can be: 1. a space-separated list of values: a separate axis limit for each bedGraph track; 2.a maximum reachable value across all visible tracks. (default: reachable maximum for each track separately)')

        parser.add_argument('-bll', dest='bedgraph_lower_limit', nargs='*', type=float, default=0,
        ...
        parser.add_argument('-bl', dest='bedgraph_label', nargs='*', default='',
        ...
        parser.add_argument('-gi', dest='genomic_intervals', nargs='*', default=[], action='append',
                            type=str,
        ...
        parser.add_argument('-gil', dest='genomic_intervals_label', nargs='*', default='',
        ...
        parser.add_argument('-st', dest='selected_transcripts', nargs='*', default=0,
        ...
        parser.add_argument('-hrf', dest='highlight_reading_frame',
                            help=' <N>   Highlight a particular reading frame (+0, +1 or +2)', default=0)

        parser.add_argument('-hc', dest='highlight_codon', type=str, default='',
                            help='<NtNtNt>   Highlight selected codon on the aminoacid sequence track, e.g. -hc ATT')

        parser.add_argument('-hf', dest='highlight_frame', nargs=3, default=0,
    ...
    def load_config(self, path_to_config):
        try:
            ...
            config['gene_id'] = 0
            config['window'] = [0]
            config['highlight_reading_frame'] = 0
            ...
            sys.exit('I can not process the config file. Please check the validity of the entered data')
    ...
    def load_palette(self):
        try:
            ...
            sys.exit('I can not process the palette file. Please check the validity of the entered data')
    def load_triplet_code(self):
        try:
            ...
            sys.exit('I can not process the triplete_code file. Please check the validity of the entered data')
    ...
    def load_font(self):
        try:
            ...
            path_to_font_regular = self.config['font_regular']
            path_to_font_medium = self.config['font_medium']

            path_to_config = self.config['path_to_config']
            path_to_dir_with_config = os.path.dirname(path_to_config)

            path_to_font_regular = os.path.join(path_to_dir_with_config, path_to_font_regular)
            path_to_font_medium = os.path.join(path_to_dir_with_config, path_to_font_medium)

            paths_to_fonts = dict(regular=path_to_font_regular, medium=path_to_font_medium)

            self.paths_to_fonts = paths_to_fonts
        except:
            sys.exit('I can not load font. Please check the validity of the entered data')
    ...
    def create_pdf(self):
        try:
            ...
            sys.exit('I can not create pdf. Please check the validity of the entered data and required modules')
...
class Axis_tics_data_praparation(Data_preparation):
    ...
    def create_dict(self):
        ...
        step = self.parameters.config['genome_axis_tics_step']
...
class Nt_seq_loader(Loader):
    ...
    def extract_data(self):
        ...
        sequnce_obj = data_processing.Extraction_of_sequence(name_of_file, name_of_scaffold, start, end)
    ...
    def create_track(self):
        ...
        for k in range(len(self.adjustable_parameters)):
            ...
            if k != (len(self.adjustable_parameters) - 1):
                ...
                self.global_parameters['pdf'].setFont('font_medium', self.config['font_size_transcript_id'])
                width_of_string1 = stringWidth(self.name1, 'font_medium', self.config['font_size_transcript_id'])
                width_of_string2 = stringWidth(self.name1, 'font_medium', self.config['font_size_transcript_id'])
                ...
                if len(result) == 0 and abs(dist) > 0.02 * length and (plus1 + plus2) < dist1:
...
class Bedgraph_data_preparation(Data_preparation):
    ...
    def create_dict(self):
        ...
        lvl_of_tics = self.parameters.config['lvl_of_bedgraph_axis_tics']
        self.maxmax = 0
        if self.parameters.config['bedgraph_upper_limit'] != 0:
            if self.parameters.config['bedgraph_upper_limit'][0] == 'max':
                self.parameters.config['bedgraph_upper_limit'] = 0
                self.maxmax = 1
        ...
        for i in range(len(self.parameters.config['bedgraph'])):
            ...
            if self.parameters.config['bedgraph_upper_limit'] == 0:
                ...
                upper_limit = self.parameters.config['bedgraph_upper_limit'][i]

            if self.parameters.config['bedgraph_lower_limit'] == 0:
                bottom_limit = 0
            else:
                bottom_limit = self.parameters.config['bedgraph_lower_limit'][i]
...
class Bedgraph_loader(Loader):
    ...
    def create_track(self):
        ...
        for k in range(len(self.adjustable_parameters)):
            ...
            if count1 != (len(self.config['c_column'])):
                color = self.config['c_column'][count1]
            else:
                count1 = 0
                color = self.config['c_column'][count1]
            count1 += 1
            ...
            if self.global_parameters['lvl_of_tics'] == -1:
                ...
                axis_tics = []
                min = min = self.adjustable_parameters[k]['bottom_limit']
                count = min
                if step != 0:
                    while count < max:
                        count += step
...
class Aa_seq_loader(Loader):
    ...
    def create_track(self):
        ...
        if len(sequence) < 160 and self.config['zoom_level'] != 'close':
...
class Image:
    ...
    def go(self):
        try:
            ...
            self.parameters.config['output_filename'] += '.pdf'
            self.real_pdf = canvas.Canvas(self.parameters.config['output_filename'], pagesize=(
            ...
            sys.exit('I can not create pdf file. Please check the validity of the entered data and required modules')


    def highlighting_the_frame(self):
        try:
            frame_start = int(self.parameters.config['highlight_frame'][0])
            frame_end = int(self.parameters.config['highlight_frame'][1])
            ...
            color_fill = (methods.hex_to_rgb(self.parameters.palette[self.parameters.config['c_fill_frame']]))

            aroundcolor = (methods.hex_to_rgb(self.parameters.palette[self.parameters.config['c_stroke_frame']]))

            self.real_pdf.setStrokeColorRGB(*aroundcolor, self.parameters.config['c_stroke_frame_alpha'])
            self.real_pdf.setDash(1)
            self.real_pdf.setLineWidth(0.1)
            self.real_pdf.setLineCap(0)

            self.real_pdf.setFillColorRGB(*color_fill, self.parameters.config['c_fill_frame_alpha'])

            self.real_pdf.rect(x_of_start, y_down, x_of_end - x_of_start, y_upper - y_down, fill=1)

            pdfmetrics.registerFont(TTFont('font_regular', self.parameters.paths_to_fonts['regular']))
            pdfmetrics.registerFont(TTFont('font_medium', self.parameters.paths_to_fonts['medium']))

            self.font_size = self.parameters.config['frame_font_size_coordinates']
            ...
            self.real_pdf.setFont('font_regular', self.parameters.config['frame_font_size_coordinates'])

            color = methods.hex_to_rgb(self.parameters.palette[self.parameters.config['c_text_frame']])
            ...
            self.real_pdf.setFont('font_regular', self.parameters.config['frame_font_size_label'])
            y_down -= 0.3 * self.height_of_string

            x_center = (x_of_end + x_of_start) / 2

            self.real_pdf.drawCentredString(x_center, y_down, self.parameters.config['highlight_frame'][2])
        except:
            sys.exit('I can not highlight the frame. Please check the validity of the entered data and required modules')

    def save(self):

        self.real_pdf.save()
        print('Image successfully saved to', self.parameters.config['output_filename'])
...
class Vgrid_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Vgrid track. Please check the validity of the entered data and required modules')
...
class Title_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Image Labael track. Please check the validity of the entered data and required modules')
...
class Axis_tics_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Axis tics track. Please check the validity of the entered data and required modules')
...
class Nt_seq_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Nucleotide sequence track. Please check the validity of the entered data and required modules')
...
class Transcript_struct_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Transcript structure tracks. Please check the validity of the entered data and required modules')
...
class Bedgraph_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Bedgraph tracks. Please check the validity of the entered data and required modules')
...
class Aa_seq_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Aminoacid sequence track. Please check the validity of the entered data and required modules')
...
class Genomic_intervals_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit('I can not create Genomic intervals track. Please check the validity of the entered data and required modules')
