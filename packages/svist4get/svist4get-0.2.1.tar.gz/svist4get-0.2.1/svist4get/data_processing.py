...
class Gtf_helper():
    ...
    def extract_data_around_ts(self, transcript_id, window, selected_transcripts=0):
        try:
            ...
            sys.exit('Unable to parse gtf file and find any annotation for the requested  site around transcript. Please check -gtf, -w  and -t parameters.')
        ...
        try:
            ...
            with open(self.name_of_file) as bedgraph:
                ...
                for line in bedgraph:
                    ...
                    if line[0] == self.name_of_scaffold and (int(line[1]) >= self.start >= int(line[2])) or (int(line[2]) <= self.end <= int(line[1])):
