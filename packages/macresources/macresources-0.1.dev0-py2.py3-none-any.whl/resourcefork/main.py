...
class ResourceAttrs(enum.IntFlag):
    ...
    The descriptions for these flags are taken from comments on the
    res*Bit and res* enum constants in <CarbonCore/Resources.h>.
    """
    
    _sysref = 1 << 7 # "reference to system/local reference" (only documented as resSysRefBit = 7 in <CarbonCore/Resources.h>
    sysheap = 1 << 6 # "In system/in application heap", "System or application heap?"
    purgeable = 1 << 5 # "Purgeable/not purgeable", "Purgeable resource?"
    locked = 1 << 4 # "Locked/not locked", "Load it in locked?"
    protected = 1 << 3 # "Protected/not protected", "Protected?"
    preload = 1 << 2 # "Read in at OpenResource?", "Load in on OpenResFile?"
    _changed = 1 << 1 # "Existing resource changed since last update", "Resource changed?"
    _compressed = 1 << 0 # "indicates that the resource data is compressed" (only documented in https://github.com/kreativekorp/ksfl/wiki/Macintosh-Resource-File-Format)
...
class Resource:
    ...
    def __init__(self, type, id, name='', attribs=0, data=None):
        self.type = type
        self.id = id
...
def parse_file(from_rsrcfork):
    if not from_rsrcfork: # empty resource forks are fine
        return

    data_offset, map_offset, data_len, map_len = struct.unpack_from('>4L', from_rsrcfork)

    typelist_offset, namelist_offset, numtypes = struct.unpack_from('>24xHHH', from_rsrcfork, map_offset)
    ...
    for i in range(numtypes):
        rtype, rtypen, reflist_offset = struct.unpack_from('>4sHH', from_rsrcfork, typelist_offset + 2 + 8*i)
        rtypen += 1
    ...
    for rtype, rtypen, reflist_offset in typelist:
        for i in range(rtypen):
            rid, name_offset, mixedfield = struct.unpack_from('>hHL', from_rsrcfork, reflist_offset + 12*i)
            rdata_offset = mixedfield & 0xFFFFFF
            rattribs = mixedfield >> 24

            rdata_offset += data_offset

            rdata_len, = struct.unpack_from('>L', from_rsrcfork, rdata_offset)
            rdata = from_rsrcfork[rdata_offset+4:rdata_offset+4+rdata_len]

            if name_offset == 0xFFFF:
                name = None
            else:
                name_offset += namelist_offset
                name_len = from_rsrcfork[name_offset]
                name = from_rsrcfork[name_offset+1:name_offset+1+name_len].decode('mac_roman')

            yield Resource(type=rtype, id=rid, name=name, attribs=rattribs, data=bytearray(rdata))


def parse_rez_code(from_rezfile):
    try:
        from_rezfile = from_rezfile.encode('mac_roman')
    except AttributeError:
        pass

    from_rezfile = from_rezfile.replace(b'\r\n', b'\n').replace(b'\r', b'\n')

    for line in from_rezfile.split(b'\n'):
        line = line.lstrip()
...
def make_file(from_iter):
    # honestly, this is pretty boring
