...
class _base_library:
    ...
    def find_pre_one(node):
        return _base_library.get_once(node.xpath('preceding-sibling::*[1]'))
    ...
    def find_next_one(node):
        return _base_library.get_once(node.xpath('following-sibling::*[1]'))
...
class _Element():
    ...
    def _build_Element(self, node):
        if isinstance(node, list):
            return [self._build_Element(n) for n in node]
        if not node is None:
