...
import os
import xlrd
from .common import FieldType,FieldInfo,getTypeByName,getCellName,getFieldSize,getFuncName,getTypeNameForCs,saveUtf8File,ensureDirExist,escapeCell

def ExcelToTs(srcFile, dstFile, csType, hasGT, strSeparator, supportTSLint):
    strCellName = ''
    try:
        ...
        #tableName = sheet.name.capitalize() # 发现这个函数会把第二个字符起的所有字符变成小写，这个是我不希望的
        if supportTSLint:
            TSLint = ' '
            MaxLineLength = 120
        else:
            TSLint = ''
            MaxLineLength = 0xffffffff
        ...
        if fieldList[0].type == FieldType.Index or fieldList[0].type == FieldType.Key or fieldList[0].type == FieldType.Map:
            hasIndexCol = True
        totalText += "export default class Cfg" + tableName + " {data:" + TSLint + "any=" + TSLint + "{\n"
        ...
        for row in range(idx_data, nrows):
            ...
            for i in range(n):
                ...
                if fieldType == FieldType.Remark:
                    ...
                    if isEmpty:
                        strVal = "0"
                    s += sep + TSLint + strFieldName + ":" + TSLint + strVal
                elif fieldType == FieldType.Map:
                    if hasIndexCol and (not hadWriteIndex):
                        s += strVal + ':' + TSLint + '{' + sep + TSLint + strFieldName + ':' + TSLint + '"' + strVal + '"'
                        hadWriteIndex = True
                    ...
                    if isEmpty:
                        ...
                        s += sep + TSLint + "get " + strFieldName + '()' + TSLint + '{return _GT("' + strVal + '");' + TSLint + '}'
                    else:
                        s += sep + strFieldName + ':' + TSLint + '"' + strVal + '"'
                    ...
                    if isEmpty:
                        ...
                        strVal = strVal.replace(strSeparator, ',' + TSLint)
                        s += sep + TSLint+ strFieldName + ':' + TSLint + '[' + strVal + ']'
                else:
                    s += sep + TSLint + strVal
                ...
                if TSLint != '':
                    if len(s) >= MaxLineLength: # 当前列数据太长，另起一行
                        if line != '   ':
                            totalText += line + '\n'
                        line = '   ' + s
                        totalText += line + '\n'
                        line = '   '
                    elif len(line) < MaxLineLength and len(line) + len(s) >= MaxLineLength:
                        if line != '   ':
                            totalText += line + '\n'
                        line = '   ' + s
                    else:
