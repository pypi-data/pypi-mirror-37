...
import os
import xlrd
from .common import FieldType,FieldInfo,getTypeByName,getFieldSize,getFuncName,getTypeNameForCpp,saveUtf8File,ensureDirExist
...
def ExcelToH(srcFile, dstFile, csType):
    ...
    for i in range(n):
        ...
        out += "    " + getTypeNameForCpp(fieldInfo.type).ljust(maxTypeLength) + (fieldInfo.name + ";").ljust(maxNameLength) + "// " + fieldInfo.desc + "\n"
    out += "};\n"
