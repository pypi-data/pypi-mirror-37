"use strict";
// Transcrypt'ed from Python, 2018-10-17 11:58:05
function main () {
	...
	__nest__ (
		__all__,
		'didery.static.transcrypt.components.fields', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.components.fields';
	...
	__nest__ (
		__all__,
		'didery.static.transcrypt.components.searcher', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.components.searcher';
	...
	__nest__ (
		__all__,
		'didery.static.transcrypt.components.tab', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.components.tab';
	...
	__nest__ (
		__all__,
		'didery.static.transcrypt.components.tabledtab', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.components.tabledtab';
					var tab =  __init__ (__world__.didery.static.transcrypt.components.tab);
	...
	__nest__ (
		__all__,
		'didery.static.transcrypt.components.tables', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.components.tables';
					var server =  __init__ (__world__.didery.static.transcrypt.server);
					var field =  __init__ (__world__.didery.static.transcrypt.components.fields);
						...
						get _view () {return __get__ (this, function (self) {
							...
							for (var field of self.fields) {
								...
								if (field == self.sortField) {
									if (self.py_reversed) {
										var icon = m ('i.angle.down.icon');
									}
									else {
										var icon = m ('i.angle.up.icon');
									}
					...
					var HistoryTable = __class__ ('HistoryTable', [Table], {
						...
						get _getField () {return __get__ (this, function (self, obj, field) {
							if (field.py_name == 'did') {
								...
								var signers = '';
								for (var signer of obj.history.signers) {
									signers += signer + ', ';
								}
								var signers = signers.__getslice__ (0, -(2), 1);
								return signers;
							}
							else if (field.py_name == 'signatures') {
								var signatures = '';
								for (var [key, value] of dict (obj.signatures).py_items ()) {
									signatures += value + ', ';
								}
								var signatures = signatures.__getslice__ (0, -(2), 1);
								return signatures;
							}
						});}
					});
					__pragma__ ('<use>' +
						'didery.static.transcrypt.components.fields' +
						'didery.static.transcrypt.server' +
					'</use>')
					__pragma__ ('<all>')
	...
	__nest__ (
		__all__,
		'didery.static.transcrypt.components.tabs', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.components.tabs';
					var tabledtab =  __init__ (__world__.didery.static.transcrypt.components.tabledtab);
					var tables =  __init__ (__world__.didery.static.transcrypt.components.tables);
	...
	__nest__ (
		__all__,
		'didery.static.transcrypt.dashboard', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.dashboard';
					var tabs =  __init__ (__world__.didery.static.transcrypt.components.tabs);
					var searcher =  __init__ (__world__.didery.static.transcrypt.components.searcher);
					var Manager = __class__ ('Manager', [object], {
						...
						get view () {return __get__ (this, function (self) {
							...
							return m ('div', m ('div.ui.top.attached.tabular.menu', m ('a.item.tab.hide', m ('span.menu-item-text', 'Server Status'), m ('i.chart.bar.icon'), m ('div.ui.label.small.menu-item-number', '0/0')), menu_items, m ('div.right.menu', m ('div.item', m ('form', dict ({'onsubmit': self.searchAll}), m ('div#search.ui.transparent.icon.input', m ('input[type=text][placeholder=Search...]', dict ({'id': self._searchId})), m ('button.ui.icon.button[type=submit]', m ('i.search.link.icon'))))))), tab_items);
						});}
					});
					__pragma__ ('<use>' +
						'didery.static.transcrypt.components.searcher' +
	...
	__nest__ (
		...
		'didery.static.transcrypt.server', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'didery.static.transcrypt.server';
					...
					var History = __class__ ('History', [object], {
						...
						get _parseAll () {return __get__ (this, function (self, data) {
							if (__in__ ('data', data)) {
								for (var [key, history] of enumerate (data ['data'])) {
	...
	__nest__ (
		...
		'router', {
			__all__: {
				...
				__init__: function (__all__) {
					var __name__ = 'router';
					var dashboard =  __init__ (__world__.didery.static.transcrypt.dashboard);
