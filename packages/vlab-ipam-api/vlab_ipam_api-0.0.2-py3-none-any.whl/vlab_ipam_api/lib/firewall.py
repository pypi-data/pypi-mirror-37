...
class FireWall(object):
    ...
    def prerouting(self, conn_port, target_addr):
        ...
        :type target_addr: String
        """
        run_cmd('sudo iptables -A PREROUTING -t nat -i ens160 -p tcp --dport {} -j DNAT --to {}'.format(target_port, target_addr))
        try:
            prerouting_id = self.find_rule(target_port, target_addr, table='nat')
        except RuntimeError:
            raise RuntimeError('Unable to find newly created PERROUTING rule for port {} to {}'.format(conn_port, target_addr))
        else:
    ...
    def find_rule(self, target_port, target_addr, table, conn_port=None):
        ...
        for rule_id, info in rules.items():
            if info['target_addr'] == target_add and info['target_port'] == target_port:
    ...
    def show(self, table='filter', format='json'):
        ...
        :param format: Set to 'raw' for unprocess output, or 'json' for just the important stuff
        :type format: String, default 'json'
        """
        with self._rlock():
            if table.lower() == 'nat':
