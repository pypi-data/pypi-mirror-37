...
import met2img_utils as met_utils #library for generating images and other ultis (find files,naming log, read parameters,...)
import models_def #models definitions

#get parameters from command line
options, args = met_utils.para_cmd()

#check if read parameters from configuration file
if options.config_file <> '':
    if os.path.isfile(options.config_file):
        met_utils.para_config_file(options)
        #print options
    else:
...
if __name__ == "__main__":   
    ...
    if options.search_already >0:
        prefix_search = met_utils.name_log_final(options, n_time_text= '*')        
        
        list_file = met_utils.find_files(prefix_search+'file*'+str(options.suff_fini)+'*',res_dir)
        
        if len(list_file)>0:
            print 'the result(s) existed at:'
            print list_file
            print 'If you would like to repeat this experiment, please set --search_already to 0'
            exit()
        else:
            if options.cudaid > -1:
                list_file = met_utils.find_files(prefix_search+'gpu'+ str(options.cudaid)+'file*ok*',res_dir)
                if len(list_file)>0:
                    print 'the result(s) existed at:'
                    print list_file
                    print 'If you would like to repeat this experiment, please set --search_already to 0'
    ...
    prefix = met_utils.name_log_final(options, n_time_text= str(time_text))
    #print prefix    
    print prefix 
    ...
    if options.test_exte == 1: #if use external validation set
    ...
    if options.type_emb == 'fill': 
        ...
        if not os.path.exists(path_write+'/fill_' + str(len(labels)-1)+'.png') or options.recreate_img > 0 :
            ...
            met_utils.fillup_image(cor_x=cordi_x,cor_y=cordi_y, X = mean_spe, type_data=options.type_data,
            ...
            for i in range(0, len(labels)):
                ...
                met_utils.fillup_image(cor_x=cordi_x,cor_y=cordi_y, X = data[i], type_data=options.type_data,
        ...
        #reading images
        data = met_utils.load_img_util(num_sample = len(data), pattern_img='fill',
        ...
        if options.test_exte == 1: #if use external validation set
            ...
            if not os.path.exists(path_write+'/testval_' + str(len(v_labels)-1)+'.png') or options.recreate_img > 0 :
                ...
                for i in range(0, len(v_labels)):
                    ...
                    met_utils.fillup_image(cor_x=cordi_x,cor_y=cordi_y, X = v_data_ori[i], type_data=options.type_data,
            ...
            #reading images
            v_data_ori = met_utils.load_img_util(num_sample = len(v_data_ori), pattern_img='testval',
                        path_write = path_write , dim_img = options.dim_img, preprocess_img = options.preprocess_img, 
                        channel = options.channel,mode_pre_img = options.mode_pre_img)       
        ...
        for index, (train_indices,val_indices) in enumerate(skf.split(data, labels)):
            ...
            if options.type_emb == "raw" or options.type_emb == "bin" : #reshape raw data into a sequence for ltsm model. 
                ...
                if options.type_emb == "bin" : 
                    if options.auto_v > 0: #if auto, adjust automatically min_v and max_v for binning
                    ...
                    for i in range(0, len(train_x)):   
                        #temp1 = [met_utils.convert_bin(y, debug=0, type=options.type_data, min_v = options.min_v, max_v = options.max_v, num_bin = options.num_bin) for y in train_x[1,:] ]
                        temp_train_x.append( [met_utils.convert_bin(value=y, debug=0, type=options.type_data, min_v = options.min_v, max_v = options.max_v, num_bin = options.num_bin) for y in train_x[i] ])
                    for i in range(0, len(val_x)):   
                        temp_val_x.append( [met_utils.convert_bin(value=y, debug=0, type=options.type_data, min_v = options.min_v, max_v = options.max_v, num_bin = options.num_bin) for y in val_x[i] ])
                    ...
                    if options.test_exte == 1:     
                        for i in range(0, len(v_data)):   
                            temp_v_data.append( [met_utils.convert_bin(value=y, debug=0, type=options.type_data, min_v = options.min_v, max_v = options.max_v, num_bin = options.num_bin) for y in v_data[i] ])
                ...
                if options.type_emb == 'fills' and options.type_data <> 'pr': #fills: use fill-up with bins created from train-set with scale, should be used with type_data='eqw'
                    ...
                    if (not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png")) or options.recreate_img > 0 :
                        ...
                        for i in range(0, len(train_y)):
                            ...
                            met_utils.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = train_x[i], type_data=options.type_data,
                        ...
                        for i in range(0, len(val_y)):
                            ...
                            met_utils.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = val_x[i], type_data=options.type_data,
                    ...
                    if options.test_exte == 1:
                        if (not os.path.exists(path_write+"/testval_" + str(len(v_labels)-1) + ".png")) or options.recreate_img > 0 :
                            for i in range(0, len(v_labels)):
                                print 'img fillup testval ' + str(i)            
                                met_utils.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = v_data[i], type_data=options.type_data,
                                    name_file= path_write+"/testval_"+str(i), fig_size = options.fig_size, 
                                    min_v = options.min_v, max_v = options.max_v, num_bin = options.num_bin, alpha_v = options.alpha_v,
                                    size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor , 
                                    colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)            
                    ...
                    if not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png") or options.recreate_img > 0 : 
                        ...
                        if options.imp_fea in ['rf']: 
                            ...
                            met_utils.embed_image(X_embedded [indices_imp], X = mean_spe[indices_imp], name_file = path_write+"/global",
                            ...
                            for i in range(0, len(train_x)):   
                                ...
                                met_utils.embed_image(X_embedded [indices_imp], X = train_x [i,indices_imp],name_file =path_write+"/train_"+str(i),
                                    marker=options.shape_drawn,size_p= options.point_size, fig_size = options.fig_size, type_data= options.type_data,
                                    margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                                    min_v = options.min_v, max_v=options.max_v, num_bin = options.num_bin,
                                    cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                            ...
                            for i in range(0, len(val_x)):                
                                met_utils.embed_image(X_embedded [indices_imp],X = val_x [i,indices_imp],name_file = path_write+"/val_"+str(i),
                                    marker=options.shape_drawn, size_p= options.point_size, fig_size = options.fig_size, type_data= options.type_data,
                                    margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                                    min_v = options.min_v, max_v=options.max_v, num_bin = options.num_bin, 
                                    cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                            ...
                            met_utils.embed_image(X_embedded, X = mean_spe, name_file = path_write+"/global",
                                marker=options.shape_drawn,size_p= options.point_size, fig_size = options.fig_size, type_data= options.type_data,
                                margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor,  colormap = options.colormap,
                                min_v = options.min_v, max_v=options.max_v, num_bin = options.num_bin, 
                                cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                            ...
                            #TRAIN: use X_embedded to create images; save labels
                            for i in range(0, len(train_x)):                       
                                met_utils.embed_image(X_embedded, X = train_x [i,:],name_file =path_write+"/train_"+str(i),
                                    marker=options.shape_drawn,size_p= options.point_size, fig_size = options.fig_size, type_data= options.type_data,
                                    margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                                    min_v = options.min_v, max_v=options.max_v, num_bin = options.num_bin,
                                    cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                            ...
                            #TEST: use X_embedded to create images; save labels               
                            for i in range(0, len(val_x)):                
                                met_utils.embed_image(X_embedded,X = val_x [i,:],name_file = path_write+"/val_"+str(i),
                                    marker=options.shape_drawn, size_p= options.point_size, fig_size = options.fig_size, type_data= options.type_data,
                                    margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                                    min_v = options.min_v, max_v=options.max_v, num_bin = options.num_bin, 
                                    cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                    ...
                    if options.test_exte == 1:
                        if not os.path.exists(path_write+"/testval_" + str(len(v_labels)-1) + ".png") or options.recreate_img > 0 : 
                            for i in range(0, len(v_data)):
                                ...
                                met_utils.embed_image(X_embedded,X = v_data [i,:],name_file = path_write+"/testval_"+str(i),
                                    marker=options.shape_drawn, size_p= options.point_size, fig_size = options.fig_size, type_data= options.type_data,
                                    margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                ...
                train_x = met_utils.load_img_util(num_sample = len(train_y), pattern_img='train',
                    path_write = path_write , dim_img = options.dim_img, preprocess_img = options.preprocess_img, 
                    channel = options.channel,mode_pre_img = options.mode_pre_img)
                val_x = met_utils.load_img_util(num_sample = len(val_y), pattern_img='val',
                    path_write = path_write , dim_img = options.dim_img, preprocess_img = options.preprocess_img, 
                    channel = options.channel,mode_pre_img = options.mode_pre_img)
                if options.test_exte==1:
                    v_data = met_utils.load_img_util(num_sample = len(v_labels), pattern_img='testval',
                        path_write = path_write , dim_img = options.dim_img, preprocess_img = options.preprocess_img, 
                        channel = options.channel,mode_pre_img = options.mode_pre_img)
            ...
            if options.model in ['none']: #only visual, not learning
                ...
                if options.visualize_model > 0:  #visualize architecture of model  
            ...
            if options.model in ['none']: #only visual, not learning
                ...
                if options.save_w == 1 : #save weights
