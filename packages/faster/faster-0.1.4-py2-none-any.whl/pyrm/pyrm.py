...
WIN_PIP_CONF_PATH = "%s\pip\pip.ini" % ( os.environ.get("APPDATA"))
WIN_PYRM_DIR = "%s\pip" % ( os.environ.get("APPDATA"))
...
def test_pip_list_speed():
    ...
    with open(PYRM_PATH) as pip_list_f:
        ...
        for pip_url in pip_list.items():
            ...
            if err:
                print(pip_url[0],url.strip(),err)
            else:
                print (pip_url[0],url.strip(),speed)
...
def get_fastest_pip():
    ...
    if pip_list:
        print ("\n\nBEST PIP : %s ,SPEED: %s" % (pip_list[0][0],pip_list[0][1]))
