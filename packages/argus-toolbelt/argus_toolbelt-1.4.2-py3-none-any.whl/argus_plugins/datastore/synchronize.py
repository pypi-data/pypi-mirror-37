...
def datastore_data(data: str) -> dict:
    ...
    for row in csv_reader:
        key = row[0].strip()
        value = row[1].strip() if len(row) > 1 else "N/A"
...
@register_command(extending="datastores")
def update(datastore: str, data: datastore_data, customer: get_customer_id = None):
    """Adds or updates entries from the data"""
    entries = [{"key": key, "value": value} for key, value in data.items()]
...
@register_command(extending="datastores")
def sync(datastore: str, data: datastore_data, customer: get_customer_id = None):
