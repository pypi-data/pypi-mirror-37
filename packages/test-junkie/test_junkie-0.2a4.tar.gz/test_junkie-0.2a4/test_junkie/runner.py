...
class Runner:
    ...
    def __run_test(suite, test, parameter=None, class_parameter=None, before_class_error=None, cancel=False):
        # TODO Remove test start time and assign it inside the function
    ...
    def __process_decorator(decorator_type, suite, test=None, parameter=None,  class_parameter=None):
        ...
        if DecoratorType.TEST_CASE != decorator_type:
            ...
            for func in functions_list:
                try:
                    ...
                    if "suite_parameter" in inspect.getargspec(func["decorated_function"]).args:
                        func["decorated_function"](func["decorated_function"], suite_parameter=class_parameter)
                    else:
            ...
            # deprecated but supports Python 2
            if "parameter" in inspect.getargspec(test.get_function_object()).args and \
                    "suite_parameter" in inspect.getargspec(test.get_function_object()).args:
                LogJunkie.debug("Test: {} takes both suite and test level parameters!".format(test.get_function_object()))
                test.get_function_object()(test.get_function_object(),
                                           parameter=parameter, suite_parameter=class_parameter)
            elif "suite_parameter" in inspect.getargspec(test.get_function_object()).args:
                LogJunkie.debug("Test: {} takes only suite level parameters!".format(test.get_function_object()))
                test.get_function_object()(test.get_function_object(), suite_parameter=class_parameter)
            elif "parameter" in inspect.getargspec(test.get_function_object()).args:
                LogJunkie.debug("Test: {} takes only test level parameters!".format(test.get_function_object()))
                test.get_function_object()(test.get_function_object(), parameter=parameter)
            else:
                LogJunkie.debug("Test: {} does not take any parameters!".format(test.get_function_object()))
                test.get_function_object()(test.get_function_object())
    ...
    def __process_event_listener(event_function, suite, test=None, param=None, class_param=None, error=None):
        ...
        def __create_properties():
            properties = {"suite_meta": suite.get_meta(),
                          ...
                          "junkie_meta": {}}
            # properties.update({"test_meta": test.get_kwargs()})
            if test:
                properties["test_meta"].update({"parameter": param})
            # properties.update({"suite_meta": suite.get_kwargs()})
