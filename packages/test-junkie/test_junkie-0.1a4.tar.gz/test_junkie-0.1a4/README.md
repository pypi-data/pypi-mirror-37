...
+ [Parallel Restriction](#parallel-test-execution): `@Suite(pr=[ATestSuite])`
...
+ `suite_multithreading`: Allows to run `N` number of suites in parallel. Default is 1 thread.
+ `test_multithreading`: Allows to run `N` number of test cases in parallel. Default is 1 thread.
...
You can, also, restrict parallel execution at the suite level. Lets say you have suites: `A`, `B`, `C` and suite `A` 
