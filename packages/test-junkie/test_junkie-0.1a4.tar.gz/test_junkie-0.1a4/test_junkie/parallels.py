...
class ParallelProcessor:
    ...
    def test_limit_reached(self, suite):

        active = 0
        parallels = ParallelProcessor.get_active_parallels().get(suite.get_class_object(), {})
        for test, parallel in parallels.get("tests", {}).items():
            if parallel.isAlive():
                active += 1
        LogJunkie.debug("Test limit: {}/{}".format(active, self.__test_limit))
        return active >= self.__test_limit
    ...
    def run_test_in_a_thread(func, suite, test, test_start_time, parameter, before_class_error):
        ...
        if suite not in ParallelProcessor.__PARALLELS:
            ParallelProcessor.__PARALLELS.update({suite.get_class_object(): {"thread": thread, "tests": {}}})
        ParallelProcessor.__PARALLELS[suite.get_class_object()]["tests"].update({test: thread})
        thread.start()
        return thread
