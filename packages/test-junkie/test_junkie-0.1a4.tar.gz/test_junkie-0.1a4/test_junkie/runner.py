    ...
    def run(self, **kwargs):
        ...
        while self.__suites:
            for suite in self.__suites:  # TODO wait due to remove call which changes list size?
                LogJunkie.info("1")
                suite_object = SuiteBuilder.get_execution_roster().get(suite, None)
                if suite_object is not None:
                    if self.__processor.suite_multithreading():
                        if self.__processor.suite_qualifies(suite_object):
                            self.__executed_suites.append(suite_object)  # TODO remove?
        ...
        for suite, parallel in self.__processor.get_active_parallels().items():
        ...
        if not suite.can_skip() and not self.__cancel:
            for suite_retry_attempt in range(1, suite.get_retry_limit() + 1):
                if suite_retry_attempt == 1 or suite.get_status() in SuiteCategory.ALL_UN_SUCCESSFUL:
                    ...
                    LogJunkie.debug("Suite Retry {}/{}".format(suite_retry_attempt, suite.get_retry_limit()))

                    before_class_error = Runner.__run_before_class(suite)
                    ...
                    for test in suite.get_test_objects():
                        ...
                        suite.get_listener().kwargs.update({"test_meta": test.get_kwargs()})
                        suite.get_listener().kwargs.update({"test_junkie_meta":
                        ...
                        if not Runner.__positive_skip_condition(suite, test) and \
                            ...
                            if not parameters or not isinstance(parameters, list):  # Validating parameters
                                ...
                                try:
                                    ...
                                    test.metrics.update_metrics(status=TestCategory.IGNORE,
                                                                ...
                                                                exception=error)
                                    Runner.__process_event_listener(suite.get_listener().on_ignore, error=error)
                            else:  # If parameters are valid, will continue processing the test
                                for param in parameters:
                                    if unsuccessful_tests is not None:
                                        if not test._is_qualified_for_retry(param):
                                            # If test does not qualify with current parameter, will move to the next
                                            continue
                                    test_start_time = time.time()  # actual start time of the test

                                    if self.__processor.test_multithreading():
                                        if self.__processor.test_limit_reached(suite):
                                            time.sleep(5)
                                        self.__processor.run_test_in_a_thread(self.__run_test,
                                                                              suite, test, test_start_time,
                                                                              param, before_class_error)
                                    else:
                            ...
                            Runner.__process_event_listener(suite.get_listener().on_skip)

                    if self.__processor.test_multithreading():
                        tests = self.__processor.get_active_parallels()[suite.get_class_object()]["tests"]
                        for test, parallel in tests.items():
                            parallel.join()
                    Runner.__run_after_class(suite)
            ...
            Runner.__process_event_listener(suite.get_listener().on_class_cancel)
        else:
            suite.metrics.update_suite_metrics(status=SuiteCategory.SKIP, start_time=suite_start_time)
            Runner.__process_event_listener(suite.get_listener().on_class_skip)

    def __run_test(self, suite, test, test_start_time, parameter=None, before_class_error=None):

        suite.get_listener().kwargs["test_junkie_meta"].update({"parameter": parameter})

        if before_class_error is not None or self.__cancel:
            _status = TestCategory.IGNORE if not self.__cancel else TestCategory.CANCEL
            _event = suite.get_listener().on_ignore if not self.__cancel else suite.get_listener().on_cancel
            test.metrics.update_metrics(status=_status,
                                        start_time=test_start_time,
                                        param=parameter,
                                        exception=before_class_error)
            Runner.__process_event_listener(_event, error=before_class_error)
            return
        ...
        for retry_attempt in range(1, test.get_retry_limit() + 1):
            ...
            try:
                ...
                Runner.__process_event_listener(suite.get_listener().on_success)
                return
            except Exception as error:
                ...
                if not isinstance(error, TestJunkieExecutionError):
                    ...
                    test.metrics.update_metrics(status=__category,
                                                ...
                                                param=parameter,
                                                exception=error)
                    Runner.__process_event_listener(__event, error=error)
                else:
                    raise error

    @staticmethod
    def __run_before_class(suite):
        ...
        try:
            ...
            if isinstance(error, AssertionError):
                Runner.__process_event_listener(suite.get_listener().on_before_class_failure, error=error)
            else:
                Runner.__process_event_listener(suite.get_listener().on_before_class_error, error=error)
            before_class_error = error
        return before_class_error

    @staticmethod
    def __run_after_class(suite):
        try:
            Runner.__process_decorator(decorator_type=DecoratorType.AFTER_CLASS, suite=suite)
            suite.get_rules().after_class()
        except Exception as error:
            if isinstance(error, AssertionError):
                Runner.__process_event_listener(suite.get_listener().on_after_class_failure, error=error)
            else:
                Runner.__process_event_listener(suite.get_listener().on_after_class_error, error=error)

    @staticmethod
    def __process_decorator(decorator_type, suite, test=None, parameter=None):
        ...
        if DecoratorType.TEST_CASE != decorator_type:
            ...
            if "parameter" in inspect.getargspec(test.get_function_object()).args:  # supports Python 2
                test.get_function_object()(test.get_function_object(), parameter=parameter)
            else:
                test.get_function_object()(test.get_function_object())
            test.metrics.update_metrics(status=TestCategory.SUCCESS, start_time=start_time, param=parameter)
