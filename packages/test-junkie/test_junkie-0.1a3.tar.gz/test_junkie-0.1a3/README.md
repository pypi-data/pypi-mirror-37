...
If you use both modes together, it will first spawn a thread to process a test suite and then that thread will 
spawn more threads for the tests that are in scope of that suite. Thread that is running a suite, wont exit until all 
of the tests in scope of the suite have finished running.
