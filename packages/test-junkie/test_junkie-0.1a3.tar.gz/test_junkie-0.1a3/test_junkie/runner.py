        ...
        for suite in self.__suites:
            ...
            if suite_object is not None:
                ...
                if self.__processor.suite_multithreading():
                    parallel = ParallelProcessor.run_suite_in_a_thread(self.__run_suite, suite_object)
                    parallels.update({suite_object: parallel})
                else:
                            ...
                            if not parameters or not isinstance(parameters, list):  # Validating parameters
                                ...
                                for param in parameters:
                                    ...
                                    if self.__processor.test_multithreading():
                                        parallel = self.__processor.run_test_in_a_thread(self.__run_test,
                                                                                         suite, test, test_start_time,
                                                                                         param, before_class_error)
                                        parallels.update({test: parallel})
                                    else:
