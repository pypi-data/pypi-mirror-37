...
class TestMetrics:
    ...
    def update_metrics(self, status, start_time, param=None, exception=None):
        string_param = str(param)
        if string_param not in self.__stats:
            self.__stats.update({string_param: {"status": None,
                                                "retry": 0,
                                                "performance": [],
                                                "exceptions": []}})
        self.__stats[string_param]["performance"].append(time.time() - start_time)
        self.__stats[string_param]["exceptions"].append(exception)
        self.__stats[string_param]["retry"] += 1
        self.__stats[string_param]["status"] = status
        self.__stats[string_param]["param"] = param
