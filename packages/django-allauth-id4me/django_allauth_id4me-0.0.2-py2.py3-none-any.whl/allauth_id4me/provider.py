...
class ID4meProvider(Provider):
    ...
    def _get_client(self, request):
        ...
        client_name = self.get_app(request).name
        if (app_type, client_name) in clientreg:
            ...
            client_id = self.get_app(request).client_id
            client = ID4meClient(
    ...
    def extract_common_fields(self, response):
        ...
        username = response['id_token'].get('id4me.identifier', self.extract_uid(response))
