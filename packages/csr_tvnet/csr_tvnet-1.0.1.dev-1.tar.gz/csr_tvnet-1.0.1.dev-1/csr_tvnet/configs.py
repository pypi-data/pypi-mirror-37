...
route-target export {RoutingProtocolASN}:{TunnelId}
route-target import {RoutingProtocolASN}:{TunnelId}
route-target import {OnpremRoutingProtocolASN}:{OnpremTunnelId}
ip vrf {OnpremConnName}-Tun-{OnpremTunnelId}
rd {OnpremRoutingProtocolASN}:{OnpremTunnelId}
route-target export {OnpremRoutingProtocolASN}:{OnpremTunnelId}
route-target import {OnpremRoutingProtocolASN}:{OnpremTunnelId}
route-target import {RoutingProtocolASN}:{TunnelId}
'''

onprem_routing_eigrp_bgp = '''
router eigrp {RoutingProtocolASN}
!
address-family ipv4 vrf {ConnName}-Tun-{TunnelId}
redistribute bgp {onpremRoutingProtocolASN} metric 100 1 255 1 1500
network {DMVPNTunnelIpNetworkNum} {DMVPNTunnelIpMask}
passive-interface default
no passive-interface Tunnel{TunnelId}
autonomous-system {RoutingProtocolASN}
...
exit-address-family
!
router bgp {onpremRoutingProtocolASN}
bgp log-neighbor-changes
!
address-family ipv4 vrf {OnpremConnName}-Tun-{OnpremTunnelId}
redistribute connected
{onpremBgpNeighbours}
exit-address-family
!
address-family ipv4 vrf {ConnName}-Tun-{TunnelId}
redistribute connected
redistribute eigrp {RoutingProtocolASN}
exit-address-family
'''

routing_onprem_bgp_neighbours = '''
neighbor {onpremIp} remote-as {onpremRoutingProtocolASN}
neighbor {onpremIp} activate
neighbor {onpremIp} next-hop-self
'''
