...
class Command(BaseCommand):
    def handle(self, *args, **options):
        call_command("fakenews_bake_data_feed")
        call_command("fakenews_bake_web_context")

        for fc in FactCheck.objects.filter(status="pub"):
            call_command("fakenews_bake_data_post", fc.id)

        for type in DisinformationType.objects.all():
            call_command("fakenews_bake_data_type", type.label)

        for tag in Tag.objects.all():
            call_command("fakenews_bake_data_tag", tag.slug)
