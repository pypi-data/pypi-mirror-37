...
class offsetvector(dict):
	...
	def __cmp__(self, other):
		...
		As explained above, the cmp() method is only present on Python 2:
		...
		"""
		warnings.warn(
			'Support for calling `cmp(a, b)` where `a` and `b` are instances '
			'of `offesetvector` is deprecated. Call `a.deltas != b.deltas '
			'instead. See <https://git.ligo.org/lscsoft/glue/issues/2>.')
