	...
	def add(self,table,dicts):
		...
		for x in dicts:
			...
			if dicts[x] is None:
				...
				value+=r'%s,'%(dicts[x]) if is_numeric(dicts[x]) else r'"%s",'%(self.pymysql.escape_string(dicts[x]))
		return self.execute(r''' insert `%s`(%s) values(%s) '''%(table,key[:-1],value[:-1]))
class redis(object):
	server=False
