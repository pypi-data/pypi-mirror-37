...
def setup_account(account):
    ...
    if nfields > 3:
        raise Exception("account argument ({}) wrong format: 'user:email:pass'".format(account))

    email = None

    if nfields == 3:
        username, email, password = account.split(':')
    elif nfields == 2:
    ...
    retval = api.Account(username, password, email)
    return retval
