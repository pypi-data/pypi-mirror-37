...
def reply2json(reply, ok=None):
    ...
    if reply.status_code != ok:
        ...
        e = exc.RemoteException.from_reply(reply)
        raise e
...
class Account(object):
    ...
    def __str__(self):
        ...
        return '"{}:{}:{}"'.format(self.username, self.email, pwd)
...
class Repo(object):
    ...
    def _apply_pwd(self, origin):
        ...
        if url.scheme == PROTO_HTTPS:
            ...
            email = self.manager.session.account.email
            password = self.manager.session.account.password
            return lurl.change_login(url, email, password).geturl()
