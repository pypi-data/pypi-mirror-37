...
class Patch(object):
    ...
    def skip_deleted_file(self):
        ...
        if self.is_binary():
            ...
            self.move(minus[1])
        if self.filename:
            self.results[self.filename] = {'new': False,
                                           'rm': True}
    ...
    def count_minus(self, count, old_count):
        self._condition(lambda x: x.startswith('-'))
        for line in self.get_lines:
            if line is None:
                break
            self.deleted[count] = (old_count, line)
            count += 1
            old_count += 1
        return old_count
    ...
    def parse_hunk(self, line):
        ...
        minus, plus = self.parse_numbers(line)
        new_count = plus[0]
        old_count = minus[0]
        ...
        for line in self.get_lines:
            ...
            if first == ' ':
                ...
                self.added[new_count] = line
                new_count += 1
            elif first == '-':
                # here we get the line number where the deleted lines
                # should be in the new file
                self.deleted[new_count] = (old_count, line)
                old_count = self.count_minus(new_count + 1, old_count + 1)
    ...
    def get_touched(self):
        added_lines = set(self.added.keys())
        deleted_lines = set(self.deleted.keys())
        touched_lines = added_lines & deleted_lines

        touched = {}
        for x in touched_lines:
            old = deleted[x]
            new = added[x]
            del added[x]
            del deleted[x]
