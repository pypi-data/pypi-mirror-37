...
VIP_PROPERTIES = ['ip_address']
LOADBALANCER_PROPERTIES = ['enabled']
...
def get_lb_and_graph(pdm, create=False, delete=False):
    ...
    lb_cdm = None
    if isinstance(pdm, provider_dm.LoadBalancer):
        vip_dm = common_dm.Vip(
            load_balancer_id=pdm.loadbalancer_id,
            ip_address=pdm.vip_address,
            subnet_id=pdm.vip_subnet_id,
            network_id=pdm.vip_network_id,
            port_id=pdm.vip_port_id)
        lb_cdm = common_dm.LoadBalancer(
            id=pdm.loadbalancer_id,
            project_id=pdm.project_id,
            name=pdm.name,
            description=pdm.description,
            enabled=pdm.admin_state_up,
            vip=vip_dm)
        vip_dm.load_balancer = lb_cdm
    else:
        lb_cdm = dl.loadbalancer_repo.get(dl.db_session, id=lb_id)

    graph = {}
    for prop in VIP_PROPERTIES:
        graph[prop] = getattr(lb_cdm.vip, prop, PROPERTY_DEFAULTS.get(prop))
    graph['vip_address'] = graph.pop('ip_address')
    graph['pip_address'] = graph['vip_address']

    for prop in LOADBALANCER_PROPERTIES:
        graph[prop] = getattr(lb_cdm, prop, PROPERTY_DEFAULTS.get(prop))
    graph['admin_state_up'] = graph.pop('enabled')
    ...
    for p in pools:
        ...
        for member in members:
            ...
            _accomplish_member_static_route_data(member, lb_cdm, m_dict)
            p_dict['members'].append(m_dict)
        graph['pools'].append(p_dict)

    return lb_cdm, statuses, graph


def _accomplish_member_static_route_data(member, lb_cdm, m_dict):
    n_driver = common_utils.get_network_driver()
    vip_subnet = n_driver.get_subnet(lb_cdm.vip.subnet_id)
    member_subnet = n_driver.get_subnet(member.subnet_id)
    if netaddr.IPAddress(member.address) not in netaddr.IPNetwork(vip_subnet.cidr):
