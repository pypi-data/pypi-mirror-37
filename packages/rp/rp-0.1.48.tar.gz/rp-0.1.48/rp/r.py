...
# except:pass
def ximage_to_xyrgb_lists(image):
...
def pseudo_terminal(*dicts,get_user_input=python_input,modifier=None,style=pseudo_terminal_style(),enable_ptpython=True,eval=eval,exec=exec):
    ...
    try:
        ...
        Enter 'MODIFIER ON', 'MODIFIER OFF', 'VARS', 'MORE', 'RETURN NOW', 'EDIT', 'GHISTORY', 'COPY', 'SPASTE', 'CHISTORY', 'DITTO', 'LEVEL', 'PREV', 'UNDO', 'PT ON', 'PT OFF','RANT'""","blue")
...
def cv_imshow(img,label="cvImshow",imgIsRgb=True,wait=10):
    #A non-blocking image display, using OpenCV
    import cv2
    if imgIsRgb:
        img=img+0
        temp=img+0
        img[:,:,0],img[:,:,2]=temp[:,:,2],temp[:,:,0]
