...
el_chrg_json='/rk2/knc6/DUMP/MLstuff/lgbm/element_charge.json'
el_chem_json='/rk2/knc6/JARVIS-DFT/MDCS/make_desciptors/Elements10b.json'

#el_chrg_json='/home/knc6/bin/element_charge.json'
#el_chem_json='/home/knc6/bin/Elements10b.json'
...
def get_prdf(s=None,comb='',cutoff=10.0,intvl=0.1,rdf_tol=0.1,filename='prdf.png'):
...
def rdf_ang_dist(s='',c_size=10.0,plot=True):
    x,y,z=get_rdf(s)
    arr=[]
    for i,j in zip(x,z):
      if j>0.0:
    ...
    angs=ang_at.keys()
    #print "Angs",angs
    norm=[float(len(i))/float(len(set(i))) for i in ang_at.values()]
    ...
    dih1=set(dih)
    #print "dih",dih1
    norm=[float(len(i))/float(len(set(i))) for i in dih_at.values()]
...
def uni_en(el='Na'):
   en='na'
   f=open('/rk2/knc6/JARVIS-DFT/uni.json','r')
...
def get_comp_descp(struct='',jcell=True,jmean_chem=True,jmean_chg=True,jrdf=False,jrdf_adf=True,print_names=False):  
       ...
       try: 
        ...
        pass
       #print ('ne cat',len(cat))
       #cat=np.array(list(cat[0:920])+list(cat[1457:1557]))
       return cat
...
desc=get_comp_descp(s)
print (len(desc))
test_str=Structure(Lattice([[4,0,0],[0,4,0],[0,0,4]]),['Si']*2,[[0, 0 ,0],[.5,.5,.5]])
desc=get_comp_descp(test_str)
print len(desc)
s=Structure.from_file('POSCAR')
print s
print
print get_effective_structure(s)

from scipy.spatial import distance

f=open('Elements10b.json','r')
d=json.load(f)
f.close()
#for k,v in d.items():
test_str=Structure(Lattice([[4,0,0],[0,4,0],[0,0,4]]),[k]*2,[[0, 0 ,0],[.5,.5,.5]])
desc=get_comp_descp(test_str)
mynewlist = [s for s in desc if s.isdigit()]
#print len(get_comp_descp(test_str))


s=Structure.from_file('POSCAR')
alfcc=get_comp_descp(s)
#print alfcc,len(alfcc)
alfcc1=get_comp_descp(s,print_names=True)
#print alfcc,len(alfcc)
order=np.argsort(alfcc)
print alfcc[order]
print np.array(alfcc1)[order]
#s=Structure.from_file('POSCAR-mp-149_Si.vasp')
#silic=get_comp_descp(s)

#s.make_supercell([2,3,5])
#print s.cart_coords
#s1=get_effective_structure(s)
#print s1.cart_coords
#c=get_prdf(s=s)
#print 'c',c
#rdf_ang_dist(s=s)
...
print 'len',len(q)
s=Structure.from_file('POSCAR')
svn=SpacegroupAnalyzer(s).get_conventional_standard_structure(s)
#svn.make_supercell([2,2,2])
q=get_comp_descp(s)
qsvn=get_comp_descp(svn)
nm=get_comp_descp(s,print_names=True)
print 'len',len(q)
print (q==qsvn).all()
for i,j,k in zip(q,qsvn,nm):
  if i!=j:
   print i,j,k
#nm=get_comp_descp(s,print_names=True)
print len(q)
...
mem.append(p)
mem.append(q)
dat=pd(mem,columns=nm)
dat['JVASP']=['JVASP-1','JVASP-2']
print dat
"""
