...
def init_data(file_name, language):
    ...
    file_set.write("g++ " + file_name + "-soure\\" + file_name + "-data." + language + " -o " + file_name + "-soure\\" + file_name + "-data" + ".exe\n")
    file_set.write("g++ " + file_name + "-soure\\" + file_name + "-ans." + language + " -o " + file_name + "-soure\\" + file_name + "-ans" + ".exe\n")
    file_set.write(".in")
    file_set.write(".ans")
    file_set.write(file_name)
...
def work(argv):
    ...
    data_name = data_in.readline().strip()
    if format_in == '_' : format_in = ''
    if format_ans == '_' : format_ans = ''
    if data_name == '_' : data_name = ''
    i = 0
    while ram != "EOF":
        ram = data_in.readline().strip()
        if ram == "EOF" : break
        if i < st : continue
        output(ram)
        os.system(datamaker_name + "< " + " data_ini.yaml " + "> " + file_name + "-data\\" + data_name + str(i) + format_in)
        os.system(ansmaker_name + "< " + file_name + "-data\\" + data_name + str(i) + format_in + " > " + file_name + "-data\\" + data_name + str(i) + format_ans)
        i += 1
        if i>en : break
    print("OK!")
