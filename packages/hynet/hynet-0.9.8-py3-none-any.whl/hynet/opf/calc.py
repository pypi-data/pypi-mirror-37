...
from hynet.reduction.copperplane import reduce_to_copperplane
...
def calc_opf(data, scenario_id=0, solver=None, solver_type=None):
    ...
    **Copperplane Models:** *hynet* supports the simulation of "copperplane
    ...
    grid model of a scenario can be reduced to a copperplane using
    ``reduce_to_copperplane``.
    ...
    hynet.reduction.copperplane.reduce_to_copperplane
    """
    timer = Timer()
    ...
    if solver.type == SolverType.QCQP and model.dim_v > len(model.islands):
        ...
        qcqp.initial_point = get_copperplane_initial_point(model, qcqp)
...
def get_relaxation_initial_point(model, qcqp, solver_type):
    ...
    SOCR-based initialization turned out to be superior to a copperplane-based
    initialization.
...
def get_copperplane_initial_point(model, qcqp):
    """
    Return a copperplane-based initial point for the given OPF QCQP.

    This method returns an initial point for the solution of the OPF QCQP that
    includes the optimal dispatch of the copperplane reduction of the model.
    The copperplane solution is typically fast to compute and can reduce the
    ...
    -------
    initial_point : QCQPPoint or None
        Copperplane model based initial point for the given OPF QCQP. In case
        the solution failed or no SOCR solver was found, None is returned.
    """
    timer = Timer()
    initial_point = None
    try:
        cp = SystemModel(reduce_to_copperplane(model.scenario),
        ...
        _log.warning("Copperplane initialization failed: " + str(exception))
    _log.debug("Copperplane initial point calculation ({:.3f} sec.)"
               .format(timer.total()))
    return initial_point
