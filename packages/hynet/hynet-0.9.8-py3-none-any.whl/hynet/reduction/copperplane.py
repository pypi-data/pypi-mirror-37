"""
Network reduction to a "copperplane".
"""
...
def reduce_to_copperplane(scenario):
    """
    Return a deep copy of the scenario with the grid reduced to a copperplane.

    In the "copperplane" reduction, every connected grid is reduced to a single
    bus, i.e., the impact of the power grid is neglected and the network laws
    are reduced to simply balance the total injection and total load.
    """
    timer = Timer()
    cpr = Scenario()
    cpr.id = scenario.id
    ...
    cpr.grid_name = "COPPERPLANE REDUCTION"
    ...
    _log.debug("Copperplane reduction ({:.3f} sec.)".format(timer.total()))
    return cpr
