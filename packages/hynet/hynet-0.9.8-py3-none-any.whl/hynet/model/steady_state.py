...
class SystemModel:
    ...
    **Custom OPF Formulations:** The steady-state model serves as a generator
    for the OPF problem. In order to customize the OPF formulation, one can
    define a corresponding steady-state model by deriving from this class,
    implementing the additional constraint generators, registering them by
    overriding ``_get_constraint_generators``, and, if desired, overriding
    ...
    for the customized OPF, create an object of the specialized steady-state
    model for the desired scenario and pass it to ``calc_opf``.
    ...
    def __init__(self, scenario, verify_scenario=True):
        """
        Initialize the steady-state model with the scenario.
    ...
    @property
    def scenario(self):
        """Return the scenario data of the steady-state model."""
        return self._scr
    ...
    def get_opf_problem(self):
        """
        Return the QCQP for the OPF problem of this steady-state model.
