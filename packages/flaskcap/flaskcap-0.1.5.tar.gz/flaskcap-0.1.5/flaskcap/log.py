...
if os.name == 'posix':
    ...
    def flock(f, flags=LOCK_EX):
        _file = win32file._get_osfhandle(f.fileno())
...
class NaturalTimedRotating(BaseRotatingHandler):
    ...
    def computeRollover(self, currentTime):
        ...
        if self.when == 'M':
            result = currentTime + 60 - t.tm_sec
        # rotate according natural Hour
        elif self.when == 'H':
            result = currentTime + 3600 - (t.tm_min * 60) - t.tm_sec
        # rotate according natural Day
        elif self.when == 'D':
            result = currentTime + (24 - t.tm_hour) * 3600 - t.tm_min * 60 - t.tm_sec
