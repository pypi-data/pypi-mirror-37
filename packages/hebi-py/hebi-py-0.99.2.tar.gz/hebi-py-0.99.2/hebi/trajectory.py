...
def create_trajectory(time, position, velocity=None, acceleration=None):
  ...
  for i in range(0, joints):
    ...
    vel_offset = _ctypes.cast(_ctypes.byref(velocity_c.contents, i * waypoints * 8), _c_double_p)
    acc_offset = _ctypes.cast(_ctypes.byref(acceleration_c.contents, i * waypoints * 8), _c_double_p)
  ...
  return _trajectory.Trajectory(trajectories, waypoints, time[0], time[-1])
