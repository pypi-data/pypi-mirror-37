...
class HtmlStorage:
    ...
    @staticmethod
    def save_content(content, save_img=True, remove_a=True, send_qiniu=True):
        """
        保存html中图片 并提取最一张图片
        :param content:
        :param remove_a:
        ...
        first_img = ''
        for img in all_imgs:
            file_path = HtmlStorage.image_parse(img, dir_name, send_qiniu, save_img)
            if first_img == '':
                first_img = file_path
        html_content = ''
    ...
    @staticmethod
    def image_parse(img_tag, dir_name='', send_qiniu=False, save_img=True):
        if not os.path.exists(settings.MEDIA_ROOT + dir_name):
            os.makedirs(settings.MEDIA_ROOT + dir_name)
        try:
            img_obj = parse.urlparse(img_tag['src'])
            img_path = img_obj.path

            # 已经是本站文件不保存
            if not str(img_path).startswith(settings.MEDIA_URL) and not str(img_path).startswith(
                    settings.QINIU_URL) and save_img:
                # 判断是不是微信图片
