...
class AutoTask(models.Model):
    ...
    lazy_photo = models.CharField('图片SRC属性', help_text='有延时图片时,不取src值,使用图片其它属性', max_length=128, blank=True, null=True, default='')
    ...
    def operate_crawl(self):
        return format_html('<a href="###" onclick="crawl_news({id})">执行任务</a>'.format(id=self.id))
