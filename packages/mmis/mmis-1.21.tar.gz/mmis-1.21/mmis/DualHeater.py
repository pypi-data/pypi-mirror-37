...
class Settings(wx.Panel):
    ...
    def __init__(self, parent, Module, pubsub1, pubsub2):
        ...
        self.SET_Ki_Amb = wx.SpinCtrlDouble(self, size=(150,-1), min =-400, max = 800, inc = 0.1, value='8.00', pos = (460,25))
        self.SET_Ki_Amb.SetDigits(3)
        ...
        self.SET_Ki_N2 = wx.SpinCtrlDouble(self, size=(150,-1), min =-400, max = 800, inc = 0.1, value='8.00', pos = (460,80))
        self.SET_Ki_N2.SetDigits(3)
    ...
    def ON_GET_INFO(self, e):
        ...
        self.Info.AppendText('Kp_Amb' + ' = ' + str(self.Kp_Data_Amb)[:6] + '\n')
        self.Info.AppendText('Ki_Amb' + ' = ' + str(self.Ki_Data_Amb)[:6] + '\n')
        self.Info.AppendText('Kp_N2' + ' = ' + str(self.Kp_Data_N2)[:6] + '\n')
        self.Info.AppendText('Ki_N2' + ' = ' + str(self.Ki_Data_N2)[:6])
...
class Main(wx.Panel):
    ...
    def __init__(self, parent, Module, pubsub1, pubsub2):
        ...
        self.button1.Bind(wx.EVT_BUTTON, self.Increase1)
        self.button1.SetForegroundColour('black')

        # Decrease button - Temperature control
        self.button2 = wx.Button(self, label = '-', size = (40,40), id=2)
        self.button2.SetFont(self.font1)
        self.button2.Bind(wx.EVT_BUTTON, self.Decrease1)
        self.button2.SetForegroundColour('black')

        # Increase button - Temperature control
        self.button7 = wx.Button(self, label = '+', size = (40,40), id=11)
        self.button7.SetFont(self.font1)
        self.button7.Bind(wx.EVT_BUTTON, self.Increase2)
        self.button7.SetForegroundColour('black')

        # Decrease button - Temperature control
        self.button8 = wx.Button(self, label = '-', size = (40,40), id=12)
        self.button8.SetFont(self.font1)
        self.button8.Bind(wx.EVT_BUTTON, self.Decrease2)
    ...
    def Increase2(self,e):
        self.pos = self.four.GetInsertionPoint()
        print (self.pos)
        var = self.four.GetValue()
        if var[0]=='-':
        ...
        if self.pos == 4:
            self.temp = self.four.GetValue()
            self.temp = self.temp + 1.0
            self.four.SetValue(self.temp)
        elif self.pos == 3:
            self.temp = self.four.GetValue()
            self.temp = self.temp + 10.0
            self.four.SetValue(self.temp)
            ...
            print ('out of range')
        self.four.SetInsertionPoint(self.pos)
        
        return self.pos
        
    def Decrease2(self,e):
