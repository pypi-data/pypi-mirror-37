...
def insert_on_duplicate_key(table, field):
    return insert(table, field) + " ON DUPLICATE KEY UPDATE " + ", ".join(["`{0}`=VALUES(`{0}`)".format(f) for f in field])
...
def init_db(host='localhost', user='root', password='root', port=3306, database='test', charset='utf8', alias=None, max_conn=10):
    if alias is None:
        alias = database
