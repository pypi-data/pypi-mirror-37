...
class DB:
    def __init__(self, host, user, password, port, database, charset='utf8', max_conn=10):
        self.db_config = {
            'host': host,
            'user': user,
            'password': password,
    ...
    def exe(self, sql, args=None):
        return self.session().exe(sql, args)
...
def init_db(host, user, password, port, database, alias=None):
    if alias is None:
        alias = database
    db_dict[alias] = DB(host, user, password, port, database)
