...
webapp3 offers a powerful and extensible system to match and build URIs,
which is explained in details in this section.


Simple routes
-------------
The simplest form of URI route in webapp3 is a tuple ``(regex, handler)``,
...
register three routes that point to those handlers::
    ...
    class ProductListHandler(webapp3.RequestHandler):
        def get(self):
...
redirection and other useful features. For this, webapp3 offers the extended
routing mechanism that we'll see next.


Extended routes
---------------
webapp3 introduces a routing mechanism that extends the webapp model to provide
additional features:
...
:class:`webapp3.Route`. Let's remake our previous routes using it::

    app = webapp3.WSGIApplication([
        webapp3.Route(r'/', handler=HomeHandler, name='home'),
        webapp3.Route(r'/products', handler=ProductListHandler, name='product-list'),
        webapp3.Route(r'/products/<product_id:\d+>', handler=ProductHandler, name='product'),
    ])
...
Check :meth:`webapp3.Route.__init__` in the API reference for the parameters
...
:ref:`guide.handlers`. For example::
    ...
    webapp3.Route(r'/products', handler='handlers.ProductsHandler', name='products-list', handler_method='list_products')
...
separated by a colon. This is equivalent to the previous example::
    ...
    webapp3.Route(r'/products', handler='handlers.ProductsHandler:list_products', name='products-list')
...
405. For example::
    ...
    webapp3.Route(r'/products', handler='handlers.ProductsHandler', name='products-list', methods=['GET'])
...
:class:`webapp3.RequestHandler` does.
...
only. For this, set the ``schemes`` parameter when defining a route::
    ...
    webapp3.Route(r'/products', handler='handlers.ProductsHandler', name='products-list', schemes=['https'])
...
using a special route class provided in the :mod:`webapp3_extras.routes`
module: the :class:`webapp3_extras.routes.DomainRoute`. It is initialized with
a pattern to match the current server name and a list of nested
:class:`webapp3.Route` instances that will only be tested if the domain or
subdomain matches.

For example, to restrict routes to a subdomain of the appspot domain::

    import webapp3
    from webapp3_extras import routes

    app = webapp3.WSGIApplication([
        routes.DomainRoute('<subdomain>.app-id.appspot.com', [
            webapp3.Route('/', handler=SubdomainHomeHandler, name='subdomain-home'),
        ]),
...
different routes for other subdomains. The webapp3 routing system can handle
this easily.
...
Path prefix routes
------------------
The :mod:`webapp3_extras.routes` provides a class to wrap routes that start
with a common path: the :mod:`webapp3_extras.routes.PathPrefixRoute`.
...
We could refactor them to reuse the common path prefix::
    ...
    app = WSGIApplication([
        routes.PathPrefixRoute('/users/<user:\w+>', [
            webapp3.Route('/', UserOverviewHandler, 'user-overview'),
            webapp3.Route('/profile', UserProfileHandler, 'user-profile'),
            webapp3.Route('/projects', UserProjectsHandler, 'user-projects'),
        ]),
    ])
...
Other prefix routes
-------------------
The :mod:`webapp3_extras.routes` has other convenience classes that accept
nested routes with a common attribute prefix:

- :mod:`webapp3_extras.routes.HandlerPrefixRoute`: receives a handler module
  prefix in dotted notation and a list of routes that use that module.
- :mod:`webapp3_extras.routes.NamePrefixRoute`: receives a handler name
...
This is done using the function :func:`webapp3.uri_for` or the method
:meth:`webapp3.RequestHandler.uri_for` inside a handler, or calling
:meth:`webapp3.Router.build` directly (a ``Router`` instance is set as an
attribute ``router`` in the WSGI application).

For example, if you have these routes defined for the application::

    app = webapp3.WSGIApplication([
        webapp3.Route('/', handler='handlers.HomeHandler', name='home'),
        webapp3.Route('/wiki', handler=WikiHandler, name='wiki'),
        webapp3.Route('/wiki/<page>', handler=WikiHandler, name='wiki-page'),
    ])
...
Check :meth:`webapp3.Router.build` in the API reference for a complete
