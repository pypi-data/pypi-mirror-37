...
class TestResponse(test_base.BaseTestCase):
    def test_write(self):
        ...
        rsp.write(var_2)
        rsp.write(var_3)
        self.assertEqual(rsp.body, webapp3._to_utf8('%rfoobar' % var_1))

        rsp = webapp3.Response()
        rsp.write(var_1)
        rsp.write(var_3)
        rsp.write(var_2)
        self.assertEqual(rsp.body, webapp3._to_utf8('%rbarfoo' % var_1))

        rsp = webapp3.Response()
        rsp.write(var_2)
        rsp.write(var_1)
        rsp.write(var_3)
        self.assertEqual(rsp.body, webapp3._to_utf8('foo%rbar' % var_1))

        rsp = webapp3.Response()
        rsp.write(var_2)
        rsp.write(var_3)
        rsp.write(var_1)
        self.assertEqual(rsp.body, webapp3._to_utf8('foobar%r' % var_1))

        rsp = webapp3.Response()
        rsp.write(var_3)
        rsp.write(var_1)
        rsp.write(var_2)
        self.assertEqual(rsp.body, webapp3._to_utf8('bar%rfoo' % var_1))

        rsp = webapp3.Response()
        rsp.write(var_3)
        rsp.write(var_2)
        rsp.write(var_1)
        self.assertEqual(rsp.body, webapp3._to_utf8('barfoo%r' % var_1))
    ...
    def test_wsgi_write(self):
        ...
        rsp = webapp3.Response(body='Page not found!', status=404)
        rsp.wsgi_write(start_response)

        rsp = webapp3.Response(status=res[0], body=res[2], headers=res[1])
        self.assertEqual(rsp.status, '404 Not Found')
...
class TestReturnResponse(unittest.TestCase):
    def test_function_that_returns_response(self):
        def myfunction(request, *args, **kwargs):
            return webapp3.Response('Hello, custom response world!')
