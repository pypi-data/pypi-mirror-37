...
import webapp3
from webapp3 import BaseRoute, Request, Route, Router
...
class TestRoute(BaseTestCase):
    ...
    def test_set_adapter(self):
        def custom_adapter(router, handler):
            class MyAdapter(webapp3.BaseHandlerAdapter):
        ...
        def myhandler(request, *args, **kwargs):
            return webapp3.Response('hello')

        app = webapp3.WSGIApplication([('/', myhandler)])
    ...
    def test_methods(self):
        ...
        self.assertTrue(router.match(req) is not None)
        req.method = 'PUT'
        self.assertRaises(webapp3.exc.HTTPMethodNotAllowed, router.match, req)
...
class TestSimpleRoute(BaseTestCase):
    def test_no_variable(self):
        router = webapp3.Router([(r'/', 'my_handler')])

        matched_route, args, kwargs = router.match(webapp3.Request.blank('/'))
        self.assertEqual(args, ())
        self.assertEqual(kwargs, {})

    def test_simple_variables(self):
        router = webapp3.Router([(r'/(\d{4})/(\d{2})', 'my_handler')])

        matched_route, args, kwargs = router.match(
            webapp3.Request.blank('/2007/10'))

        self.assertEqual(args, ('2007', '10'))
        self.assertEqual(kwargs, {})

    def test_build(self):
        route = webapp3.SimpleRoute('/', None)
    ...
    def test_route_repr(self):
        ...
        self.assertEqual(
            str(webapp3.SimpleRoute(r'/<foo>', None)),
            "<SimpleRoute('/<foo>', None)>"
        )
