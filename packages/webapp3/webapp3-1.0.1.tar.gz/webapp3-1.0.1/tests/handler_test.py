...
"""
Tests for webapp3 webapp3.RequestHandler
"""
import os
import sys
import unittest
...
class HomeHandler(webapp3.RequestHandler):
    def get(self, **kwargs):
...
class RedirectToHandler(webapp3.RequestHandler):
    def get(self, **kwargs):
...
class RedirectAbortHandler(webapp3.RequestHandler):
    def get(self, **kwargs):
        self.response.headers.add_header('Set-Cookie', 'a=b')
        self.redirect('/somewhere', abort=True)


class BrokenHandler(webapp3.RequestHandler):
    def get(self, **kwargs):
...
class PositionalHandler(webapp3.RequestHandler):
    def get(self, month, day, slug=None):
        self.response.out.write('%s:%s:%s' % (month, day, slug))


class HandlerWithError(webapp3.RequestHandler):
    def get(self, **kwargs):
        self.response.out.write('bla bla bla bla bla bla')
        self.error(403)


class InitializeHandler(webapp3.RequestHandler):
    def __init__(self):
        pass
...
class WebDavHandler(webapp3.RequestHandler):
...
class AuthorizationHandler(webapp3.RequestHandler):
    def get(self):
        self.response.out.write('nothing here')


class HandlerWithEscapedArg(webapp3.RequestHandler):
    def get(self, name):
...
app = webapp3.WSGIApplication([
    ('/bare', BareHandler),
    webapp3.Route('/', HomeHandler, name='home'),
    webapp3.Route('/methods', MethodsHandler, name='methods'),
    webapp3.Route('/broken', BrokenHandler),
    webapp3.Route('/broken-but-fixed', BrokenButFixedHandler),
    webapp3.Route('/<year:\d{4}>/<month:\d{1,2}>/<name>', None,
                  name='route-test'),
    webapp3.Route('/<:\d\d>/<:\d{2}>/<:\w+>', PositionalHandler,
                  name='positional'),
    webapp3.Route('/redirect-me', webapp3.RedirectHandler,
                  defaults={'_uri': '/broken'}),
    webapp3.Route('/redirect-me2', webapp3.RedirectHandler,
                  defaults={'_uri': get_redirect_url}),
    webapp3.Route('/redirect-me3', webapp3.RedirectHandler,
                  defaults={'_uri': '/broken', '_permanent': False}),
    webapp3.Route('/redirect-me4', webapp3.RedirectHandler,
                  defaults={'_uri': get_redirect_url, '_permanent': False}),
    webapp3.Route('/redirect-me5', RedirectToHandler),
    webapp3.Route('/redirect-me6', RedirectAbortHandler),
    webapp3.Route('/lazy', 'tests.resources.handlers.LazyHandler'),
    webapp3.Route('/error', HandlerWithError),
    webapp3.Route('/initialize', InitializeHandler),
    webapp3.Route('/webdav', WebDavHandler),
    webapp3.Route('/authorization', AuthorizationHandler),
    webapp3.Route('/escape/<name:.*>', HandlerWithEscapedArg, 'escape'),
], debug=False)
...
class TestHandler(BaseTestCase):
    ...
    def test_404(self):
        req = webapp3.Request.blank('/nowhere')
    ...
    def test_500(self):
        req = webapp3.Request.blank('/broken')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 500)

    def test_500_but_fixed(self):
        req = webapp3.Request.blank('/broken-but-fixed')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
    ...
    def test_positional(self):
        req = webapp3.Request.blank('/07/31/test')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
        self.assertEqual(rsp.body, b'07:31:test')

        req = webapp3.Request.blank('/10/18/wooohooo')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
        self.assertEqual(rsp.body, b'10:18:wooohooo')

    def test_redirect(self):
        req = webapp3.Request.blank('/redirect-me')
        rsp = req.get_response(app)
    ...
    def test_redirect_with_callable(self):
        req = webapp3.Request.blank('/redirect-me2')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 301)
        self.assertEqual(rsp.body, b'')
    ...
    def test_redirect_not_permanent(self):
        req = webapp3.Request.blank('/redirect-me3')
        rsp = req.get_response(app)
    ...
    def test_redirect_with_callable_not_permanent(self):
        req = webapp3.Request.blank('/redirect-me4')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 302)
        self.assertEqual(rsp.body, b'')
        self.assertEqual(rsp.headers['Location'], 'http://localhost/methods')
    ...
    def test_redirect_to(self):
        req = webapp3.Request.blank('/redirect-me5')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 302)
        self.assertEqual(rsp.body, b'')
        self.assertEqual(
    ...
    def test_redirect_abort(self):
        req = webapp3.Request.blank('/redirect-me6')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 302)
        self.assertEqual(
            rsp.body,
    ...
    def test_run(self):
        ...
        with mock.patch('webapp3.handlers.sys.stdin') as patched_stdin:
            with mock.patch('webapp3.handlers.sys.stdout') as patched_stdout:
    ...
    def test_get_valid_methods(self):
        req = webapp3.Request.blank('http://localhost:80/')
        req.app = app
    ...
    def test_uri_for(self):
        ...
        req = webapp3.Request.blank('http://localhost:80/')
        req.route = webapp3.Route('')
        req.route_args = tuple()
        req.route_kwargs = {}
        req.app = app
        app.set_globals(app=app, request=req)
        handler = Handler(req, webapp3.Response())
        handler.app = app
    ...
    def test_extra_request_methods(self):
        ...
        for method in webdav_methods:
            ...
            req = webapp3.Request.blank('/webdav')
            req.method = method
            rsp = req.get_response(app)
            self.assertEqual(rsp.status_int, 501)
        ...
        # self.assertEqual(
        # sorted(webapp3.get_valid_methods(WebDavHandler)),
        ...
        for method in webdav_methods:
            ...
            rsp = req.get_response(app)
            self.assertEqual(rsp.status_int, 200)
            self.assertEqual(rsp.body, webapp3._to_utf8('Method: %s' % method))
    ...
    def test_escaping(self):
        def get_req(uri):
            req = webapp3.Request.blank(uri)
            app.set_globals(app=app, request=req)
            handler = webapp3.RequestHandler(req, None)
            handler.app = req.app = app
            return req, handler

        req, handler = get_req('http://localhost:80/')
        uri = webapp3.uri_for('escape', name='with space')
        ...
        req, handler = get_req('http://localhost:80/')
        uri = webapp3.uri_for('escape', name='with+plus')
        req, handler = get_req(uri)
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
        ...
        req, handler = get_req('http://localhost:80/')
        uri = webapp3.uri_for('escape', name='with/slash')
        req, handler = get_req(uri)
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
    ...
    def test_function_handler(self):
        ...
        def other_view(request, *args, **kwargs):
            return webapp3.Response('Hello again, function world!')

        def one_more_view(request, *args, **kwargs):
            self.assertEqual(args, ())
            self.assertEqual(kwargs, {'foo': 'bar'})
            return webapp3.Response('Hello you too!')

        app = webapp3.WSGIApplication([
            webapp3.Route('/', my_view),
            webapp3.Route('/other', other_view),
            webapp3.Route('/one-more/<foo>', one_more_view)
        ])
        ...
        req = webapp3.Request.blank('/other')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
        ...
        req = webapp3.Request.blank('/one-more/bar')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
    ...
    def test_custom_method(self):
        ...
        app = webapp3.WSGIApplication([
            webapp3.Route('/', MyHandler, handler_method='my_method'),
            webapp3.Route('/other', MyHandler,
    ...
    def test_custom_method_with_string(self):
        ...
        app = webapp3.WSGIApplication([
            webapp3.Route('/', handler=handler),
            webapp3.Route('/bleh', handler=handler),
        ])
        ...
        req = webapp3.Request.blank('/bleh')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
        self.assertEqual(rsp.body, b'I am a custom method.')

        self.assertRaises(
            ValueError, webapp3.Route, '/',
            handler=handler,
    ...
    def test_factory_2(self):
        ...
        app = webapp3.WSGIApplication([
            webapp3.Route('/', handler=MyHandler),
        ])
    ...
    def test_encoding(self):
        class PostHandler(webapp3.RequestHandler):
            def post(self):
