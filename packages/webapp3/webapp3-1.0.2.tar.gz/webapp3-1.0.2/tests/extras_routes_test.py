...
import webapp3
from webapp3_extras.routes import DomainRoute
from webapp3_extras.routes import HandlerPrefixRoute
from webapp3_extras.routes import NamePrefixRoute
from webapp3_extras.routes import PathPrefixRoute
from webapp3_extras.routes import RedirectRoute
...
class TestRedirectRoute(BaseTestCase):
    ...
    def test_easy_redirect_to(self):
        ...
        req = webapp3.Request.blank('/redirect-me-easily2')
        rsp = req.get_response(app)
        ...
        req = webapp3.Request.blank('/redirect-me-easily3')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 302)
        self.assertEqual(rsp.body, b'')
        self.assertEqual(
    ...
    def test_redirect_to_name(self):
        req = webapp3.Request.blank('/redirect-to-name')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 301)
        self.assertEqual(rsp.body, b'')
        self.assertEqual(
    ...
    def test_strict_slash(self):
        req = webapp3.Request.blank('/strict-foo')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
        self.assertEqual(rsp.body, b'home sweet home')

        req = webapp3.Request.blank('/strict-bar/')
        rsp = req.get_response(app)
        self.assertEqual(rsp.status_int, 200)
        self.assertEqual(rsp.body, b'home sweet home')
...
class TestPrefixRoutes(BaseTestCase):
    def test_simple(self):
        router = webapp3.Router([
            PathPrefixRoute('/a', [
                webapp3.Route('/', 'a', 'name-a'),
                webapp3.Route('/b', 'a/b', 'name-a/b'),
                webapp3.Route('/c', 'a/c', 'name-a/c'),
                PathPrefixRoute('/d', [
                    webapp3.Route('/', 'a/d', 'name-a/d'),
                    webapp3.Route('/b', 'a/d/b', 'name-a/d/b'),
                    webapp3.Route('/c', 'a/d/c', 'name-a/d/c'),
                ]),
            ])
        ])
        ...
        match = ((), {})
        self.assertEqual(
            router.match(webapp3.Request.blank(path))[1:], match)
        self.assertEqual(
            router.build(webapp3.Request.blank('/'),
        ...
        match = ((), {})
        self.assertEqual(
            router.match(webapp3.Request.blank(path))[1:],
            match
        )
        self.assertEqual(
            router.build(webapp3.Request.blank('/'),
        ...
        path = '/a/d/'
        match = ((), {})
        self.assertEqual(router.match(webapp3.Request.blank(path))[1:], match)
        self.assertEqual(
            router.build(webapp3.Request.blank('/'),
            ...
            PathPrefixRoute('/user/<username:\w+>', [
                HandlerPrefixRoute('apps.users.', [
                    NamePrefixRoute('user-', [
                        webapp3.Route('/', 'UserOverviewHandler', 'overview'),
                        webapp3.Route(
    ...
    def test_simple(self):
        router = webapp3.Router([
            DomainRoute('<subdomain>.<:.*>', [
                webapp3.Route('/foo', 'FooHandler', 'subdomain-thingie'),
            ])
        ])

        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank('/foo'))
    ...
    def test_guide_examples(self):
        router = webapp3.Router([
            DomainRoute(r'www.mydomain.com', [
                webapp3.Route('/path1', 'Path1', 'path1'),
            ]),
            DomainRoute(r'<subdomain:(?!www\.)[^.]+>.mydomain.com', [
                webapp3.Route('/path2', 'Path2', 'path2'),
            ]),
            DomainRoute(r'<:(app-id\.appspot\.com|www\.mydomain\.com)>', [
                webapp3.Route('/path3', 'Path3', 'path3'),
            ]),
            DomainRoute(
                r'<subdomain:(?!www)[^.]'
                r'+>.<:(app-id\.appspot\.com|mydomain\.com)>',
                [webapp3.Route('/path4', 'Path4', 'path4'), ]
            ),
        ])
        ...
        self.assertEqual(
            router.match(webapp3.Request.blank(uri1a))[1:],
            ((), {})
        )
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri1b))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri1c))

        self.assertEqual(
            router.match(webapp3.Request.blank(uri2a))[1:],
            ((), {'subdomain': 'sub'})
        )
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri2b))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri2c))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri2d))

        self.assertEqual(
            router.match(webapp3.Request.blank(uri3a))[1:],
            ((), {})
        )
        self.assertEqual(
            router.match(webapp3.Request.blank(uri3b))[1:],
            ((), {})
        )
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri3c))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri3d))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri3e))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri3f))

        self.assertEqual(
            router.match(webapp3.Request.blank(uri4a))[1:],
            ((), {'subdomain': 'sub'})
        )
        self.assertEqual(
            router.match(webapp3.Request.blank(uri4b))[1:],
            ((), {'subdomain': 'sub'})
        )
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri4c))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri4d))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri4e))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri4f))
        self.assertRaises(
            webapp3.exc.HTTPNotFound,
            router.match, webapp3.Request.blank(uri4g))
