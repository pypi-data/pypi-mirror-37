...
webapp3 as ``webapp3.HTTPException``.
...
:meth:`webapp3.RequestHandler.handle_exception`. It is a good idea to define
...
app classes extend it::
    ...
    class BaseHandler(webapp3.RequestHandler):
        def handle_exception(self, exception, debug):
            ...
            if isinstance(exception, webapp3.HTTPException):
...
:attr:`webapp3.WSGIApplication.error_handlers`. This is a dictionary that
...
abort()
-------
The function :func:`webapp3.abort` is a shortcut to raise one of the HTTP
exceptions provided by WebOb: it takes an HTTP status code (403, 404, 500 etc)
and raises the corresponding exception.

Use ``abort`` (or :func:`webapp3.RequestHandler.abort` inside handlers)
