...
class BasePlugin(object):
    ...
    def __repr__(self):
        return '<{}: {} object [{}]>'.format(type(self).__bases__[0].__name__, self.id, getattr(self.status, 'value', 'NA'))
    ...
    def register_configuration(cls):
        ...
        """
        # Register fields
        fastr.config.register_fields(cls.configuration_fields)

        # Validate that current values are correct
        for field_name, field_spec in cls.configuration_fields.items():
            fastr.log.debug('Tested config value: {}'.format(getattr(fastr.config, field_name)))
