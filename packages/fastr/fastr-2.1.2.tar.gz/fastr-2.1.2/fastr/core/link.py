...
class Link(HasDimensions, Updateable, Serializable):
    ...
    def __init__(self, source, target, parent, id_=None, collapse=None, expand=None):
        """
    ...
    def __setstate__(self, state):
        ...
        self.__updating__ = False
        if 'parent' in state and isinstance(state['parent'], (fastr.Network, fastr.NetworkRun)):
            parent = state['parent']
            del state['parent']
        else:
    ...
    def dimensions(self):
        ...
        if self.source is not None:
            ...
            if self.expand:
                # Add a new dimension
                fastr.log.debug('Link expand changes dimensions from {} to {}'.format(dimensions, dimensions + (self.source.cardinality(),)))
    ...
    def source(self, value):
        ...
        if isinstance(value, BaseOutput):
            ...
            if (hasattr(self, '_target') and isinstance(self.target, BaseInput)
                    and typelist.match_types(self.target.datatype,
                                             value.datatype,
    ...
    def target(self, value):
        ...
        if isinstance(value, BaseInput):
            ...
            # Test for matching datatypes
            if (hasattr(self, '_source') and isinstance(self._source, BaseOutput)
                    and typelist.match_types(self._source.datatype,
                                             value.datatype,
