...
class DefaultNetworkChunker(NetworkChunker):
    ...
    def chunck_network(self, network):
        ...
        fastr.log.debug('START Pool: {}'.format(self.used_nodes))
        fastr.log.debug('SOURCELIST: {}'.format(network.sourcelist))
        fastr.log.debug('CONSTANTLIST: {}'.format(network.constantlist))
        candidates = deque()

        for source in network.sourcelist.values():
            if source in self.used_nodes:
                candidates.append(source)

        for constant in network.constantlist.values():
            if constant in self.used_nodes:
                candidates.append(constant)

        fastr.log.debug('START candidates: {}'.format([x.id for x in candidates]))
        ...
        while len(self.used_nodes) > 0:
            ...
            while len(candidates) > 0:
                ...
                fastr.log.debug('Considering NodeRun {}'.format(node.id))

                if node in candidates:
                    fastr.log.error('NODE HAS ENTERED CANDIDATES TWICE! {}'.format(node))

                if node not in self.pool:
                    fastr.log.debug('Used nodes: {}\nPool: {}\nCandidates: {}\nNew pool: {}'
                                    '\nNew candidates: {}'.format([x.id for x in self.used_nodes],
                                                                  [x.id for x in self.pool],
                                                                  [x.id for x in candidates],
                                                                  [x.id for x in new_pool],
                                                                  [x.id for x in new_candidates]))
                    message = 'Encountered previously visited NodeRun {}!'.format(node.id)
                ...
                if not self._node_is_candidate(node):
                    fastr.log.debug('Moving {} to next chunk (not a candidate)'.format(node.id))
                ...
                if self._node_is_blocked(node):
                    fastr.log.debug('Moving {} to next chunk (is blocked)'.format(node.id))
                    new_candidates.append(node)
                    continue

                # Change node status and append to working chunk
                fastr.log.debug('Adding {} to chunk'.format(node.id))
                self.used_nodes.remove(node)
                self.node_status[node.id] = 'visited'
                new_chunk.append(node)

                fastr.log.debug('Processing listeners for {}'.format(node.id))
                # Recurse into following Nodes
                for listener in node.listeners:
                    lnode = listener.target.node
                    fastr.log.debug('Considering listener {}'.format(lnode.id))

                    if lnode not in self.used_nodes:
                        fastr.log.debug('Ignoring {}'.format(lnode.id))
                        continue

                    if not self._node_is_blocked(lnode):
                        if lnode not in candidates:
                            fastr.log.debug('Adding {} to candidates'
                                            ' (blocking {})'.format(lnode.id,
                                                                    self._node_is_blocked(lnode)))
                            candidates.append(lnode)
                        else:
                            fastr.log.debug('Listener {} already in candidates'.format(lnode.id))
                    else:
                        if lnode not in new_candidates:
                            fastr.log.debug('Queueing {} ({})'.format(lnode.id, self._node_is_blocked(lnode)))
                            if lnode in candidates:
                                candidates.remove(lnode)
                            new_candidates.append(lnode)

            # Add the newly created chunk to the list
            chunk_start = tuple(x for x in chunk_start if x in new_chunk)
            fastr.log.debug('Adding chunk {} with start {}'.format([x.id for x in new_chunk],
                                                                   [x.id for x in chunk_start]))
            ...
            fastr.log.debug('Start new chuck with candidates {} and pool {}'.format([x.id for x in candidates],
                                                                                    [x.id for x in self.pool]))
    ...
    def _node_is_candidate(self, node):
        ...
        for node in node.get_sourced_nodes():
            if node not in self.processed_nodes and node not in self.pool:
                return False
    ...
    def _node_is_blocked(self, target):
        ...
        for node in target.get_sourced_nodes():
            if node in self.pool and (node.blocking or self._node_is_blocked(node)):
                fastr.log.debug('NodeRun {} is the cause of the blocked node {} (pool: {})'.format(node.id,
                                                                                                target.id,
                                                                                                [x.id for x in self.pool]))
                return True
    ...
    def _network_walker(self, node):
        ...
        for input_ in node.inputs.itervalues():
            ...
            for parentnode in input_.get_sourced_nodes():
                ...
                self.used_nodes.add(parentnode)
