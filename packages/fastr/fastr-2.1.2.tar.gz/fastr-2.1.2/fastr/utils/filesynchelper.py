...
def filesynchelper_enabled():
    return len(fastr.config.filesynchelper_url) > 0
...
class FileSyncHelper():
    ...
    _redis = Redis.from_url(fastr.config.filesynchelper_url) if filesynchelper_enabled() else None
    ...
    def wait_for_pickle(self, url, timeout=300):
        fastr.log.debug('waiting for pickle {}'.format(url))
    ...
    def store(self, url, data):
        ...
        fastr.log.debug('storing {} -> {}'.format(url, key))
        self._redis.delete(key)
    ...
    def load(self, url):
        key = self._generate_key_for_string(str(url))
        fastr.log.debug('loading {} from {}'.format(url, key))
    ...
    def make_file_promise(self, url):
        ...
        fastr.log.debug('making file promise {} -> {}'.format(url, key))
    ...
    def has_file_promise(self, url):
        ...
        fastr.log.debug('has_file_promise {} {} {}'.format(url, result, self._generate_key_for_string(str(url))))
        return result

    def wait_for_vfs_url(self, vfs_url, timeout=300):
        fastr.log.debug('wait_for_vfs_url {}'.format(vfs_url))
    ...
    def wait_for_file(self, path, timeout=300):
        fastr.log.debug('wait_for_file {}'.format(path))
    ...
    def _wait_for_file_and_suburls(self, path, suburls, timeout):
        fastr.log.debug('waiting for {}'.format(path))
        # wait for file or timeout
        timeoutafter = time.time() + timeout
