...
class FastrRefResolver(jsonschema.RefResolver):
    ...
    def __init__(self, base_uri, referrer, store=(), cache_remote=True, handlers=()):
        ...
        base_uri = urlunparse(['file', '', urllib.pathname2url(base_uri), '', '', ''])
...
_TYPECASTS = {
    (u"array", collections.OrderedDict): _ordereddict_to_array,
    (u"array", type(None)): _none_to_array,
    (u"array", str): _str_to_list,
    (u"boolean", str): _str_to_boolean,
    (u"boolean", unicode): _str_to_boolean,
}
...
def _match_type(validator, instance, schema):
    ...
    if not any(validator.is_type(instance, dtype) for dtype in desired_type):
        for dtype in desired_type:
            try:
                if (dtype, type(instance)) in _TYPECASTS:
                    return _TYPECASTS[dtype, type(instance)](instance)
                return _DEFAULT_TYPECASTS[dtype](instance)
            except (ValueError, TypeError):
                pass  # Not coercable, just hope another will success
        #raise TypeError('Cannot match the data and schema type ({} and {})!'.format(type(instance).__name__, desired_type))
    else:
...
def not_draft4(validator, not_schema, instance, schema):
    ...
    # All arguments must be used because this function is called like this
    # pylint: disable=unused-argument
    temp_instance = copy.deepcopy(instance)  # Make sure not to change instance
...
"""
    ...
    Validator = jsonschema.validators.extend(
        validator_cls, {
            u"anyOf": any_of_draft4,
            u"oneOf": one_of_draft4,
            u"not": not_draft4
            }
    )
    ...
    if blueprint is None:
        with open(uri, 'r') as fin:
            try:
                ...
                raise ValueError('{} ({})'.format(exception.message, uri))
