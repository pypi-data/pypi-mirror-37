...
class DRMAAExecution(ExecutionPlugin):
    ...
    def configuration_fields(cls):
        return {
            ...
            "drmaa_default_walltime": (str, '24:00:00', 'Walltime used for jobs that don\'t specify walltime HH:MM:SS, defaults to: 24:00:00 (24 hours) if not set'),
            "drmaa_default_memory": (str, '1G', 'Memory used for jobs that don\'t specify memory, defaults to: 1G (1 Gibibyte) if not set'),
            "drmaa_default_cores": (int, 1, 'Cores used for jobs that don\'t specify cores, defaults to: 1 (1 core) if not set')
        }
        ...
        while self.running:
            try:
                ...
                cl_job_id = self.send_job(command[0], command[1:],
                                          ...
                                          ncores=required_cores,
                                          walltime=required_time,
