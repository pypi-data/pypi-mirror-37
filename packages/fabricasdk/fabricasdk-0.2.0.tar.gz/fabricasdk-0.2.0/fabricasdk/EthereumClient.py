...
class EthereumClient:
    def __init__(self, state, network_url=None, private_key=None, cosigner_address=None):
        ...
        if self.private_key is None:
            self.user_address = self.w3.eth.accounts[0]
        else:
            self.user_address = self.get_address_from_private_key(self.private_key)
    ...
    def get_nonce(self):
        key = "{}_{}_nonce".format(self.network_id, self.user_address)
        local_nonce = self.state.get_state(key)
        blockchain_nonce = self.w3.eth.getTransactionCount(self.user_address)
