...
def chksize():
	global checkbox_show_sizes
	if checkbox_show_sizes:
		checkbox_show_sizes = False
	else:
		checkbox_show_sizes = True
	refresh_apps()	
	...
	for item in sorted(applist, reverse=True):
		...
		for group in groups:
			...
			if item in groups[group]:
				size = ""
				if checkbox_show_sizes:
					size = humanfriendly.format_size(os.stat(pth+item).st_size)+" | "
				
				cats[group].insert(0, size+item)
				grouped = True
		...
		if not grouped:
			...
			appgui.apps.insert(0, size+item)	
...
def run_app():
	...
	if tkinter.messagebox.askokcancel(title=f"Run {name}", message=f"Launch {name}?"):
		...
		try:
			...
			tkinter.messagebox.showinfo(message=f"Setting executable permissions for {name}...")
...
def delete_app():
	...
	if page:
		...
		return
	size = humanfriendly.format_size(os.stat(pth+name).st_size)
	if tkinter.messagebox.askokcancel(title=f"Delete {name}", message=f"Permenantly delete {name} ({size}) from system?"):
		res = subprocess.run(["rm", pth+name], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def install_apps():
	...
	if len(files) > 0:
		...
		i = 0
		for item in files:
			fp = partial(toggleinstall, item)
			size = humanfriendly.format_size(os.stat(pth+item).st_size)
			ck = tkinter.Checkbutton(w, text=f"{item} ({size})", command=fp)
			ck.grid(row=i, sticky="w")
			i += 1
		
		runinstp = partial(run_inst_f, w)
		runinstbut = tkinter.Button(w, text="Install", command=runinstp).grid(row=i)
	else:
		tkinter.messagebox.showinfo(message="No apps in download folder.")
	refresh_apps()
...
def toggleinstall(app):
	if app in installque:
		installque.remove(app)
	else:
		installque.append(app)
...
command=group_man_main, cursor="hand1", width=5)
		...
		self.b_grp.grid(column=2, row=5, pady=0, padx=0, ipady=0, ipadx=0)
		self.b_gman.grid(column=3, row=5)
		self.chk = tkinter.Checkbutton(self.mainwin, text="Show grouped apps", command=chkgrp)
		self.chk2 = tkinter.Checkbutton(self.mainwin, text="Show sizes", command=chksize)
		self.chk.grid(row=6, column=2, columnspan=2, sticky="w")
		self.chk2.grid(row=7, column=2, columnspan=2, sticky="w")
		self.chk2.select()
		...
		try:
			...
			#subprocess.run(['wget', '-p', scriptdir, 'https://github.com/Kaiz0r/AppImages-Manager/blob/master/icon.png'])
			pass
