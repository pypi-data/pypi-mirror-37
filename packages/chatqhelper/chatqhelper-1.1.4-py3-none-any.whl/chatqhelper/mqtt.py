...
from chatqhelper.notifiers import notify_on_err, ErrorStreamHandler
    ...
    def publish_exception(self, exception, message):
        ...
        try:
            ...
            error = {
                ...
                'source_payload': message.payload,
                'trace_back': debug.log_traceback(logger),
...
class MqttErrNotifier():
    def __init__(self):
        self._mqtt_client = None
        client = MqttClient.create(self.on_connect, post_fix='err-notifier')
        client.loop_start()
    ...
    def publish_err(self, err):
        client = self._mqtt_client
        if client:
            client.publish(
                'exceptions/chat',
                json.dumps({'exceptions':str(err)})
            )


class MqttErrorStreamHandler(ErrorStreamHandler):
    _mqtt_notifier = MqttErrNotifier()
    @classmethod
    def handle(cls, error):
        notifier = cls._mqtt_notifier
        notifier.publish_err(error)


mqtt_err_publisher = notify_on_err(stream=MqttErrorStreamHandler)