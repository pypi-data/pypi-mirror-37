...
class PosettaGui(tk.Tk):
    ...
    def translate(self):
        ...
        # Delay call to translator by 1 ms to allow GUI to update
        self.after(1, self._translate, file_from, file_to, fmt_from, fmt_to, options)

    def _translate(self, file_from, file_to, fmt_from, fmt_to, options):
        """Translate in a separate function in order to update GUI first"""
        try:
...
class Status(ttk.Frame):
    def __init__(self, master):
        ...
        # Lay out widgets
        self._text = scrolledtext.ScrolledText(self, height=12, wrap=tk.WORD)
    ...
    def write(self, text):
        self._text.config(state=tk.NORMAL)
        self._text.insert(tk.END, f"\n{text}")  # \n in front avoids blank line at end
