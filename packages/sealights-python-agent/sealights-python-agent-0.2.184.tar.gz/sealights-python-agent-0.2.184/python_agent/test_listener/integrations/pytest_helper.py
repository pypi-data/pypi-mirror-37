...
class SealightsPlugin(object):
    ...
    def pytest_runtest_logstart(self, nodeid, location):
        try:
            # when working with xdist pytest_runtest_logstart occurs twice for the same test
            # one for a node with an execution id initialized, which we want
            # one for a node without an execution id initialized, which we don't want
            if not self.execution_id:
                log.debug("execution_id was not initialized. Not notifying test start for %s" % get_test_name(nodeid))
                return
    ...
    def pytest_runtest_logreport(self, report):
        try:
            # when working with xdist pytest_runtest_logreport occurs twice for the same test
            # one for a node with an execution id initialized, which we want
            # one for a node without an execution id initialized, which we don't want
            if not self.execution_id:
                log.debug("execution_id was not initialized. Not notifying test end for %s" % get_test_name(report.nodeid))
                return
