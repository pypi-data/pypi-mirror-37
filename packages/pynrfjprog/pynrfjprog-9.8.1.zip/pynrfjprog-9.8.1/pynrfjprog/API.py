...
class API(object):
    ...
    def __init__(self, device_family, jlink_arm_dll_path=None, log_str_cb=None, log=False, log_str=None, log_file_path=None):
        ...
        if DEBUG_OUTPUT:
            log = True
        self._log_str_cb = self._generate_log_str_cb(log, log_str_cb, log_str, log_file_path)
    ...
    """
    def _generate_log_str_cb(self, log, log_str_cb, log_str, log_file):
        """
        Setup API's debug output logging mechanism.

        """
        if not log and log_str_cb is None and log_str is None and log_file is None:
            return None 
        
        if log_str_cb is not None:
            return ctypes.CFUNCTYPE(None, ctypes.c_char_p)(lambda x: log_str_cb(self._decode_string(x).strip()))
        ...
        if log is True or log_str is not None or log_file is not None:
            ...
            return ctypes.CFUNCTYPE(None, ctypes.c_char_p)(lambda x: print('{} {} {}'.format(datetime.datetime.now().replace(microsecond=0).isoformat(' '), log_str, self._decode_string(x).strip()), file=self._log_file))
