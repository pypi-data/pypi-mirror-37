...
         Isso também atualiza o prazo de solicitação."""
    ...
    # __create_pack
    # --------------------------------
    def __create_pack(self, message):
        # Verifica se a mensagem é uma lista
        # Verifica se há 3 elementos (multipart)
        if type(message) == list and len(message) == 3:
            # Obtém a string JSON do pacote
            self.content = message[2:]
            # Verifica se o conteúdo da mensagem é uma lista
