...
class kgCal_PreRSL{
	...
	kgVal** _prvResults;
	boost::posix_time::ptime**  _prvtimeRsls;
	boost::gregorian::date**  _prvdateRsls;
	char ** _prvcharRsls;
	...
	public:
		...
		~kgCal_PreRSL(void){
			if(_fldSize>0){
				for(size_t i=0;i<_fldSize;i++){
					...
					delete _prvResults[i];
					if( _prvtimeRsls[i]!=NULL ){ delete _prvtimeRsls[i]; }
					if( _prvdateRsls[i]!=NULL ){ delete _prvdateRsls[i]; }
				}
				delete [] _prvcharRsls;
			}
		}
		void resize(size_t i){
			_fldSize=i;
			_prvResults  = new kgVal* [_fldSize];
			_prvtimeRsls = new boost::posix_time::ptime* [_fldSize];
			_prvdateRsls = new boost::gregorian::date* [_fldSize];
			_prvcharRsls = new char* [_fldSize];
			for(size_t j=0;j<_fldSize;j++){
				_prvResults[j]  = new kgVal;
			...
			if(!_prvResults[pos]->null()){
				try {
					...
					case 'S':
						...
						break;
					case 'D':
						if(_prvdateRsls[pos]!=NULL){ delete _prvdateRsls[pos]; }
						_prvdateRsls[pos] = new boost::gregorian::date( *(rls->d()) );
						_prvResults[pos]->d(_prvdateRsls[pos]);
						break;
					case 'T':
						if(_prvtimeRsls[pos]!=NULL){ delete _prvtimeRsls[pos]; }
   						_prvtimeRsls[pos]= new boost::posix_time::ptime( *(rls->t()) ) ;
						_prvResults[pos]->t(_prvtimeRsls[pos]);
						break;
					}
				}catch(...){
