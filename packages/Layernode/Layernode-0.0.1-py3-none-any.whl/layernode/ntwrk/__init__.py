...
from layernode.ntwrk.message import Message
from layernode.ntwrk.response import Response
...
def command(peer, message, node_id):
    ...
    if sock is not None:
        ...
        result = send(
            Message(headers={'id': message_id, 'node_id': node_id}, body=message), sock)
        if result:
