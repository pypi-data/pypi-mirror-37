...
def get_pings(address, return_time_length=20, average_return_time_length=200):
    ...
    *average_return_time*.
    ...
    :return_time: A python list object to store ping return time
    :average_return_time: A python list object to store the average return time
    :returns: The *return_time* list, the *average_return_time* list
    """
    return_time, average_return_time = [], []
    while 1:
        if name != 'nt':
        ...
        for line in stdout.splitlines():
            if len(return_time) > return_time_length:
                return_time.pop(0)

            line_value = _decode_line(line)

            if line_value is not None:
                return_time.append(line_value)

        else:  # On end of for loop, calculate average and append it
            average_return_time.append(sum(return_time) / len(return_time if return_time != 0 else 1))
            if len(average_return_time) > average_return_time_length:
                average_return_time.pop(0)

        yield return_time, average_return_time
