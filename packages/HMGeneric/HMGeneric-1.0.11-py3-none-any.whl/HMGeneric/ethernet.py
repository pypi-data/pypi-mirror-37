...
def get_my_interface():
    if(os.name == "nt"): #windows
        ...
        interface = get_terminal_lines(['arp', '-a'])[0].split('on ')[1].split('\n')[0]
        ip = get_phrase_from_list(get_terminal_lines(['ifconfig', interface]), 'inet')[0].split('inet ')[1].split(' ')[0]
        mac = get_phrase_from_list(get_terminal_lines(['ifconfig', interface]), 'ether')[0].split('ether ')[1].split(' ')[0]
