...
class SECHelper:
    ...
    def trained_model_id(self) -> str:
        return self.sec["trainedModelId"]
    ...
    def trained_model_download_link(self) -> URLObject:
        ...
        return URLObject(response)

    def pull_trained_model_data(self) -> bytes:
        logger.info(f'pull trained model data: {self.trained_model_id}')
        response = requests.get(self.trained_model_download_link)
        if response.status_code not in (200, 201):
            logger.error(f'{response.status_code, response.content}')
            raise Exception(f'Cannot pull trained model data {self.trained_model_id}')

        return response.content

    def upload_trained_model(self, file_path: str, name: str) -> None:
        logger.info(f'uploading trained model {name}...')
        with open(file_path, 'rb') as f:
            response = self.data_endpoint.request(self.data_endpoint.POST,
