...
from dpaycliapi.exceptions import ApiNotSupported, MissingRequiredActiveAuthority
...
from dpaycli.constants import DPAY_VOTE_REGENERATION_SECONDS, DPAY_1_PERCENT, DPAY_100_PERCENT, DPAY_VOTING_MANA_REGENERATION_SECONDS
...
class Account(BlockchainObject):
    """ This class allows to easily access Account data
        ...
        :rtype: dictionary
        :raises dpaycli.exceptions.AccountDoesNotExistsException: if account
                does not exist
    ...
    def _parse_json_data(self, account):
        parse_int = [
            "bbd_seconds", "savings_bbd_seconds", "average_bandwidth", "lifetime_bandwidth", "lifetime_market_bandwidth", "reputation", "withdrawn", "to_withdraw",
        ]
        for p in parse_int:
    ...
    def get_rc_manabar(self):
        ...
        current_mana = int(last_mana + diff_in_seconds * max_mana / DPAY_VOTING_MANA_REGENERATION_SECONDS)
        ...
        if max_mana > 0:
            ...
            current_pct = 0
        max_rc_creation_adjustment = Amount(rc_param["max_rc_creation_adjustment"], dpay_instance=self.dpay)
    ...
    @property
    def bp(self):
        """ Returns the accounts DPay Power
        """
    ...
    def print_info(self, force_refresh=False, return_str=False, use_table=False, **kwargs):
        ...
        try:
            ...
            rc = self.get_rc()
            rc_calc = RC(dpay_instance=self.dpay)
        except:
            rc_mana = None
            rc_calc = None
        ...
        if use_table:
            ...
            t.add_row(["DPay Power", "%.2f %s" % (self.get_dpay_power(), self.dpay.dpay_symbol)])
    ...
    def get_reputation(self):
        """ Returns the account reputation in the (dsocial) normalized form
        """
        if not self.dpay.is_connected():
            return None
    ...
    def get_manabar(self):
        ...
        if max_mana == 0:
            props = self.dpay.get_chain_properties()
            required_fee_dpay = Amount(props["account_creation_fee"], dpay_instance=self.dpay)
            max_mana = int(self.dpay.bp_to_vests(required_fee_dpay))
    ...
    def get_dpay_power(self, onlyOwnSP=False):
        """ Returns the account dpay power
        """
        return self.dpay.vests_to_sp(self.get_vests(only_own_vests=onlyOwnSP))
    ...
    def get_recharge_timedelta(self, voting_power_goal=100, starting_voting_power=None):
        ...
        if missing_vp < 0:
            return 0
        recharge_seconds = missing_vp * 100 * DPAY_VOTING_MANA_REGENERATION_SECONDS / DPAY_100_PERCENT
    ...
    def get_manabar_recharge_timedelta(self, manabar, recharge_pct_goal=100):
        ...
        if missing_rc_pct < 0:
            return 0
        recharge_seconds = missing_rc_pct * 100 * DPAY_VOTING_MANA_REGENERATION_SECONDS / DPAY_100_PERCENT
        return timedelta(seconds=recharge_seconds)
    ...
    def get_blog_entries(self, start_entry_id=0, limit=100, raw_data=True,
        ...
        """ Returns the list of blog entries for an account
            ...
            .. code-block:: python
                ...
                dsocial - firstpost - dsocial - 1970-01-01T00:00:00
    ...
    def get_balance(self, balances, symbol):
        """ Obtain the balance of a specific Asset. This call returns instances of
            :class:`dpaycli.amount.Amount`. Available balance types:
    ...
    def get_curation_reward(self, days=7):
        ...
        reward_vests = Amount(0, self.dpay.vests_symbol, dpay_instance=self.dpay)
        ...
        """ Update an account's profile in json_metadata
            ...
            .. code-block:: js

                {
                    ...
                    'website': 'https://github.com/holgern/dpaycli'
                }
    ...
    def transfer_to_vesting(self, amount, to=None, account=None, **kwargs):
        ...
        if to is None:
            ...
            to = Account(to, dpay_instance=self.dpay)
        amount = self._check_amount(amount, self.dpay.dpay_symbol)
    ...
    def convert(self, amount, account=None, request_id=None):
        """ Convert DPayDollars to DPay (takes 3.5 days to settle)
        ...
        if account is None:
            ...
            account = Account(account, dpay_instance=self.dpay)
        amount = self._check_amount(amount, self.dpay.bbd_symbol)
        if request_id:
    ...
    def claim_reward_balance(self,
        ...
        reward_dpay = self._check_amount(reward_dpay, self.dpay.dpay_symbol)
        reward_bbd = self._check_amount(reward_bbd, self.dpay.bbd_symbol)
        reward_vests = self._check_amount(reward_vests, self.dpay.vests_symbol)
    ...
    def delegate_vesting_shares(self, to_account, vesting_shares,
        ...
        vesting_shares = self._check_amount(vesting_shares, self.dpay.vests_symbol)
    ...
    """
        if account is None:
            ...
            account = Account(account, dpay_instance=self.dpay)
        amount = self._check_amount(amount, self.dpay.vests_symbol)
...
class AccountsObject(list):
    ...
    def print_summarize_table(self, tag_type="Follower", return_str=False, **kwargs):
        ...
        if (len(eff_sp) > 0):
            t.add_row(["Summed eff. BP", "%.2f" % sum(eff_sp)])
            t.add_row(["Mean eff. BP", "%.2f" % (sum(eff_sp) / len(eff_sp))])
            t.add_row(["Max eff. BP", "%.2f" % max(eff_sp)])
...
class Accounts(AccountsObject):
    """ Obtain a list of accounts
        ...
        :param dpay dpay_instance: DPay() instance to use when
            accessing a RPCcreator = Account(creator, dpay_instance=self)
    """
