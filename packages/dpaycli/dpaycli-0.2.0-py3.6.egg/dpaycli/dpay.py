...
class DPay(object):
    ...
    def __init__(self,
        ...
        # set dpayid
        if self.dpayconnect is not None and not isinstance(self.dpayconnect, DPayID):
            raise ValueError("dpayid musst be DPayID object")
        if self.dpayconnect is None and self.use_dpid:
            self.dpayconnect = DPayID(dpay_instance=self, **kwargs)
        elif self.dpayconnect is not None and not self.use_dpid:
            self.use_dpid = True
    ...
    def claim_account(self, creator, fee=None, **kwargs):
        ...
        if not creator:
            raise ValueError(
                ...
                "creator=x, or set the default_account using dpay")
        creator = Account(creator, dpay_instance=self)
        op = {
            "fee": Amount(fee, dpay_instance=self),
    ...
    def bbd_symbol(self):
        ...
        # some networks (e.g. whaleshares) do not have BBD
        try:
