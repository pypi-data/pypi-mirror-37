...
class KubAppInstanceIdentifierCreator(IAppInstanceIdentifierCreator):
    ...
    def get_pod_json_by_ip_with_retries(self, app_name: str, ip_address: str):
        ...
        while True:
            try:
                pod_json = self._kub_api_service.try_get_pod_json_by_ip(ip_address=ip_address)
                if pod_json:
                    return pod_json
                else:
                    if remains == 0:
                        raise Exception("pod for app '{}' with address '{}' not found".format(app_name, ip_address))
                    else:
                        self._logger.info("pod for app '{}' with address '{}' not found ({} remaining attempts)"
                                          .format(app_name, ip_address, remains))
            except Exception as e:
                if remains == 0:
                    raise e
                else:
                    self._logger.exception("error while getting pod json by ip ({REMAINS} remaining attempts)"
                                           .format(REMAINS=remains))
