...
from djangokarat.utils import (filter_relational_fields, switch_values_to_keys, append_field_name_to_keys,
...
class Worker(Thread):
    ...
    def get_main_id_model_relation(self, model, parts, value, object_exists=True, unique_fields_dict={}):
        ...
        if parts:
            ...
            instance_value = self.get_main_id_model_relation(self, relation_model, parts, value)
            model_filter = model.objects.filter(**{part: instance_value.pk}, **unique_fields_dict)
            if object_exists and model_filter.exists():
                relation_model_instance = model_filter.get()
            else:
                relation_model_instance = model.objects.create(**{part: instance_value})
    ...
    def prepare_relational_field_to_karat(self, instance, field):
    ...
    def prepare_data_to_karat(self, instance):
        ...
        for name, value in karats.items():
            ...
            karats[name] = strip_exclamation_mark(karats[name])
        ...
        for relation_field in relational_fields:
            ...
            if new_model._meta.karat_table == instance._meta.karat_table:
                karat_data = {**self.prepare_data_to_karat(self, new_model), **karat_data}
            else:
                karat_data[field_name] = self.prepare_relational_field_to_karat(self, instance, relation_field)
        return karat_data
    ...
    def prepare_data_to_send(self, used_instances):
        ...
        for instance in used_instances:
            data_to_send = {**self.prepare_data_to_karat(self, instance), **data_to_send}
        return data_to_send
    ...
    def update_or_create_instance(self, tables, data):
        ...
        for table in tables:
            ...
            if isinstance(karat_fields, dict):
                ...
                model_fields = switch_values_to_keys(karat_fields)
                # strip field_names of exclamation mark and brackets
            ...
            # prepare data to retrieve or create instance by incoming data. Check dict
            # has object pks instead of object reference
            try:
        ...
        data_to_sent = self.prepare_data_to_send(self, used_instances)
        print(data_to_sent)
        print()
