...
class Worker(Thread):
    ...
    def skip_empty_model(self, data, karat_fields):
        ...
        if isinstance(karat_fields, dict):
            ...
            for id_column in id_columns:
                ...
                if '!' in model_fields[id_column] or not str(data.get(id_column, '')).strip():
                    should_skip = True
        return should_skip

    def get_lookup_relation(self, model, parts, value, unique_fields_dict):
        part = parts.pop(0)
        part_model = get_relation_model(model, part)
        ...
        if relation_model_field_type == 'OneToOneField':
            field_related_name = get_self_related_name(model, part)
    ...
    def get_main_id_model_relation(self, model, parts, value, object_exists=True, unique_fields_dict={}):
        ...
        part = parts.pop(0)
        if parts:
            relation_model = get_relation_model(model, part)
    ...
    def build_instance_from_map(self, model, instance_map):
        ...
        '''
        models_instance_map = self.build_models_in_instance_map(self, model, instance_map)
        instance = self.get_or_create_instance_from_dict(self, model, models_instance_map)
        return instance

    def update_instance_data_from_map(self, instance, instance_map):
        models_instance_map = self.build_models_in_instance_map(self, instance._meta.model, instance_map)
        updated_instance = self.update_fields_in_instance(self, instance, models_instance_map)
        return updated_instance

    def update_fields_in_instance(self, instance, data_dict):
        ...
        while has_dict:
            ...
            for instance_model_name, data in instance_map.items():
                if isinstance(data, dict):
                    ...
                    # recurse to get instance model id
                    instance_id = self.build_models_in_instance_map(self, instance_model, data, level + 1)
    ...
    def update_or_create_instance(self, tables, data):
        ...
        for table in tables:
            ...
            instance = self.build_instance_from_map(self, model, instance_relation_map)
            updated_instance = self.update_instance_data_from_map(self, instance, instance_data_map)
