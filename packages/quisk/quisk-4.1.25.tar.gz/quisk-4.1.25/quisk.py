...
The new code supports multiple corrections per band.""")
  ...
  def MakeButtons(self, frame, gbs):
    ...
    item = self.split_menu.AppendRadioItem(-1, 'Play both, High Freq on R')
    self.Bind(wx.EVT_MENU, self.OnMenuSplitPlay1, item)
    item = self.split_menu.AppendRadioItem(-1, 'Play both, High Freq on L')
    self.Bind(wx.EVT_MENU, self.OnMenuSplitPlay2, item)
    item = self.split_menu.AppendRadioItem(-1, 'Play only Rx')
    self.Bind(wx.EVT_MENU, self.OnMenuSplitPlay3, item)
    item = self.split_menu.AppendRadioItem(-1, 'Play only Tx')
    self.Bind(wx.EVT_MENU, self.OnMenuSplitPlay4, item)
