...
{	// Add samples to the Tx buffer.
	...
	if ( ! key_down) {
		if (hermes_num_samples != HERMES_TX_BUF_SAMPLES / 2)
...
{	// Send one UDP block of mic samples using the Metis-Hermes protocol.  Timing is from blocks received, rate is 48k.
	...
	*tx_records -= 63 * 2 * ratio;
	...
	if (key_down)	// We do not remove samples unless the key is down
	...
	if (C0_index == 0) {	// Do not change receiver count without stopping Hermes and restarting
		...
		if (key_down)		// send filter selection on J16
	...
	if ( ! key_down)	// If the key is not down, continue to send zero samples.
		hermes_read_index = 0;	// We do not remove samples unless the key is down.
