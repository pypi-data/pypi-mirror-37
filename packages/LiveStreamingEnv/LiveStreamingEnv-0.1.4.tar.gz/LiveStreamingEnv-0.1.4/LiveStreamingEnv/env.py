...
class Environment:
    ...
    def get_video_frame(self, quality, target_buffer):
        ...
        if self.time < self.cdn_arrive_time[quality][self.video_chunk_counter] and not end_of_video: # CDN can't get the frame
            ...
            self.newest_frame_time = self.cdn_arrive_time[quality][self.video_chunk_counter]
            ...
            # calculate the play time , the real time ,the latency
                ...
                self.latency = (self.newest_frame_time - self.cdn_arrive_time[quality][self.video_chunk_counter]) * 0.04  + self.buffer_size
            ...
            i = self.video_chunk_counter
            current_new = self.cdn_arrive_time[quality][i]
            while(current_new > self.time):
                i += 1
                current_new = self.cdn_arrive_time[quality][i]
            self.newest_frame_time = self.cdn_arrive_time[quality][i]
        ...
        if self.buffer_status and not end_of_video:
            ...
            if self.latency > latency_threshold and not self.skip_flag :
                ...
                if ADD_FRAME > 0 :
                    self.video_chunk_counter = self.play_time_counter + skip_add_frame 
                else:
                    ADD_FRAME = 0
                    #print("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww1", self.skip_time, self.play_time_counter, self.video_chunk_counter, ADD_FRAME)
            else:
                self.video_chunk_counter += 1
        ...
        # If not buffering
            ...
            if self.latency > latency_threshold and not self.skip_flag:
                ...
                if ADD_FRAME > 0: 
                    ...
                    ADD_FRAME = 0
                #print("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww2", self.skip_time, self.play_time_counter, self.video_chunk_counter, ADD_FRAME)
