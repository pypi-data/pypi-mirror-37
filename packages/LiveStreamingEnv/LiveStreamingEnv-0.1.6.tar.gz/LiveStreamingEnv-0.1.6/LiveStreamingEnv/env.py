    ...
    def get_video_frame(self, quality, target_buffer):
        ...
        if self.time < self.cdn_arrive_time[quality][self.video_chunk_counter] and not end_of_video: # CDN can't get the frame
            ...
            # calculate the play time , the real time ,the latency
                ...
                if self.Debug:
                    self.log_file.write("real time %.4f\t"%self.time +  
                                      "cdn rebuf%.4f\t"%cdn_rebuf_time + 
                                      "client rebuf %.3f\t"%rebuf  + 
                                      "download duration %.4f\t"%duration + 
                                      "frame size %.4f\t"%video_frame_size + 
                                      "play time len %.4f\t"% (cdn_rebuf_time * play_duration_weight) +
                                      "download id %d\t"%self.video_chunk_counter + 
                                      "cdn newest frame %d\t"%self.newest_frame + 
                                      "client buffer %.4f\t"%self.buffer_size  +
                                      "play_time %.4f\t"%self.play_time + 
        ...
        # If not buffering
            ...
            if self.Debug:
                 self.log_file.write("real time %.4f\t"%self.time +  
                                      ...
                                      "download duration %.4f\t"%duration + 
                                      "frame size %.4f\t"%video_frame_size + 
                                      "play time len %.4f\t"% (duration * play_duration_weight) +
                                      "download id %d\t"%self.video_chunk_counter + 
                                      "cdn newest frame %d\t"%self.newest_frame + 
                                      "client buffer %.4f\t"%self.buffer_size  +
                                      "play_time %.4f\t"%self.play_time + 
