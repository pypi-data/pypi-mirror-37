...
@routes.route('/data_source_health.json')
def data_source_health_api():
...
def stmo_status_api():
    ...
    for data_source_id in health_data:
        data_source_name = health_data[data_source_id]["metadata"]["name"]
        dashboard_label = "[Data Source Health] {name}".format(name=data_source_name)
        status[dashboard_label] = health_data[data_source_id]["status"]
...
def datasource_health(app=None):
    ...
    # Add a new view with full health data
    app.view_functions['%s.data_source_health_api' % routes.name] = data_source_health_api
