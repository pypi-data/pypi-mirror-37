...
def log_hyperparams(data):
    """Record hyperparameters for the current run"""
    run_id = get_run_id()
...
def log_metrics(data):
    """Record metrics for the current run"""
    run_id = get_run_id()
...
def get_session_url():
    """Get the tracking URL for the current session"""
    global current_proj
    global current_run_id
    if current_proj is not None and current_run_id is not None:
        return f"{WEB_APP_URL}/projects/{current_proj}/experiments/{current_run_id}"
    else:
        return ""


def get_project_url():
    """Get the tracking URL for the current project"""
    global current_proj
    if current_proj is not None:
        return f"{WEB_APP_URL}/projects/{current_proj}"
    else:
        return ""
...
def commit(message):
    ...
    print(f'[swiftace] Commit {res['experimentId']} successful.')
