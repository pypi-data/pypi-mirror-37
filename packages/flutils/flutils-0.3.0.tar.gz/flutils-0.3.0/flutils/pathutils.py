...
def normalize_path(path: _PATH) -> Path:
    ...
    #. Redundant separators and up-level references will be normalized, so
       that ``A//B``, ``A/B/``, ``A/./B`` and ``A/foo/../B`` all become ``A/B``.

    Args:
        path (str or bytes or pathlib.PosixPath or pathlib.WindowsPath): The
            path to be normalized.

    Returns:
        pathlib.PurePath: :obj:`PosixPath <pathlib.PosixPath>` or
        :obj:`WindowsPath <pathlib.WindowsPath>` depending on the system.

        |

        .. Note:: :obj:`PurePath <pathlib.PurePath>` objects are
           immutable. Therefore, any :obj:`PosixPath <pathlib.PosixPath>` or
           :obj:`WindowsPath <pathlib.WindowsPath>` objects given to this
           function will not be the same objects returned.
...
def find_paths(
    ...
    Args:
        pattern (str or bytes or pathlib.PosixPath or pathlib.WindowsPath): The
            path to find; which may contain a :term:`glob pattern`.

    Returns:
        generator: A generator that yields :obj:`pathlib.PosixPath` or
        :obj:`pathlib.WindowsPath`.
...
def exists_as(path: _PATH) -> str:
    ...
    Args:
        path (str or bytes or pathlib.PosixPath or pathlib.WindowsPath): The
            path to check existance.

    Returns:
        str: one of the following values:

        :``''``: if the given ``path`` does NOT exist; or, is a broken
            symbolic link; or, other errors (such as permission errors) are
            propagated.
...
def get_os_user(name: _STR_OR_INT_OR_NONE = None) -> pwd.struct_passwd:
    ...
    Args:
        name (str or int, optional): The "login name" or ``uid``.  Defaults
    ...
    Returns:
        :obj:`struct_passwd <pwd>`: A tuple like object.
...
def get_os_group(name: _STR_OR_INT_OR_NONE = None) -> grp.struct_group:
    ...
    Args:
        name (str or int, optional): The "group name" or ``gid``.  Defaults
            to the current users's group.
    ...
    Returns:
        :obj:`struct_group <grp>`: A tuple like object.
...
def chown(
    ...
    Args:
        path (str or bytes or pathlib.PosixPath or pathlib.WindowsPath): The
            path of the file or directory that will have it's ownership
            changed.  This value can be a :term:`glob pattern`.
        user (str or int, optional): The "login name" used to set the
            owner of ``path``.  A value of ``'-1'`` will leave the
            owner unchanged.  Defaults to the "login name" of the current user.
        group (str or int, optional): The group name used to set
            the group of ``path``.  A value of ``'-1'`` will leave the
            group unchanged.  Defaults to the current user's group.
        include_parent (bool, optional): A value of ``True`` will
            chown the parent directory of the given ``path`` that contains
            a :term:`glob pattern`.  Defaults to ``False``.
    ...
    Examples:
        ...
        Supports a :term:`glob pattern`.  So to recursively change the ownership
...
def chmod(
    ...
    Args:
        path (str or bytes or pathlib.PosixPath or pathlib.WindowsPath): The
            path of the file or directory to have it's mode changed.  This
            value can be a :term:`glob pattern`.
        mode_file (int, optional): The mode applied to the given ``path``
            that is a file or a symlink target that is a file. Defaults to
            ``0o600``.
        mode_dir (int, optional): The mode applied to the given ``path`` that
            is a directory or a symlink target that is a directory.  Defaults
            to ``0o700``.
        include_parent (bool, optional): A value of ``True`` will chmod the
            parent directory of the given ``path`` that contains a
            a :term:`glob pattern`.  Defaults to ``False``.
...
def directory_present(
    ...
    Args:
        ...
        mode (int, optional): The mode applied to the ``path``.  Defaults to
            ``0o700``.
        user (str or int, optional): The "login name" used to set the owner
            of the given ``path``.  A value of ``'-1'`` will leave the
            owner unchanged.  Defaults to the "login name" of the current user.
        group (str or int, optional): The group name used to set
            the group of the given ``path``.  A value of ``'-1'`` will leave the
            group unchanged.  Defaults to the current user's group.

    Raises:
        ValueError: if the given :obj:`path` contains a glob pattern.
        ValueError: if the given :obj:`path` is not an absolute path.
        FileExistsError: if the given :obj:`path` exists and is not a
            directory.
        FileExistsError: if a parent of the given :obj:`path` exists and is
            not a directory.
...
def path_absent(
    ...
    Args:
        path (str or bytes or pathlib.PosixPath or pathlib.WindowsPath): The
            path to remove.  This value can also be a :term:`glob pattern`.
    ...
    if '*' not in path.as_posix():
        if os.path.exists(path.as_posix()):
            if path.is_symlink() is True or path.is_dir() is False:
                path.unlink()
                return
            path = path.joinpath('**')
            include_parent = True
            ...
            return
    search = ''.join(path.as_posix().partition('*')[1:])
    path = Path((path.as_posix().partition('*')[0]))

    if path.is_dir() is True:
        # Go through an unlink any directories that are symlinks.
        # No need to delete any files within symlinked directories.
        for pth in reversed(sorted(path.glob(search))):
            if pth.is_dir() and pth.is_symlink():
                pth.unlink()
        for pth in reversed(sorted(path.glob(search))):
            if pth.is_dir():
                pth.rmdir()
