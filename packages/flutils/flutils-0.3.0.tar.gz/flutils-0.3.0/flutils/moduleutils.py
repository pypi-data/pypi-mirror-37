...
class _CherryPickingLoader(Loader):
    ...
    def exec_module(  # pylint: disable=no-self-use
        ...
        ident_keys = module.__cherry_pick_map__.identifiers.keys  # type: ignore
        for attr in ident_keys():
            _all_.append(attr)
...
def cherry_pick(
    ...
    .. Note:: For projects where startup time is critical, this function allows
       for potentially minimizing the cost of loading a module if it is never
       used. For projects where startup time is not essential, the use of this
       function is heavily discouraged due to error messages created during
       loading being postponed and thus occurring out of context.

    Use this function when there is a need to :term:`cherry-pick` modules that
    will be loaded (unless already loaded) and executed when an attribute is
    accessed.

    Args:
        namespace (dict): This should always be set to
            :obj:`globals() <globals>`
    ...
    Example:
        ...
        Use this dictionary to pass any values to :term:`cherry-picking module`.
        ...
        Please be aware that there are some cases when all of the cherry-picked
        modules will be loaded automatically.  Using any program that
        automatically inspects the cherry-picking module will cause the all of
        the cherry-picked modules to be loaded.  Programs such as ipython and
        pycharm will do this.
    """
...
def lazy_import_module(
    ...
    .. Note:: For projects where startup time is critical, this function allows
       for potentially minimizing the cost of loading a module if it is never
       used. For projects where startup time is not essential then use of this
       function is heavily discouraged due to error messages created during
       loading being postponed and thus occurring out of context.
    ...
    Args:
        ...
        package (str, optional): If ``name`` is specified in relative terms,
            then the ``package`` argument must be set to the name of the
            package which is to act as the anchor for resolving the package
            name.  Defaults to ``None``.
    Returns:
        types.ModuleType: The lazy imported :obj:`module <types.ModuleType>`.
        This module will postpone the execution of it's loader until the
