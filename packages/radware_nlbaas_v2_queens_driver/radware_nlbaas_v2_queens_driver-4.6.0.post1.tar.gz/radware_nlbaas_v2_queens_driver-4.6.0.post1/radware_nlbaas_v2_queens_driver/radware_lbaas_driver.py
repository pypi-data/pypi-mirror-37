...
class RadwareLBaaSV2Driver(base_driver.RadwareLBaaSBaseV2Driver):
    ...
    def __init__(self, plugin, module_fqn=None):
        ...
        rad = rad_config.RadwareDirectConfig(module_name)
        if rad.missing():
            rad = rad_config.RadwareOsloConfig(module_name)
        if rad.missing():
        ...
        self.workflow_params['provision_service'] = rad.provision_service
        self.workflow_params['configure_l3'] = rad.configure_l3
        self.workflow_params['configure_l4'] = rad.configure_l4
    ...
    @log_helpers.log_method_call
    def get_status(self, ctx, lb):

        wf_name = self._get_wf_name(lb)
        resource = '/api/workflow/%s/action/%s' % (
            wf_name, self.status_action_name)
        response = _rest_wrapper(self.rest_client.call('POST', resource,
                                 None, TEMPLATE_HEADER), success_codes=[202])
        LOG.debug('status_action  response: %s ', response)

        resource = '/api/workflow/%s/parameters' % (wf_name)
        response = _rest_wrapper(self.rest_client.call('GET', resource,
                                 None, TEMPLATE_HEADER), success_codes=[200])
        LOG.debug('status_values  response: %s ', response)
