...
def gmd(R, S, fm, fs):
    ...
    Returns:
        double : the generalized merge distance between the two list of groups        
    """
...
def evaluate(R, S, metric):
    """
    Evaluate R against S using the metric 
    Args:
        R (list of list): the gold standard
        S (list of list): the entity resolution we want to evaluate
        metric (str) : the metric we want to use to evaluate : bmd, precision, recall or variation_of_information
    Returns:
    ...
    if metric == 'precision':
        ...
        distance_to_splitted = gmd(
            R, S_splitted, metrics_cost[metric]['merge_cost'], metrics_cost[metric]['split_cost'])
        if distance_to_splitted == 0:
            return 0
        return 1 - gmd(R, S, metrics_cost[metric]['merge_cost'], metrics_cost[metric]['split_cost']) / distance_to_splitted

    if metric == 'recall':
        S_splitted = splitted(S_set)
        distance_from_splitted = gmd(
            S_splitted, S, metrics_cost[metric]['merge_cost'], metrics_cost[metric]['split_cost'])
        if distance_from_splitted == 0:
            return 0
        return 1 - gmd(R, S, metrics_cost[metric]['merge_cost'], metrics_cost[metric]['split_cost']) / distance_from_splitted
