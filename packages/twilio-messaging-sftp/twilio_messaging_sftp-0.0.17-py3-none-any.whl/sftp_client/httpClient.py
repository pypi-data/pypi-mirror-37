...
class MessageWorker(Thread):
    ...
    def run(self):
        while True:
            ...
            client, index, msg, result = self.queue.get()
            try:
                logger.info('{}: Sending message ({})'.format(self.name,msg))
                msg.all_params['response'] = client.send_message(msg)
            except Exception as error:
                logger.error('{}: Error {} sending message ({})'.format(self.name,error,msg))
                msg.all_params['response'] = str(error)
            finally:
                result[index] = msg.all_params
...
class MessageClient:
    ...
    def response_list_to_csv_file(self,responses,filename):
        with open(filename, 'w') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=responses[0].keys())
            writer.writeheader()
    ...
    def send_message(self, query_params):
        ...
        return ("MessageSid: {} / Status: {} / Error: {} / ErrorCode: {}".format(message.sid,message.status,message.error_message,message.error_code))

    def enqueue_messages(self, messages, output_filename):
        ...
        for index,msg in enumerate(messages):
            ...
            queue.put((self,index,msg,responses))

        # Causes the main thread to wait for the queue to finish processing all the tasks
        queue.join()
        logging.info('Took %s seconds', time() - ts)
        print(responses)
        self.response_list_to_csv_file(responses,output_filename)
        return responses
