           ...
           validDocumentToImportExtensions:
            try:
                ...
                for row in range(2, sheet.max_row + 1):
                    ...
                    self.logger.info('Processing {2}/{3}: Complex Asset Id: '\
                        '[{0}]; File name: [{1}]; External ID: [{4}]'.format(
                        assetId, fileNameToUpload, row, sheet.max_row,
                        external_id))
                    if fileNameToUpload is not None and \
                    not fileNameToUpload.isspace() and \
                    os.path.exists(fileNameToUpload):
                        if fileNameToUpload.lower().split('.')[1] in validImageExtensions:
