...
import csv
import logging
from numpy import array, dot, eye, log, zeros
from scipy import linalg

from equilibrator_api.sbtab_multi import ReadMultiSBtab, ToDataFrame, \
    GetCustomTableInfo
...
class Pathway(object):
    ...
    def __init__(self, reactions, fluxes, dG0_r_primes, name_to_cid=None,
        ...
        null_proj = eye(self.S.shape[1]) - dot(self.S.T, Spinv)
    ...
    def from_sbtab(self, file_name):
        ...
        """
        tdict = ReadMultiSBtab(file_name)
        tdict = dict(map(lambda s: (s.lower(), tdict[s]), tdict.keys()))
        expected_tnames = ['reaction', 'relativeflux',
                           'concentrationconstraint', 'parameter']
        missing = set(expected_tnames).difference(tdict.keys())
        if missing:
    ...
    def from_separate_sbtabs(self, reaction_sbtab, flux_sbtab,
        ...
        reaction_df = ToDataFrame(reaction_sbtab)
        flux_df = ToDataFrame(flux_sbtab)
        bounds_df = ToDataFrame(bounds_sbtab)
        keqs_df = ToDataFrame(keqs_sbtab)
        ...
        pH = GetCustomTableInfo(keqs_sbtab, 'pH')
        ionic_strength = GetCustomTableInfo(keqs_sbtab, 'IonicStrength')
