from midaxusers.models import db, User, UserAttributes, UserLogin, PasswordNeedsResetError
...
def create_user():
    ...
    verify_domain_rights(loggeduser, targetdomain = request.json['domain'])    
...
def update_user(uuid):    
    ...
    verify_domain_rights(request.loggeduser, targetdomain = targetuser.domain)  

    try:
        verify_domain_rights(request.loggeduser, targetdomain = request.json['domain'])  
    except KeyError:
        pass 
...
def change_user_logins(uuid):
    ...
    verify_domain_rights(loggeduser, targetuser = targetuser)    
    ...
    try:                   
        for key, value in request.json.items():
            ...
            if value['login_key'] is None or len(value['login_key']) == 0:
                del targetuser.logins[key]
            else:
                targetuser.logins[key] = UserLogin(value)            
...
def deactivate_user(uuid):
    ...
    verify_domain_rights(request.loggeduser, targetuser = targetuser)
