...
from .visual_clustering import plot_colony_clusters
...
#             self.data[new_col_name] = self.data[new_col_name].str.join('')
    ...
    def update_plate_layout(self):
        ...
        well_letters = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H',
                        'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P']
        plate_well_letters = well_letters[:self.plate_rows]
        print(self.plate_rows, self.plate_cols, plate_well_letters)
        for row, well_letter in enumerate(plate_well_letters):
            for col in range(self.plate_cols):
                ...
                try:
                    ...
                    self.plate_layout_dict[well_letter + str(col+1)] = ''
        # Restore the plate layout. Put it here sinece the clear button is
        # connected to this function
        self.table.setVerticalHeaderLabels(well_letters)
...
#                self.plot_colony_clusters_wrapper())
    ...
    def plot_colony_clusters_wrapper(self):
        ...
        visual_plot_params['color_col'] = str(
            self.clustering_window.comboBox_color.currentText())
        plot_colony_clusters(self, visual_plot_params)
    ...
    def load_csv(self):
        ...
        if len(thousands_sep) < 1:
            thousands_sep = None
        print(value_sep, dec_sep, com_prefix, thousands_sep)
        ...
        if self.file_path == '':
            ...
            self.data = self.data.rename(columns={
                'WellId': 'Well', 'FieldNumber': 'Field', 'XCentroid': 'Left',
                'YCentroid': 'Top'})
...
#            self.tableWidget_plate_layout.clear()
            ...
            comb_box_items = [self.comboBox_well_col.itemText(i) for i in
                              range(self.comboBox_well_col.count())]
            if 'Well' in comb_box_items:
                self.comboBox_well_col.setCurrentText('Well')
            if 'Left' in comb_box_items:
                self.comboBox_x_col.setCurrentText('Left')
            if 'Top' in comb_box_items:
                self.comboBox_y_col.setCurrentText('Top')
