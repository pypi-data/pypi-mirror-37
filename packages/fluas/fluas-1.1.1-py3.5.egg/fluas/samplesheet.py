...
def is_samplesheet(dataframe, additional_cols=None):
    ...
    """
    required_cols = ['POC', 'SAMPLE_ID', 'PRIMER_PLATE_NUMBER',
        'PRIMER_WELL_POSITION', 'PRIMER_BARCODE']
    if additional_cols:
...
def preprocess_samplesheet(excel_file, out_file=None, cols=None, reverse_complement=False):
    ...
    if any(df.POC.isnull()):
        raise ValueError("All rows must contain a POC")
    # make sure there's a barcode per sample
    if any(df.PRIMER_BARCODE.isnull()):
        raise ValueError("All rows must contain a PRIMER BARCODE")
    # make sure there's a sample id per sample entry
    if any(df.SAMPLE_ID.isnull()):
        raise ValueError("All rows must contain a SAMPLE ID")
    ...
    df['VALID_SAMPLE_ID'] = np.where(df.duplicated(subset="VALID_SAMPLE_ID", keep=False),
                                df['VALID_SAMPLE_ID'] + '-' + df['PRIMER_BARCODE'],
                                df['VALID_SAMPLE_ID'])
    # change separator where multiple POCs are listed
    df['POC'] = df['POC'].apply(lambda x: x.replace(",", ";"))
    if cols:
...
def barcode_tsv_from_samplesheet(csv, out_file=None):
    ...
    df[['VALID_SAMPLE_ID', 'PRIMER_BARCODE']].to_csv(out_file, index=False, header=False, sep="\t")
    return out_file
