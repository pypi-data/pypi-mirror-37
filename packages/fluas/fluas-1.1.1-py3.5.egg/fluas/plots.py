...
def lorenz_curve(x, out_file, xlabel="Fraction of Samples", ylabel="Fraction of On Target Reads", title="Read Distribution"):
    """Generate Lorenz curve plot as of `x` to file `out_file`. Outfile is PDF.
...
def read_joining_summary_plot(a, b, out_file, colors=['r', 'b'],
...
def heatmap_by_group(df, out_file, values, index, column, groupby):
    ...
    fig, axes = plt.subplots(figsize=(6,12), nrows=grouped_df.ngroups, sharex=True, sharey=True)
    col_labels = df[column].unique()
    row_labels = df[index].unique()
    for key, ax in zip(sorted(grouped_df.groups.keys()), axes.flatten()):
        tdf = pd.pivot_table(grouped_df.get_group(key), values=values,
