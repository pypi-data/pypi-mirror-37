...
class Variable:
    ...
    def __array_dimensions(self) -> str:
        if isinstance(self.array, CArrayLiteral):
            ...
            array = "[]"
        elif self.array is None and isinstance(self.value, Iterable):
            try:
                val_shape = _shape(self.value)
    ...
    def generate_initialization(
        ...
        if isinstance(self.value, (CArrayLiteral, CStructLiteral)):
            self.__generate_array_struct_initialization(
                self.value, indent, writer
            )
        else:
            formatters = LiteralFormatters()
            assignment = generate_c_value_initializer(  # type: ignore
                self.value, formatters
            )
