...
class DelegatedPermission(BasePermission):
    ...
    def __init__(self, rest_permissions, *delegated_fields, mapping=None, delegated_objects_getter=None,
        ...
        :param mapping: either a string (all actions will be mapped to this string), dict or
                        callable taking an action and returning mapped action name
...
class RestPermissions:
    ...
    def apply(self, permissions=None, add_django_permissions=None):
        ...
        def decorate(viewset_class):
            ...
            model_queryset_factory = self.create_queryset_factory(
                model_class,
                getattr(viewset_class, 'get_perms_queryset',
    ...
    def get_model_permissions(self, model_class):
        ...
        class _Permission(permissions.BasePermission):
            def has_object_permission(self, request, view, obj):
                ret = condition.has_object_permission(request, view, obj)
                if not ret:
                    no_object_permissions_signal.send_robust(sender=view, request=request, object=obj)
                return ret

            def has_permission(self, request, view):
                ret = condition.has_permission(request, view)
                if not ret:
                    no_permissions_signal.send_robust(sender=view, request=request)
                return ret
    ...
    def has_object_permission(self, request, clazz, action, instance):
        ...
        ret = instance_permissions.has_object_permission(request, instance_view, instance)
        if not ret:
            no_object_permissions_signal.send_robust(sender=self, request=request, action=action, object=instance)
        return ret


no_object_permissions_signal = django.dispatch.Signal(providing_args=["request", "object"])
no_permissions_signal = django.dispatch.Signal(providing_args=["request"])
