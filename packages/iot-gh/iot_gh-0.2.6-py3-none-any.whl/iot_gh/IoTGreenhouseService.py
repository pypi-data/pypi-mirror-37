...
class IoTGreenhouseService(object):
    ...
    def __init__(self):
        ...
        self.green_house = make_greenhouse("new greenhouse")
    ...
    def _make_greenhouse(self, name):
        ...
        self.green_house.version = self.version

        self. update_green_house()

        return gh
    
    def update_green_house(self):
        """Updates IoTGreenhouse object by reading all house 
        states and refreshing gh object.
        """
        try:
    
            self.green_house.led_red_state = self.lamps.red.get_state()
            self.green_house.led_red_status = self.lamps.red.get_status()
            self.green_house.led_white_state = self.lamps.white.get_state()
            self.green_house.led_white_status = self.lamps.white.get_status()
            self.green_house.led_dual_state =  self.lamps.dual.get_state()
            self.green_house.led_dual_status = self.lamps.dual.get_status()
            self.green_house.switch_pb_state = self.switches.push_button.get_state()
            self.green_house.switch_pb_status = self.switches.push_button.get_status()
            self.green_house.switch_toggle_state = self.switches.toggle.get_state()
            self.green_house.switch_toggle_status = self.switches.toggle.get_status()
            self.green_house.fan_state =  self.fan.get_state()
            self.green_house.fan_status =  self.fan.get_status()
            self.green_house.servo_position = self.servo.get_value()
            self.green_house.servo_status = self.servo.get_status()
            self.green_house.heater_state = self.lamps.white.get_state()
            self.green_house.heater_status = self.lamps.white.get_status()
            self.green_house.buzzer_state = self.buzzer.get_state()
            self.green_house.buzzer_status = self.buzzer.get_status()
            self.green_house.ain_pot_raw = self.analog.pot.get_value()
            self.green_house.ain_light_raw =  self.analog.light.get_value()
            self.green_house.ain_aux_raw =  self.analog.aux.get_value()

            self.green_house.temp_inside_C = self.temperature.get_inside_temp_C()
            self.green_house.temp_inside_F = self.temperature.get_inside_temp_F()
            self.green_house.temp_outside_C = self.temperature.get_outside_temp_C()
            self.green_house.temp_outside_F = self.temperature.get_outside_temp_F()
            
            self.green_house.last_update = datetime.datetime.now().replace(microsecond=0).isoformat(' ')
            
            _make_message()
            

        except Exception as e:
            self.green_house.message = "Updata exception: %s" % e.args[0]
