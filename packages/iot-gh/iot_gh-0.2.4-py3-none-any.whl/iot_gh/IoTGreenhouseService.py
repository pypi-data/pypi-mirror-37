...
class IoTGreenhouseService(object):
    ...
    def make_greenhouse(self, name):
        ...
        gh.house_number = config["IOT_GREENHOUSE"]["HOUSE_NUMBER"]
    ...
    def update_state(self, gh):
        ...
        try:
            ...
            gh.led_red_status = self.lamps.red.get_status()
            gh.led_white_state = self.lamps.white.get_state()
            gh.led_white_status = self.lamps.white.get_status()
            gh.led_dual_state =  self.lamps.dual.get_state()
            gh.led_dual_status = self.lamps.dual.get_status()
            gh.switch_pb_state = self.switches.push_button.get_state()
            gh.switch_pb_status = self.switches.push_button.get_status()
            gh.switch_toggle_state = self.switches.toggle.get_state()
            gh.switch_toggle_status = self.switches.toggle.get_status()
            gh.fan_state =  self.fan.get_state()
            gh.fan_status =  self.fan.get_status()
            gh.servo_position = self.servo.get_value()
            gh.servo_status = self.servo.get_status()
            gh.buzzer_state = self.buzzer.get_state()
            gh.buzzer_status = self.buzzer.get_status()
            gh.ain_pot_raw = self.analog.pot.get_value()
            gh.ain_light_raw =  self.analog.light.get_value()
            gh.ain_aux_raw =  self.analog.aux.get_value()

            gh.temp_inside_C = self.temperature.get_inside_temp_C()
            gh.temp_inside_F = self.temperature.get_inside_temp_F()
            gh.temp_outside_C = self.temperature.get_outside_temp_C()
            gh.temp_outside_F = self.temperature.get_outside_temp_F()
            
            gh.valid_state = "True"
            gh.last_update = datetime.datetime.now().replace(microsecond=0).isoformat(' ')
            gh.message = "IoT Greenhouse"
