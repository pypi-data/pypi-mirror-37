...
class Sandbox(object):
    ...
    def _handle_result(self, data, execution_type='pipeline', pipeline_step=-1):
        ...
        if execution_type == 'pipeline' and data.get('number_of_pages', 0) > 0:
    ...
    def intermediate_data(self, pipeline_step):
    ...
    def auto(self, auto_params, seed, run_parallel=False):
        ...
        if seed.name == 'Custom':
            ...
            print("Running Auto Pipeline with {} Seed and Auto Query:\n".format(seed.name))
            ...
            segmenter_name = [step['name'] for step in self.pipeline.to_list() if step['type'] == 'segmenter'][0]
            print("Running Auto Pipeline with {} Seed and {} Segmenter:\n".format(seed.name, segmenter_name))
            full_pipeline = deepcopy(self.pipeline.to_list()) + seed.pipeline
        else:
    ...
    def retrieve_autosegment(self):
        result = self.retrieve(execution_type='autosegment_search')
        return result

    def retrieve(self, execution_type='pipeline', silent=False, page_index=1, **kwargs):
    ...
    def retrieve_gridsearch(self):
        result = self.retrieve(execution_type='grid_search')
        return result
...
def describe_pipeline(pipeline, show_params=True):
    print("------------------------------------------------------------------------")
...
def filter_params(inputs, num_tabs=1):
    not_params = [
        'input_columns', 'input_column', 'input_data', 'group_column',
    ...
    param_values = [
        num_tabs * "\t" + "Param: {0} {1}".format(param + ':',
                                                  inputs[param])
        for param in params
        ]
    return '\n'.join(param_values)
