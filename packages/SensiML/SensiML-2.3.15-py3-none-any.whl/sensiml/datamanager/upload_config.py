...
def get_config(config_info_file=None):
    ...
    if config_info_file is None:
        raise Exception("*** Sorry, Config File Missing *** " + "file exist? " + str(os.path.isfile(config_info_file)) + '..filename: ' + config_info_file )
    ...
    metadata_columns = (config.get(sections[2], 'metadata_column')).split(',') if len(sections) > 2 else None
