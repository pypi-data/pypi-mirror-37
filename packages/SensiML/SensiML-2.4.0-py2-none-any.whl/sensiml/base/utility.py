...
def read_response_json_data(response):
    ...
    try:
        ...
        category_response_error(json.loads(response.reason), response.status_code)
    except:
        pass
    ...
    if 'eventlabels' in data.columns:
        ...
        for label in labels:
            for name, info in label.items():
                for values in info:
                    try:
                        column_name = '{0}: {1}'.format(name.encode('ascii', 'ignore'), values['value'])
                    except UnicodeEncodeError:
                        column_name = '{0}: {1}'.format(name.encode('ascii', 'ignore'), values['value'].encode('ascii', 'ignore'))
...
def make_cost_report(device_config, cost_dict=None, neuron_array=None):
    ...
    for i, step in enumerate(cost_dict['pipeline']):
        ...
        fe_subtotals = [a + (int(b) if b == '0' else b) for a, b in zip(fe_subtotals, costs)]
    ...
    if cost_dict.get('framework', None):
        print('\n\nFramework Costs')
        horizontal_line()
        framework_costs = populate_cost_keys(cost_columns, cost_dict['framework'], na_string)
    ...
    totals_minus_sensors ={cost: sum_or_string(cost_dict_minus_sensors, cost, na_string) for cost in cost_columns}
    ...
    if cost_dict.get('neurons', None) is None:
        print('   This table does not include the costs of the neuron array or sensor buffer')
