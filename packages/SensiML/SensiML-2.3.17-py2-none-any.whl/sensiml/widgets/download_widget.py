...
class DownloadWidget(BaseWidget):
    ...
    def select_platform(self, b):
        ...
        if len(self._widget_source.options) > 0:
            self._widget_source.value= self._widget_source.options[0] if default_source_driver is None else default_source_driver[0]
        else:
        ...
        if len(self._widget_source_rate.options) > 0:
            self._widget_source_rate.value = self._widget_source_rate.options[0] if default_source_driver is  None else default_source_driver[1]
        else:
    ...
    def get_platform_names(self):
        ...
        for platform in self._dsk.platforms.platform_list:
            pf['{} {}'.format(platform.board_name, platform.platform_version)] =  platform.id
    ...
    def _change_rate(self, change):
        ...
        if self._widget_source.value == 'Custom':
            ...
            self._widget_source_rate.options = supported_source_drivers.get(self._widget_source.value, [''])
