...
class PlatformDescription(object):
    ...
    def refresh(self):
        ...
        if err is False:
            ...
            self._supported_source_drivers = json.loads(response_data['supported_source_drivers'])
            self._supported_outputs = json.loads(response_data['supported_outputs'])
            self._supported_outputs = json.loads(response_data['target_os_options'])
            self._execution_parameters = json.loads(response_data['execution_parameters'])
    ...
    def initialize_from_dict(self, input_dictionary):
        ...
        self._supported_source_drivers = json.loads(input_dictionary.get('supported_source_drivers', '{"Motion" : [200, 100, 50, 25], "Audio" : [16000], "Default" : ["Motion", 100], "Custom":[]}'))
        self._supported_outputs = json.loads(input_dictionary.get('supported_outputs', '["BLE","SERIAL","LED"]'))
        self._target_os_options = json.loads(input_dictionary.get('target_os_options', '["ARM GCC Generic"]'))
        self._execution_parameters = json.loads(input_dictionary.get('execution_parameters', '{"application":"Default"}'))
