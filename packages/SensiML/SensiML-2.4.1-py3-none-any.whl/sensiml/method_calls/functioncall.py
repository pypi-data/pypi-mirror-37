...
class FunctionCall(PipelineStep):
    ...
    def _check_inputs(self, output_vars = []):
        ...
        for param in [getattr(self, input['name']) for input in self._input_contract]:
            if isinstance(param, string_types) and param.find('temp.') >  -1 and not param in output_vars:
    ...
    def _to_dict(self):
        ...
        prop_dict['inputs'] = {}
        for prop in [input['name'] for input in self._input_contract if not input.get('handle_by_set', False)]:
            if prop != 'outputs':
