...
def connect(url):
    ...
    while not connected:    
      try:
        sock.connect(netloc)
        globals.logger.info ( "connected to docker at " + netloc )
        return sock, hostname
      except Exception as ex:
        globals.logger.warn  ("ERROR: could not connect to Docker at " + netloc + " ex: " +  str(ex) )
        globals.logger.warn  ( ".. retry in 5s" )
        time.sleep(5) 
        pass
