...
cdef int _INT_MAX = 1000000000
cdef int _INT_MIN = -1000000000
...
def d(int sides) -> int:
    assert sides >= 1, f"Input Error, 'sides' must be >= 1."
    return _d(sides)

def dice(int rolls, int sides) -> int:
    assert rolls >= 1 and sides >= 1, f"Input Error, 'rolls' & 'sides' must be >= 1."
...
def random_value(arr) -> object:
    assert len(arr) >= 1, f"Input Error, sequence must not be empty."
    idx = _random_below(len(arr))
    return arr[idx]
...
def cumulative_weighted_choice(table: object) -> object:
    assert len(table) >= 1, f"Input Error, sequence must not be empty."
...
class Mostly:
    """ The Quantum Monty """
    __slots__ = ("data", "max_id", "methods")

    def __init__(self, arr):
        size = len(arr)
        assert size >= 3, f"Input Error, sequence length must be >= 3."
        self.data = tuple(arr)
        self.max_id = size - 1
        self.methods = (
            self.mostly_first,
    ...
    def mostly_flat(self) -> object:
        return self.data[_zero_flat(self.max_id)]
...
class WeightedChoice:
    ...
    @staticmethod
    def _setup(weighted_table, is_cumulative: bool, non_unique: bool=False):
    ...
    @staticmethod
    def _optimize(weighted_table: tuple, is_cumulative: bool) -> list:
    ...
    def _package(self, data: list) -> tuple:
        cum_weight = 0
        for w_pair in data:
            cum_weight += w_pair[0]
            w_pair[0] = cum_weight
