...
`}static get properties(){return{hass:{type:Object,observer:"_hassChanged"},_config:Object}}constructor(){super();this._elements=[]}ready(){super.ready();if(this._config)this._buildConfig()}getCardSize(){return 1+(this._config?this._config.entities.length:0)}_showHeaderToggle(show){return!1!==show}_showHeader(config){return config.title||config.show_header_toggle}setConfig(config){this._config=config;this._rows=config.entities.map(item=>"string"===typeof item?{entity:item}:item);if(this.$)this._buildConfig()}_buildConfig(){const root=this.$.states;while(root.lastChild){root.removeChild(root.lastChild)}this._elements=[];for(const row of this._rows){const entityId=row.entity,element=createRowElement(row);if(entityId&&!common_const.c.includes(Object(compute_domain.a)(entityId))){element.classList.add("state-card-dialog");element.addEventListener("click",()=>this.fire("hass-more-info",{entityId}))}element.hass=this.hass;this._elements.push(element);const container=document.createElement("div");container.appendChild(element);root.appendChild(container)}}_hassChanged(hass){this._elements.forEach(element=>{element.hass=hass})}_filterEntities(items){return items.filter(item=>"string"===typeof item||item.entity).map(item=>"string"===typeof item?item:item.entity)}}customElements.define("hui-entities-card",hui_entities_card_HuiEntitiesCard);var lit_element=__webpack_require__(260);function turnOnOffEntity(hass,entityId,turnOn=!0){const stateDomain=Object(compute_domain.a)(entityId),serviceDomain="group"===stateDomain?"homeassistant":stateDomain;let service;switch(stateDomain){case"lock":service=turnOn?"unlock":"lock";break;case"cover":service=turnOn?"open_cover":"close_cover";break;default:service=turnOn?"turn_on":"turn_off";}hass.callService(serviceDomain,service,{entity_id:entityId})}function toggleEntity(hass,entityId){const turnOn=common_const.h.includes(hass.states[entityId].state);turnOnOffEntity(hass,entityId,turnOn)}function validEntityId(entityId){return /^(\w+)\.(\w+)$/.test(entityId)}var state_icon=__webpack_require__(112),compute_state_domain=__webpack_require__(22),styleMap=__webpack_require__(453),localize_base_mixin=__webpack_require__(73);const HassLocalizeLitMixin=(superClass)=>class extends Object(localize_base_mixin.a)(superClass){static get properties(){return{hass:{},localize:{}}}connectedCallback(){super.connectedCallback();let language,resources;if(this.hass){language=this.hass.language;resources=this.hass.resources}this.localize=this.__computeLocalize(language,resources)}updated(changedProperties){super.updated(changedProperties);if(!changedProperties.has("hass")){return}let oldLanguage,oldResources;const hass=changedProperties.get("hass");if(hass){oldLanguage=hass.language;oldResources=hass.resources}let language,resources;if(this.hass){language=this.hass.language;resources=this.hass.resources}if(oldLanguage!==language||oldResources!==resources){this.localize=this.__computeLocalize(language,resources)}}};class hui_entity_button_card_HuiEntityButtonCard extends HassLocalizeLitMixin(lit_element.a){static get properties(){return{hass:{},config:{}}}getCardSize(){return 2}setConfig(config){if(!validEntityId(config.entity)){throw new Error("Invalid Entity")}this.config=config;if(this.hass){this.requestUpdate()}}render(){if(!this.config){return lit_element.b``}const stateObj=this.hass.states[this.config.entity];return lit_element.b`
      ${this.renderStyle()}
      <ha-card @click="${this.handleClick}">
        ${!stateObj?lit_element.b`<div class="not-found">Entity not available: ${this.config.entity}</div>`:lit_element.b`
              <paper-button>
                <div>
                  <ha-icon
    ...
    `}static get properties(){return{_config:Object}}setConfig(config){this._config=config}getCardSize(){return 4}_toStr(obj){return JSON.stringify(obj,null,2)}}customElements.define("hui-error-card",hui_error_card_HuiErrorCard);var classMap=__webpack_require__(454),repeat=__webpack_require__(455);class hui_glance_card_HuiGlanceCard extends HassLocalizeLitMixin(lit_element.a){static get properties(){return{hass:{},config:{}}}getCardSize(){return 3}setConfig(config){this.config=config;const entities=processConfigEntities(config.entities);for(const entity of entities){if("call-service"===entity.tap_action&&!entity.service){throw new Error("Missing required property \"service\" when tap_action is call-service")}}this.style.setProperty("--glance-column-width",config.column_width||"20%");if(config.theming){if("string"!==typeof config.theming){throw new Error("Incorrect theming config.")}this.classList.add(`theme-${config.theming}`)}this.configEntities=entities;if(this.hass){this.requestUpdate()}}render(){if(!this.config||!this.hass){return lit_element.b``}const{title}=this.config,states=this.hass.states,entities=this.configEntities.filter(conf=>conf.entity in states);return lit_element.b`
      ${this.renderStyle()}
    ...
    `}renderEntity(entityConf){const stateObj=this.hass.states[entityConf.entity];return lit_element.b`
      <div
        class="entity"
        .entityConf="${entityConf}"
        @click="${this.handleClick}"
      >
        ${!1!==this.config.show_name?lit_element.b`<div class="name">${"name"in entityConf?entityConf.name:Object(compute_state_name.a)(stateObj)}</div>`:""}
        <state-badge
          .stateObj="${stateObj}"
          .overrideIcon="${entityConf.icon}"
        ></state-badge>
        ${!1!==this.config.show_state?lit_element.b`<div>${Object(compute_state_display.a)(this.localize,stateObj)}</div>`:""}
      </div>
...
//# sourceMappingURL=ee75d8d43fc454407590.chunk.js.map