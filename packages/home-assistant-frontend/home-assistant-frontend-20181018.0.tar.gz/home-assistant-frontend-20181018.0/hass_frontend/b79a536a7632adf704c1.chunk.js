...
`}static get properties(){return{hass:{type:Object,observer:"_hassChanged"},_config:Object}}constructor(){super();this._elements=[]}ready(){super.ready();if(this._config)this._buildConfig()}getCardSize(){return 1+(this._config?this._config.entities.length:0)}_showHeaderToggle(show){return!1!==show}_showHeader(config){return config.title||config.show_header_toggle}setConfig(config){this._config=config;this._rows=config.entities.map(item=>"string"===typeof item?{entity:item}:item);if(this.$)this._buildConfig()}_buildConfig(){const root=this.$.states;while(root.lastChild){root.removeChild(root.lastChild)}this._elements=[];for(const row of this._rows){const entityId=row.entity,element=createRowElement(row);if(entityId&&!common_const.c.includes(Object(compute_domain.a)(entityId))){element.classList.add("state-card-dialog");element.addEventListener("click",()=>this.fire("hass-more-info",{entityId}))}element.hass=this.hass;this._elements.push(element);const container=document.createElement("div");container.appendChild(element);root.appendChild(container)}}_hassChanged(hass){this._elements.forEach(element=>{element.hass=hass})}_filterEntities(items){return items.filter(item=>"string"===typeof item||item.entity).map(item=>"string"===typeof item?item:item.entity)}}customElements.define("hui-entities-card",hui_entities_card_HuiEntitiesCard);var lit_element=__webpack_require__(229);function turnOnOffEntity(hass,entityId,turnOn=!0){const stateDomain=Object(compute_domain.a)(entityId),serviceDomain="group"===stateDomain?"homeassistant":stateDomain;let service;switch(stateDomain){case"lock":service=turnOn?"unlock":"lock";break;case"cover":service=turnOn?"open_cover":"close_cover";break;default:service=turnOn?"turn_on":"turn_off";}hass.callService(serviceDomain,service,{entity_id:entityId})}function toggleEntity(hass,entityId){const turnOn=common_const.h.includes(hass.states[entityId].state);turnOnOffEntity(hass,entityId,turnOn)}function validEntityId(entityId){return /^(\w+)\.(\w+)$/.test(entityId)}var state_icon=__webpack_require__(112),compute_state_domain=__webpack_require__(22),apply_themes_on_element=__webpack_require__(142),styleMap=__webpack_require__(454),localize_base_mixin=__webpack_require__(73);const HassLocalizeLitMixin=(superClass)=>class extends Object(localize_base_mixin.a)(superClass){static get properties(){return{hass:{},localize:{}}}connectedCallback(){super.connectedCallback();let language,resources;if(this.hass){language=this.hass.language;resources=this.hass.resources}this.localize=this.__computeLocalize(language,resources)}updated(changedProperties){super.updated(changedProperties);if(!changedProperties.has("hass")){return}let oldLanguage,oldResources;const hass=changedProperties.get("hass");if(hass){oldLanguage=hass.language;oldResources=hass.resources}let language,resources;if(this.hass){language=this.hass.language;resources=this.hass.resources}if(oldLanguage!==language||oldResources!==resources){this.localize=this.__computeLocalize(language,resources)}}};class hui_entity_button_card_HuiEntityButtonCard extends HassLocalizeLitMixin(lit_element.a){static get properties(){return{hass:{},config:{}}}getCardSize(){return 2}setConfig(config){if(!validEntityId(config.entity)){throw new Error("Invalid Entity")}this.config=Object.assign({theme:"default"},config);if(this.hass){this.requestUpdate()}}render(){if(!this.config){return lit_element.c``}const stateObj=this.hass.states[this.config.entity];Object(apply_themes_on_element.a)(this,this.hass.themes,this.config.theme);return lit_element.c`
      ${this.renderStyle()}
    ...
    `}static get properties(){return{_config:Object}}setConfig(config){this._config=config}getCardSize(){return 4}_toStr(obj){return JSON.stringify(obj,null,2)}}customElements.define("hui-error-card",hui_error_card_HuiErrorCard);var classMap=__webpack_require__(346),repeat=__webpack_require__(455);class hui_glance_card_HuiGlanceCard extends HassLocalizeLitMixin(lit_element.a){static get properties(){return{hass:{},config:{}}}getCardSize(){return 3}setConfig(config){this.config=Object.assign({theme:"default"},config);const entities=processConfigEntities(config.entities);for(const entity of entities){if("call-service"===entity.tap_action&&!entity.service){throw new Error("Missing required property \"service\" when tap_action is call-service")}}this.style.setProperty("--glance-column-width",config.column_width||"20%");this.configEntities=entities;if(this.hass){this.requestUpdate()}}render(){if(!this.config||!this.hass){return lit_element.c``}const{title}=this.config,states=this.hass.states,entities=this.configEntities.filter(conf=>conf.entity in states);Object(apply_themes_on_element.a)(this,this.hass.themes,this.config.theme);return lit_element.c`
      ${this.renderStyle()}
      <ha-card .header="${title}">
        <div class="entities ${Object(classMap.a)({"no-header":!title})}">
          ${Object(repeat.a)(entities,entityConf=>entityConf.entity,entityConf=>this.renderEntity(entityConf))}
        </div>
      </ha-card>
    `}renderStyle(){return lit_element.c`
      <style>
    ...
    `}renderEntity(entityConf){const stateObj=this.hass.states[entityConf.entity];return lit_element.c`
      <div
        class="entity"
    ...
    `}static get properties(){return{hass:{type:Object,observer:"_hassChanged"},entity:String,image:String,stateImage:Object,cameraImage:String,aspectRatio:String,filter:String,stateFilter:Object,_imageSrc:String}}static get observers(){return["_configChanged(image, stateImage, cameraImage, aspectRatio)"]}connectedCallback(){super.connectedCallback();if(this.cameraImage){this.timer=setInterval(()=>this._updateCameraImageSrc(),1e4)}}disconnectedCallback(){super.disconnectedCallback();clearInterval(this.timer)}_configChanged(image,stateImage,cameraImage,aspectRatio){const ratio=parseAspectRatio(aspectRatio);if(ratio&&0<ratio.w&&0<ratio.h){this.$.wrapper.style.paddingBottom=`${(100*ratio.h/ratio.w).toFixed(2)}%`;this.$.wrapper.classList.add("ratio")}if(cameraImage){this._updateCameraImageSrc()}else if(image&&!stateImage){this._imageSrc=image}}_onImageError(){this._imageSrc=null;this.$.image.classList.add("hidden");if(!this.$.wrapper.classList.contains("ratio")){this.$.brokenImage.style.setProperty("height",`${this._lastImageHeight||"100"}px`)}this.$.brokenImage.classList.remove("hidden")}_onImageLoad(){this.$.image.classList.remove("hidden");this.$.brokenImage.classList.add("hidden");if(!this.$.wrapper.classList.contains("ratio")){this._lastImageHeight=this.$.image.offsetHeight}}_hassChanged(hass){if(this.cameraImage||!this.entity){return}const stateObj=hass.states[this.entity],newState=!stateObj?"unavailable":stateObj.state;if(newState===this._currentState)return;this._currentState=newState;this._updateStateImage();this._updateStateFilter(stateObj)}_updateStateImage(){if(!this.stateImage){this._imageFallback=!0;return}const stateImg=this.stateImage[this._currentState];this._imageSrc=stateImg||this.image;this._imageFallback=!stateImg}_updateStateFilter(stateObj){let filter;if(!this.stateFilter){filter=this.filter}else{filter=this.stateFilter[this._currentState]||this.filter}const isOff=!stateObj||common_const.h.includes(stateObj.state);this.$.image.style.filter=filter||isOff&&this._imageFallback&&"grayscale(100%)"||""}async _updateCameraImageSrc(){try{const{content_type:contentType,content}=await this.hass.callWS({type:"camera_thumbnail",entity_id:this.cameraImage});this._imageSrc=`data:${contentType};base64, ${content}`;this._onImageLoad()}catch(err){this._onImageError()}}}customElements.define("hui-image",hui_image_HuiImage);class hui_image_element_HuiImageElement extends element_click_mixin(polymer_element.a){static get template(){return html_tag.a`
      ...
      ></hui-image>
    `}static get properties(){return{hass:Object,_config:Object}}ready(){super.ready();this.registerMouse(this._config)}setConfig(config){if(!config){throw Error("Error in element configuration")}this.classList.toggle("clickable","none"!==config.tap_action);this._config=config}}customElements.define("hui-image-element",hui_image_element_HuiImageElement);__webpack_require__(211);class hui_service_button_element_HuiServiceButtonElement extends polymer_element.a{static get template(){return html_tag.a`
      <style>
...
//# sourceMappingURL=b79a536a7632adf704c1.chunk.js.map