...
`}static get properties(){return{hass:Object,_loading:{type:Boolean,value:!1},_statusMsg:String,_errorMsg:String,mfaModules:Array}}connectedCallback(){super.connectedCallback();if(!registeredDialog){registeredDialog=!0;this.fire("register-dialog",{dialogShowEvent:"show-mfa-module-setup-flow",dialogTag:"ha-mfa-module-setup-flow",dialogImport:()=>__webpack_require__.e(60).then(__webpack_require__.bind(null,665))})}}_enable(ev){this.fire("show-mfa-module-setup-flow",{hass:this.hass,mfaModuleId:ev.model.module.id,dialogClosedCallback:()=>this._refreshCurrentUser()})}_disable(ev){if(!confirm(this.localize("ui.panel.profile.mfa.confirm_disable","name",ev.model.module.name))){return}const mfaModuleId=ev.model.module.id;this.hass.callWS({type:"auth/depose_mfa",mfa_module_id:mfaModuleId}).then(()=>{this._refreshCurrentUser()})}_refreshCurrentUser(){this.fire("hass-refresh-current-user")}}customElements.define("ha-mfa-modules-card",ha_mfa_modules_card_HaMfaModulesCard);var paper_icon_button=__webpack_require__(62),paper_tooltip=__webpack_require__(237),format_date_time=__webpack_require__(77);class ha_settings_row_HaSettingsRow extends polymer_element.a{static get template(){return html_tag.a`
    <style>
      :host {
        display: flex;
        padding: 0 16px;
    ...
    `}static get properties(){return{hass:Object,narrow:Boolean,languageSelection:{type:String,observer:"languageSelectionChanged"},languages:{type:Array,computed:"computeLanguages(hass)"}}}static get observers(){return["setLanguageSelection(language)"]}computeLanguages(hass){if(!hass||!hass.translationMetadata){return[]}return Object.keys(hass.translationMetadata.translations).map(key=>({tag:key,nativeName:hass.translationMetadata.translations[key].nativeName}))}setLanguageSelection(language){this.languageSelection=language}languageSelectionChanged(newVal){if(newVal!==this.language){this.fire("hass-language-select",{language:newVal})}}ready(){super.ready();if(this.hass&&this.hass.language){this.setLanguageSelection(this.hass.language)}}}customElements.define("ha-pick-language-row",ha_pick_language_row_HaPickLanguageRow);class ha_pick_theme_row_HaPickThemeRow extends Object(localize_mixin.a)(Object(events_mixin.a)(polymer_element.a)){static get template(){return html_tag.a`
    <style>
      a { color: var(--primary-color); }
    </style>
    <ha-settings-row narrow='[[narrow]]'>
...
//# sourceMappingURL=7ac4d90b62e88ad15ee1.chunk.js.map