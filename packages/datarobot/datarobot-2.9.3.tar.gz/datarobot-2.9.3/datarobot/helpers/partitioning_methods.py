...
from datarobot.enums import (CV_METHOD, PERIODICITY_MAX_TIME_STEP, PERIODICITY_TIME_UNITS,
                             TREAT_AS_EXPONENTIAL, DIFFERENCING_METHOD)
...
class FeatureSettings(object):
    ...
    feature_name : string
        name of the feature
    a_priori : bool
        whether feature is an a priori feature, i.e. should be known in advance. If not
        specified, the value from `default_to_a_priori` flag is used.
    """
    def __init__(self, feature_name, a_priori=False):
    ...
    def __eq__(self, other):
        return self.feature_name == other.feature_name and self.a_priori == other.a_priori
...
class DatetimePartitioningSpecification(PartitioningMethod):
    ...
    default_to_a_priori : bool
        (New in version v2.8) Only used for time series projects. Whether to default to treating
        features as a priori.  If not specified, defaults to False.  A priori features are expected
        to be known for dates in the future when making predictions, e.g. "is this a holiday".
    feature_derivation_window_start : int or None
        ...
        Expressed in terms of the timeUnit of the datetime_partition_column.
    feature_derivation_window_end : int or None
        ...
        in terms of the timeUnit of the datetime_partition_column.
    ...
    def __init__(self, datetime_partition_column, autopilot_data_selection_method=None,
                 ...
                 feature_settings=None, forecast_window_start=None, forecast_window_end=None,
                 treat_as_exponential=None, differencing_method=None, periodicities=None):
    ...
    def collect_payload(self, skip_extra_time_series_controls=False):
        ...
        feature_settings = [fs.collect_payload() for fs in self.feature_settings] \
        ...
        payload = {
            ...
            'default_to_a_priori': self.default_to_a_priori,
        ...
        if skip_extra_time_series_controls:
            return payload

        payload['treat_as_exponential'] = self.treat_as_exponential
        payload['differencing_method'] = self.differencing_method

        if self.periodicities:
            payload['periodicities'] = [p.collect_payload() for p in self.periodicities] or None
...
class DatetimePartitioning(object):
    ...
    forecast_window_end : int or None
        ...
        terms of the timeUnit of the datetime_partition_column.
    treat_as_exponential : string, optional
        (New in version v2.9) defaults to "auto". Used to specify whether to treat data
        as exponential trend and apply transformations like log-transform. Use values from
        :py:class:`datarobot.enums.TREAT_AS_EXPONENTIAL` enum.
    ...
    def from_server_data(cls, data):
        ...
        if 'feature_settings' in converted_data:
            converted_data['feature_settings'] = [FeatureSettings(**fs) for fs
    ...
    def generate(cls, project_id, spec):
        ...
        # workaround since API version v2.9 doesn't support special config in datetime partitioning
        # but future versions will
        # TODO: remove skip_extra_time_series_controls=False for
        # DateTimePartitioningSpecification.collect_payload when periodicities got supported
        # on server side for datetimePartitioning/ endpoint
        payload = spec.collect_payload(skip_extra_time_series_controls=True)
        payload.pop('cv_method')
        response = cls._client.post(url, data=payload)
        # TODO: get back to `return cls.from_server_data(response.json())`
        spec_arguments = dict(
            response.json(),
            treat_as_exponential=spec.treat_as_exponential,
            differencing_method=spec.differencing_method,
            periodicities=[p.collect_payload()
                           for p in spec.periodicities] if spec.periodicities else None
        )
        return cls.from_server_data(spec_arguments)
    ...
    def to_specification(self):
        ...
        return DatetimePartitioningSpecification(
            ...
            use_time_series=self.use_time_series, default_to_a_priori=self.default_to_a_priori,
            feature_derivation_window_start=self.feature_derivation_window_start,
            feature_derivation_window_end=self.feature_derivation_window_end,
            forecast_window_start=self.forecast_window_start,
            forecast_window_end=self.forecast_window_end,
    ...
    def to_dataframe(self):
        ...
        Also excludes the time series specific parameters for use_time_series, default_to_a_priori,
        and defining the feature derivation and forecast windows.
        """
