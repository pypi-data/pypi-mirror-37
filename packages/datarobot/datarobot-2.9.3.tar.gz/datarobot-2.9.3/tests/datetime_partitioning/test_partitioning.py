...
def test_partition_to_specification(datetime_partition):
    ...
    assert part_spec.default_to_a_priori == datetime_partition.default_to_a_priori
...
def test_retrieve_time_series(project_id, project_url, datetime_partition_time_series_server_data):
    ...
    assert partition.default_to_a_priori
...
def test_generate(project_id, project_url, datetime_partition_server_data,
    ...
    assert partition.default_to_a_priori == datetime_partition_server_data['defaultToAPriori']
...
def test_generate_time_series(project_id, project_url, datetime_partition_time_series_server_data,
    ...
    assert partition.default_to_a_priori == time_series_partition_spec.default_to_a_priori
...
def test_generate_feature_settings(
    ...
    expected_payload = {
        ...
        'defaultToAPriori': datetime_partition_spec_feature_settings.default_to_a_priori,
...
@pytest.mark.parametrize('a_priori', [None, 'True', 1, {'true': True}, [True]])
def test_create_feature_settings_bad_a_priori(a_priori):
    with pytest.raises(trafaret.DataError):
        FeatureSettings('feature_name', a_priori)
...
@responses.activate
@pytest.mark.usefixtures('client')
def test_generate_no_skip_extra_time_series_controls_in_payload(
    project_id, project_url, holdout_start_date,
    ...
    DatetimePartitioning.generate(project_id, spec_fixture)
    ...
    expected_backtests = [{
        ...
        'validationStartDate': bt.validation_start_date.isoformat(),
        'validationDuration': bt.validation_duration
    } for bt in spec_fixture.backtests]
    expected_payload = {
        'datetimePartitionColumn': spec_fixture.datetime_partition_column,
        'autopilotDataSelectionMethod': spec_fixture.autopilot_data_selection_method,
        'validationDuration': spec_fixture.validation_duration,
        'holdoutStartDate': holdout_start_date.isoformat(),
        'holdoutDuration': spec_fixture.holdout_duration,
        'gapDuration': spec_fixture.gap_duration,
        'numberOfBacktests': spec_fixture.number_of_backtests,
        'backtests': expected_backtests,
        'useTimeSeries': spec_fixture.use_time_series,
        'defaultToAPriori': spec_fixture.default_to_a_priori,
    }
    assert actual_payload == expected_payload
    assert 'periodicity' not in actual_payload
    assert 'treat_as_exponential' not in actual_payload
    assert 'differencing_method' not in actual_payload
