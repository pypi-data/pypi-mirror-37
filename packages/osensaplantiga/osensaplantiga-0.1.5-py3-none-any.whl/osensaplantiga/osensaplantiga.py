...
def connect(dock_serial_number, baudrate=115200, modbus_id=247, timeout=0.1):
...
class Pod():
    ...
    def enter_bootloader(self, timeout=1):
    ...
    def upload_firmware(self, fw_file, timeout=1):
        retries = 3
        ...
        # Set device into bootloader mode
        if not self.enter_bootloader(timeout=timeout):
            if self.print_debug:
    ...
    def erase_flash(self, fast_erase=True, pause=0.2):
