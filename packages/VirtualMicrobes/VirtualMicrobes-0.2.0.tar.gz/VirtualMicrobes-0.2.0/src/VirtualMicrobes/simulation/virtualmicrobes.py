...
def start_evo_sim(args):
    ...
    if options.get('mem_limit',None) is not None:
        ...
        soft, hard = resource.getrlimit(rsrc)
        print 'Soft memory limit changed to :', soft
...
def start_lod_analysis(args):
    ...
    with lod.LOD_Analyser(args) as analyser:
        ...
        if args.pop_cells:
            print 'Entering pop cell function'
...
''' % (program_shortdesc, str(__date__))
    ...
    evo_parser.add_argument('--no-overwrite-savefiles', dest='overwrite_savefiles', action='store_false',
                            default=True,
                            help=argparse.SUPPRESS)
    ...
    pop_par.add_argument('--hgt-genomesize-scaling', action='store_true', default=False,
                        help='Donors with many genes are more likely to contribute (as though theres a local gene pool)'
                        ) 
    pop_par.add_argument('--no-hgt-genomesize-scaling', action='store_false', dest='hgt_genomesize_scaling',
                         help=argparse.SUPPRESS
                         )
    pop_par.add_argument('--per-gene-hgt-rate', action='store_true', default=False,
                         help='HGT events are not during life-time (default), but only at birth of new cells'
                         )
    pop_par.add_argument('--no-per-gene-hgt-rate', action='store_false', dest='per_gene_hgt_rate',
                         help=argparse.SUPPRESS
                         )
