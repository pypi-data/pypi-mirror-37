...
class WriteForcing(object):
    ...
    def add_variable(self, name, data, dimensions, attributes=None, format='f4', ncopts={}, stringattr=False):
        """
        ...
        if attributes:
            for attribute in attributes:
                if not stringattr:
                    ...
                    self.nc.setncattr_string(attribute, attributes[attribute])
...
class RegularReader(FileReader):
    ...
    def _load_grid(self, netcdf_filestr):
        ...
        # If we got nav_{lon, lat} but not lon and lat, make copies called lon and lat to simplify our lives later.
        is_nemo = hasattr(self.grid, 'nav_lon') and hasattr(self.grid, 'nav_lat')
        is_cmems = hasattr(self.grid, 'lon') and hasattr(self.grid, 'lat')
        if is_nemo and not is_cmems:
            self.grid.lon = self.grid.nav_lon
            self.grid.lat = self.grid.nav_lat
        elif is_cmems and not is_nemo:
            # Make the grid data the right shape for us to assume it's an FVCOM-style data set. We only want to do this
            # for CMEMS-style files.
        ...
        related_variables = {'lon': ['lon'], 'lat': ['lat']}
