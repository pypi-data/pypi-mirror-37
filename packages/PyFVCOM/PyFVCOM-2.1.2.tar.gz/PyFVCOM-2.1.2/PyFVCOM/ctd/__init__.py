    ...
    def _write_lst(self, filename):
        ...
        for counter, current_file in enumerate(file_names):
            ...
            if 'mm_dd_yyyy' in self.variables.names[counter] and 'hh_mm_ss' in self.variables.names[counter]:
                num_fields -= 2
            # num_samples = self.header['num_records'][counter]
                    ...
                    if line.strip():
                        ...
                        if at_data:
                            ...
                            for name_index, name in enumerate(columns):
                                ...
                                # If we've got names which are <prefix>:<number>, assume they're multidimensional
                                # data and just concatenate them into one long series. This is suboptimal.
                                multi_dimensional = False
                                if ':' in name:
                                    multi_dimensional = True
                                # Replace illegal characters in names.
                                ...
                                num_dimensions = 1  # assume a single dimension for all variables
                                if not hasattr(self, name):
                                    if multi_dimensional:
                                        # Find how many of this variable there are.
                                        prefix = name.split(':')[0]
                                        num_dimensions = sum([i.startswith(prefix) for i in columns])
                                        setattr(self, clean_name, np.zeros(header['num_records'] * num_dimensions))
                                    else:
                                        setattr(self, clean_name, np.zeros(header['num_records']))
                                data_index = (int(line_list[0].replace(')', '')) * num_dimensions) - 1
                                if any(c.isalpha() for c in line_list[name_index + 1]):
                                    # Assume any letters in the data means it's been flagged in QC. Set the value to
                                    # NaN here instead.
                                    current_value = np.nan
                                else:
                                    # Nice. Some data have '>' in them.
                                    current_value = float(line_list[name_index + 1].replace('>', ''))
                                getattr(self, clean_name)[data_index] = current_value
