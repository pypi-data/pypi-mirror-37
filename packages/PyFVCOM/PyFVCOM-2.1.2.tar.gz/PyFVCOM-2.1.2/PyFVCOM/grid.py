...
class ElementDistanceGraph(Graph):
    """
    Supporter class for refining paths
    ...
    def __init__(self, domain):
        ...
        Create a graph of an FVCOM domain which will work on element centres. This is handy for finding paths through
        the grid at element centres rather than nodes.

        domain : PyFVCOM.read.FileReader
            The model domain.

        """
        super(ReducedFVCOMdist, self).__init__()

        if not hasattr(domain.grid, 'nbe'):
            # We need the element connectivity, so calculate it here. Might as well save all the other bits whilst
            # we're at it.
            domain.grid.ntve, domain.grid.nbve, domain.grid.nbe, domain.grid.isbce, domain.grid.isonb = grid_metrics(domain.grid.triangles)

        for this_cell in range(domain.dims.nele):
            self.add_node(this_cell)

        for this_cell, this_nbe in enumerate(domain.grid.nbe):
            for i in range(2):
                this_dist = np.hypot(domain.grid.x[this_cell] - domain.grid.x[domain.grid.nbe[i]],
                                     domain.grid.y[this_cell] - domain.grid.y[domain.grid.nbe[i]])
                self.add_edge(this_cell, domain.grid.nbe[i], this_dist)
                self.add_edge(domain.grid.nbe[i], this_cell, this_dist)
