...
class TFModel:
    def __init__(self, model_tensors, checkpoint_dir=None, is_notebook=False):
        ...
        if self._is_notebook:
            self._progress_range = tnrange
        else:
            self._progress_range = trange
    ...
    def train(self, train_op, gntr, num_epochs,
        ...
        with tf.Session(config=sess_config_proto) as sess:
            ...
            for _ in range(num_epochs):
                ...
                for _ in self._progress_range(steps_per_epoch, desc=progress_desc):
                ...
                if listeners:
                    for l in listeners:
                        l.run(sess, global_step_value, self._is_notebook)
    ...
    def train_stepwise(self, train_op, gntr, num_steps, steps_per_checkpoint,
        ...
        with tf.Session(config=sess_config_proto) as sess:
            ...
            for _ in range(num_checkpoints):
                progress_desc = f'Checkpoint {id_checkpoints+1}'
                for _ in self._progress_range(steps_per_checkpoint, desc=progress_desc):
                    xb, yb = sess.run(data_gntr)
                    fd = {self._inputs: xb, self._labels: yb, self._is_training: True}
                    if global_step_value == 0:
                        sess.run(update_ops, feed_dict=fd)
                ...
                if listeners:
                    for l in listeners:
                        l.run_stepwise(sess, global_step_value, self._is_notebook)
