...
class Listener:
    ...
    def run(self, session, global_step_value, is_notebook=False):
        ...
        if is_notebook:
            progress_range = tnrange
        progress_desc = f'{self._name} evaluation {epoch}'
        for _ in progress_range(self._steps_per_epoch, desc=progress_desc):
    ...
    def run_stepwise(self, session, global_step_value, is_notebook=False):
        ...
        if is_notebook:
            progress_range = tnrange
        progress_desc = f'{self._name} evaluation {id_checkpoints}'
        metrics_values = []
        for _ in progress_range(steps_per_checkpoint, desc=progress_desc):
            session.run(self._reset_ops)
            xb, yb = session.run(self._data_gn)
            fd = {self._inputs: xb, self._labels: yb, self._is_training: False}
            session.run(self._update_ops, feed_dict=fd)
