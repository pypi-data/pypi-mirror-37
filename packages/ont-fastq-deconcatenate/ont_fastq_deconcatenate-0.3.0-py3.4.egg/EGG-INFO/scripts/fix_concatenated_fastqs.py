#!/builds/ofan/ont_fastq_deconcatenate/venv/bin/python
"""
...
def find_and_fix_fastqs(directory, quarantine_directory, recursive=True):
    ...
    logger.info('Files will be quarantined to directory {} if they cannot '
                'be fixed.'.format(quarantine_directory))
    ...
    for index, filename in enumerate(files):
        if fastq_fix.correct_concatenation_errors(filename):
            bad_fastqs += 1
        if fastq_fix.quarantine_if_required(filename, quarantine_directory):
            quarantined_fastqs +=1
        pbar.update(index)
    pbar.finish()
    logger.info('Attempted to fix {} fastq files.'.format(bad_fastqs))
    logger.info('Quarantined {} fastq files.'.format(quarantined_fastqs))
...
if __name__ == '__main__':
    ...
    argparser.add_argument('-q', '--quarantine_dir', required=True, 
                           help='Folder to place quarantined files in')
    argparser.add_argument('-r', '--recursive', default=True,
                           action='store_true', help='Input folder')
    
    args = argparser.parse_args()
    find_and_fix_fastqs(args.input, args.quarantine_dir, args.recursive)
