	...
	if instrument.close[1] > bollinger10.top[1]: # If the price rises above the Bollinger top line...
		if _long_position is None: # ... and if there is no opened long position yet...
			...
			if _long_position is not None:
				_alg.setSLTP( loss=atr10[1]*2, profit=atr10[1]*2 ) # Setting stop loss and take profit
				_alg.onPositionClose( _long_position, onLongPositionClose ) # Specifying the function to be called when the position is being closed 
		...
		if _short_position is None: # ... and if there is no opened short position yet...
			...
			if _short_position is not None:
				_alg.setSLTP( loss=atr10[1]*2, profit=atr10[1]*2 ) # Setting stop loss and take profit
				_alg.onPositionClose( _short_position, onShortPositionClose ) # Specifying the function to be called when the position is being closed 
