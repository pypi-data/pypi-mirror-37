...
class FFT(object):
    ...
    def describe_decision(self, t_id, level, metrics, reversed=False):
        ...
        if level == 0:
            ...
            description = ("\t| " * (level + 1) +
                       " ".join(["if attribute(", str(cue), ")", direction, str(threshold)]) +
                       "\t then class--> " + results[1 - decision if reversed else decision]).ljust(30, " ")
        elif level < self.max_depth - 1:
            direction = mappings[direction] if reversed else direction+" "
            description = ("\t| " * (level + 1) +
                       " ".join(["elif attribute(", str(cue), ")", direction, str(threshold)]) +
                       "\t then class--> " + results[1 - decision if reversed else decision]).ljust(30, " ")
        elif level == self.max_depth - 1:
            direction = mappings[direction] if reversed else direction+" "
            description = ("\t| " * (level + 1) +
            ...
            description = description + '\n' + ("\t| " * (level + 2) +
                       " ".join(["else"]) +
                       "\t class--> " + results[1 - decision if reversed else decision]).ljust(30, " ")
