...
def classification_reports(y_train, x_train, y_test, x_test, arpu_segment_train, arpu_segment_test, model):
	...
	arpu_unique = arpu_segment_train.append(arpu_segment_test).unique()
	arpu_unique.sort()
...
def report_with_thresholds(fact_train, pred_train, arpu_segment_train, fact_test, pred_test, arpu_segment_test):
	...
	known_arpu = dict()
	known_arpu[1] = 'Extra Low'
	known_arpu[2] = 'General Low'
	known_arpu[3] = 'Medium'
	known_arpu[4] = 'General High'
	known_arpu[5] = 'Extra High'
	...
	arpu = []
	for i in arpu_unique:
		arpu_str = "Unknown Arpu " + str(i)
		
		if i in known_arpu:
			arpu_str = known_arpu[i]
		
		arpu = arpu + [arpu_str]
	
	res = pd.DataFrame({'segment_ARPU': np.repeat(arpu, 4),
						'top': [10, 20, 30, 40] * len(arpu),
						'precision_train': .0,
						'recall_train': .0,
						'precision_test': .0,
						'recall_test': .0,
	...
	for i in np.arange(1, len(arpu_unique) + 1):
		f_test = df_test.loc[df_test.arpu_segment == arpu_unique[i - 1]].fact
		p_test = df_test.loc[df_test.arpu_segment == arpu_unique[i - 1]].pred
		f_train = df_train.loc[df_train.arpu_segment == arpu_unique[i - 1]].fact
		p_train = df_train.loc[df_train.arpu_segment == arpu_unique[i - 1]].pred
...
def reports_with_thresholds(y_train, x_train, y_test, x_test, arpu_segment_train, arpu_segment_test, model):
	...
	pred_train = pd.DataFrame(model.predict_proba(x_train), columns=model.classes_)
	pred_test = pd.DataFrame(model.predict_proba(x_test), columns=model.classes_)
...
def auc_report(class_reports, levels):
	...
	arpu_unique = list(class_reports.keys())
	...
	report = pd.DataFrame({'segment_ARPU': arpu,
						   'AUC_train, avg': .0,
						   'AUC_test, avg':.0})
	
	for l in levels:
		report = pd.concat([report.reset_index(drop=True),
							pd.DataFrame({'AUC_train, class ' + str(l) : [0.0] * len(arpu),
										  'AUC_test, class ' + str(l) : [0.0] * len(arpu)})],
						   axis = 1)

	for i in np.arange(1, len(arpu_unique) + 1):
		report.at[i - 1, 'AUC_train, avg'] = class_reports[arpu_unique[i - 1]]["train"].AUC[len(levels)]
		report.at[i - 1, 'AUC_test, avg'] = class_reports[arpu_unique[i - 1]]["test"].AUC[len(levels)]
		
		for l in levels:
			report.at[i - 1, 'AUC_train, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["train"].AUC[l]
			report.at[i - 1, 'AUC_test, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["test"].AUC[l]
...
def precision_report(class_reports, levels):
	...
	for l in levels:
		report = pd.concat([report.reset_index(drop=True),
							pd.DataFrame({'Precision_train, class ' + str(l) : [0.0] * len(arpu),
										  'Precision_test, class ' + str(l) : [0.0] * len(arpu)})],
						   axis = 1)

	for i in np.arange(1, len(arpu_unique) + 1):
		report.at[i - 1, 'Precision_train, avg'] = class_reports[arpu_unique[i - 1]]["train"].precision[len(levels)]
		report.at[i - 1, 'Precision_test, avg'] = class_reports[arpu_unique[i - 1]]["test"].precision[len(levels)]
		
		for l in levels:
			report.at[i - 1, 'Precision_train, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["train"].precision[l]
			report.at[i - 1, 'Precision_test, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["test"].precision[l]
...
def recall_report(class_reports, levels):
	...
	for l in levels:
		report = pd.concat([report.reset_index(drop=True),
							pd.DataFrame({'Recall_train, class ' + str(l) : [0.0] * len(arpu),
										  'Recall_test, class ' + str(l) : [0.0] * len(arpu)})],
						   axis = 1)

	for i in np.arange(1, len(arpu_unique) + 1):
		report.at[i - 1, 'Recall_train, avg'] = class_reports[arpu_unique[i - 1]]["train"].recall[len(levels)]
		report.at[i - 1, 'Recall_test, avg'] = class_reports[arpu_unique[i - 1]]["test"].recall[len(levels)]
		
		for l in levels:
			report.at[i - 1, 'Recall_train, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["train"].recall[l]
			report.at[i - 1, 'Recall_test, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["test"].recall[l]
...
def f1_report(class_reports, levels):
	...
	for l in levels:
		report = pd.concat([report.reset_index(drop=True),
							pd.DataFrame({'f1_train, class ' + str(l) : [0.0] * len(arpu),
										  'f1_test, class ' + str(l) : [0.0] * len(arpu)})],
						   axis = 1)

	for i in np.arange(1, len(arpu_unique) + 1):
		report.at[i - 1, 'f1_train, avg'] = class_reports[arpu_unique[i - 1]]["train"]['f1-score'][len(levels)]
		report.at[i - 1, 'f1_test, avg'] = class_reports[arpu_unique[i - 1]]["test"]['f1-score'][len(levels)]
		
		for l in levels:
			report.at[i - 1, 'f1_train, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["train"]['f1-score'][l]
			report.at[i - 1, 'f1_test, class ' + str(l)] = class_reports[arpu_unique[i - 1]]["test"]['f1-score'][l]
...
def percentage_report(y_train, y_test, arpu_segment_train, arpu_segment_test):
	...
	for l in levels:
		report = pd.concat([report.reset_index(drop=True), 
							pd.DataFrame({'%_train, class ' + str(l) : [0.0] * len(arpu),
										  '%_test, class ' + str(l) : [0.0] * len(arpu)})],
						   axis = 1)

	for i in np.arange(1, len(arpu_unique) + 1):
		y_train_i = y_train[arpu_segment_train.index[arpu_segment_train == arpu_unique[i - 1]]]
		y_test_i = y_test[arpu_segment_test.index[arpu_segment_test == arpu_unique[i - 1]]]
