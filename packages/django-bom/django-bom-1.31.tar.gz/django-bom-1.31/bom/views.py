...
def manufacturer_part_delete(request, manufacturer_part_id):
    ...
    return HttpResponseRedirect(reverse('bom:part-info', kwargs={'part_id': part.id}) + '#sourcing')
...
def sellerpart_edit(request, sellerpart_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('bom:part-info', kwargs={'part_id': sellerpart.manufacturer_part.part.id}) + '#sourcing')
    else:
