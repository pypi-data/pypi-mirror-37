...
class PolarView(_View, session.Style,
    ...
    @property
    def extents(self):
        """`tuple`: View extents in grid units of transformed ``(x, y)``.
        ...
        * x2, y2: Lower right corner of the extents.
        ...
        Set the view of the polar plot to view the full extents of the plot area::
            ...
            90.0
        """
        return RectTuple(self._get_style(sv.EXTENTS, sv.X1),
...
class ReadOnlyViewport(session.Style):
    ...
    @property
    def bottom(self):
        """`float` in percentage of frame height from the bottom of the: (`float`) Bottom position of viewport relative to the `Frame`.
            frame.
    ...
    @property
    def left(self):
        """`float` in percentage of frame width from the left of the frame.: (`float`) Left position of viewport relative to the `Frame`.
    ...
    @property
    def right(self):
        """`float` in percentage of frame width from the left of the frame.: (`float`) Right position of viewport relative to the `Frame`.
    ...
    @property
    def top(self):
        """`float` in percentage of frame height from the bottom of the: (`float`) Top position of viewport relative to the `Frame`.
            frame.
