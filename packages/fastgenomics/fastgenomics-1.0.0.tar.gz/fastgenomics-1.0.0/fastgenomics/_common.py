...
def assert_manifest_is_valid(config: dict):
    ...
    err_msg = lambda x, y: "'{}'-type not supported for {}-operations.".format(x, y)
...
def get_app_manifest() -> dict:
    ...
    try:
        ...
        err_msg = (f"App manifest {manifest_file} is not compatible with fastgenomics_py. "
                   "Check the docs if your parameter- or I/O-types are supported.")
        raise RuntimeError(err_msg)
    return _MANIFEST
