...
class db():
    ...
    def __obtenerIpMongo(self):
        ...
        if self.__validarVariableEntorno("MQ"):
            print("MODO : sin registrar en mongo la configuracion")
            try:
                miSecreto = os.environ["MONGO_URI"]
            except:
                miSecreto = ""
        else:
            try:
...
class microservicio(db):
    ...
    def __init__(self,app = "",servicio = ""):
        ...
        #Verificar si existe secrets
        miMQSecret = ""
        try:
            miMQSecret = os.environ["MQ"]
        except:
            try:
                with open("/run/secrets/MQ", 'r') as secret_file:
                    miMQSecret = secret_file.read()
                    miMQSecret = miSecreto.rstrip('\n')
                    os.environ["MQ"] = miMQSecret 
            except:
                print("No se encontro variable de entorno (MQ) ó Secrets Docker (MQ)")

        # ? Validar si existe solo MQ no se validara en bd configuracion
        if self.__validarVariableEntorno("MQ"):
            print("MODO : sin registrar en mongo la configuracion")
        else:
    ...
    def __functionBifurcacion(self,configService,jsonArguments):
        ...
        try:
            log(Fore.GREEN+"[ "+ID_TRANSACCION + " ] - input - ["+metadata["owner"]+"] \n"+Fore.WHITE+"Headers : "+Fore.GREEN+ str(headers) +Fore.WHITE+"\nData : "+Fore.GREEN+ str(data) +"\n"+Fore.WHITE+"Response : "+Fore.GREEN+ str(response)+Fore.WHITE)
    ...
    def escribir(self,topico,respuesta,idTransaction):
        ...
        if topico[:9] == "respuesta":
            print(Fore.YELLOW+"[ "+idTransaction+" ] -  output - ["+topico+"]")
            print(json.dumps(respuesta["response"],sort_keys=True,indent=4))
            print(" "+Fore.WHITE)
        else:
            print(Fore.RED+"[ "+idTransaction+" ] -  output - ["+topico+"]\n"+Fore.WHITE+"Headers : "+Fore.RED+str(respuesta["headers"])+Fore.WHITE+"\nData : "+Fore.RED+str(respuesta["data"]) +Fore.WHITE)
    ...
    def __streamMessageQueue(self,function):
        ...
        if self.__validarVariableEntorno("MQ"):
            ...
            mon = ""
            try:
                mon = os.environ[str("MONGO_URI")]
            except:
                mon = ""
            self.__inicializaciones["configuraciones"]["kafka_servers"] = var2
