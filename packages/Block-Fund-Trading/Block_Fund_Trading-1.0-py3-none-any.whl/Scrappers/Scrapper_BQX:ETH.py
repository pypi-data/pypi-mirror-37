...
def order_book_dataframe_sqlite(sym, num_entries=20):
    ...
    df_asks = pd.DataFrame(dict(update_time = server_time, askprices = s1, askvolumes = s2)).reset_index()
    ...
    try:
        ...
        df.to_sql('BQX/ETH_order_book' , con , if_exists = 'replace', index = True,)
        df_asks.to_sql("BQX/ETH_ask_book", con , if_exists = 'replace', index = True,)
        df_bids.to_sql("BQX/ETH_bid_book", con , if_exists = 'replace', index = True,)
        df_spread.to_sql("BQX/ETH_spread", con, if_exists = 'append', index = False)
        cur = con.cursor()
