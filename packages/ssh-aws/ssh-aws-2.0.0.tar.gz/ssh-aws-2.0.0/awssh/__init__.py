...
def display_instances(instances):
    table_col_names = ['Name', 'Instance ID', 'Public IP',
                       'Private IP', 'Zone', 'Key Name', 'State']
    print("     {:<30}{:<21}{:<17}{:<17}{:<14}{:<20}{}".format(*table_col_names))

    details_fmt = "{:2} - {name:<30}{id:<21}{public_ip_address:<17}" \
                  "{private_ip_address:<17}{availability_zone:<14}{key_name:<20}({state})"
        ...
        if instances[server_selection].public_ip_address is None:
            for instance in instances:
                if instance.key_name == instances[server_selection].key_name and instance.public_ip_address is not None:
                    ...
                    jump_connect(instances[server_selection], instance, args)
        else:
