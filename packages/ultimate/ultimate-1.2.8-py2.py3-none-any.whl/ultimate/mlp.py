...
class MLP(Mesh):
        ...
        for epoch in range(epoch_train):
            ...
            for idx in range(SIZE):
                ...
                loss_ = self.train_one(
                    in_arr[idx_], target_arr[idx_], self.rate, loss_range, importance_out)
    ...
    def predict(self, in_arr, out_arr=None, verbose=0, iteration_log=100):
        if out_arr is None:
            out_arr = numpy.zeros(
                (len(in_arr), self.layer_size[-1]), dtype=_DTYPE)
    ...
    def train_one(self, in_buf, target_buf, rate, loss_range, importance_out):
