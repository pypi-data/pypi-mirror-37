...
def make_request(url, data=None, timeout=60, api=True, log=True):
    """
    ...
    try:
        ...
        if data:
            r = session.post(url, timeout=timeout, data=data)
        else:
        ...
        if api:
            return -1, {'url': url, 'status_code': -1,
        ...
        if api:
            try:
                ...
                r_json.update(dict(url=url, status_code=r.status_code))
