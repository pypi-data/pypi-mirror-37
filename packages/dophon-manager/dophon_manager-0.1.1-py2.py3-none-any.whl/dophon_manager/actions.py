...
class ActionObj:
    ...
    def new_action(
        ...
        # 利用poetry创建项目
        print('create new project,name:', project_name)
        ...
        while True:
            if not os.path.exists(project_path):
                ...
                time.sleep(1)
            # 询问必要参数
            entity_point = input('project boot file (default:dophon_project):\n')
    ...
    def project_install(self, project_path):
        ...
        Thread(target=self.listen_popen, args=(install_process,)).start()

    def listen_popen(self, popen):
        before_lines = []
        while int(popen.poll()) != 0:
