...
class ActionObj:
    ...
    def new_action(
        ...
        # 利用poetry创建项目
        print('create new project,name (default:dophon_project):', project_name)
    ...
    def init_project_file(self, project_path, entity_point):
        entity_prefix = entity_point.split(':')[0].split('.')
        entity_file = entity_prefix.pop() + '.py'
        entity_file_path = project_path + os.sep + os.sep.join(entity_prefix)
