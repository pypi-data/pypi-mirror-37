...
def bayesian_ols(formula, table, metadata, tree=None,
    ...
    -------
    BayesianOLS
        BayesianOLS model object.
    """
    table = load_table(table_file)
    # TODO: Need to add checks for samples
    table, tree = sparse_match_tips(table, tree)
    def filter_ids(id_, val_, md_):
        return id_ in metadata.index
    def filter_empty(id_, val_, md_):
        return (np.var(val_) > 0) and np.sum(val_) > 0
    ...
    sort_f = lambda x: list(G_data.index)
...
class BayesianOLS():
    ...
    def fit(self, Y, X, basis=None):
        ...
        N, D = Y.shape
        if basis is None:
            basis = coo_matrix(_gram_schmidt_basis(D))

        p = X.shape[1]   # number of covariates

        # dummy variables for mini-batch size
        batch_idx = tf.placeholder(tf.int32, shape=[batch_size, 2],
                                   name='batch_idx')
        ...
        # via minibatch SGD
        Y_ph = tf.placeholder(tf.float32, shape=[self.batch_size],
                              name='Y_placeholder')
        ...
        for i in range(self.inference.n_iter):
            # get batches
            idx, idx_data = minibatch(M=batch_size, Y=data, p=batch_proportion)
            info_dict = inference.update(
                feed_dict={batch_idx: idx, Y_ph: idx_data})
