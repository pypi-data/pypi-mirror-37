...
class Export2xls(object):
    ...
    def __init__(self, out_file, filenames, database):
        ...
        if self.db.dbtype == "sqlite3":
            self.write_experiment_sheet()
            self.write_spark_sheet()
            self.write_stage_sheet()
        else:
            print("Export is not supported for this type of the database: " + self.db.dbtype)
    ...
    def write_spark_sheet(self):
        table = self.db.table(database_table_spark_extended)
        sheet = self.workbook.add_worksheet(database_table_spark)
        show_keys = list(spark_Parameters.keys()) + list(spark_Results.keys())

        # Prepare header
        sheet.write(0, 0, 'sqlite key:', self.bold_cell_format)
        ...
        row = 2
        for experiment in self.experiment_records:
            c = list(self.db.query("SELECT " + (', ').join(['s.%s as %s' %(x,x) for x in show_keys+['roi_id']]) + " FROM "
                                   + table + " s INNER JOIN roi r ON" +
    ...
    def write_stage_sheet(self):
        ...
        show_keys = list(stage_Parameters.keys()) + list(stage_Results.keys())
