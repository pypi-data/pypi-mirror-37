...
BINPATH = {
    "Windows": "sc2_X_x64.exe",
    "Darwin": "sc2_X.app/Contents/MacOS/sc2_X",
    "Linux": "sc2_X_x64"
}
...
def get_env():
    # TODO: Linux env conf from: https://github.com/deepmind/pysc2_X/blob/master/pysc2_X/run_configs/platforms.py
    return None
...
def latest_executeble(versions_dir):
    ...
    version, path = latest
    if version < 55958:
        logger.critical(f"Your sc2_X binary is too old. Upgrade to 3.16.1 or newer.")
        exit(1)
    return path / BINPATH[PF]


class _MetaPaths(type):
    """"Lazily loads paths to allow importing the library even if sc2_X isn't installed."""
    def __setup(self):
        ...
        try:
            ...
            logger.critical(f"sc2_X installation not found: File '{e.filename}' does not exist.")
            exit(1)
...
class Paths(metaclass=_MetaPaths):
    """Paths for sc2_X folders, lazily loaded using the above metaclass."""
