...
from helper import image_picker, get_nuclei_array
...
class CTCDataset(DistributionPolicyDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cache is not None and uid in self.cache:
            ...
            fp = image_picker(self.root, uid, mode='CTC')
            ...
            label_gt = get_nuclei_array(csv_path, json_path, (h, w))
...
class Compose():
    ...
    def __call__(self, sample):
        image, label, label_gt = sample['image'], sample['label'], sample['label_gt']
        ...
        if self.augment:
            ...
            # perform RandomCrop()
            # TODO:
            #  (1) if no targets, randomcrop, 
            #  (2) if has target(s), random pick one up & crop its surroundings
            i, j, h, w = transforms.RandomCrop.get_params(image, self.size)
            image, label, label_gt = [tx.crop(x, i, j, h, w) for x in (image, label, label_gt)]
            print(f'has targets? {np.any(np.asarray(label) > 0)}')
            ...
            if random.random() > 0.5:
                image, label, label_gt = [tx.hflip(x) for x in (image, label, label_gt)]

            # perform RandomVerticalFlip()
            if random.random() > 0.5:
                image, label, label_gt = [tx.vflip(x) for x in (image, label, label_gt)]

            # perform Random Rotation (0, 90, 180, and 270 degrees)
            random_degree = random.randint(0, 3) * 90
            image, label, label_gt = [tx.rotate(x, random_degree) for x in (image, label, label_gt)]
        ...
        # perform ToTensor()
        if self.tensor:
            image, label, abel_gt = [tx.to_tensor(x) for x in (image, label, label_gt)]
            # perform Normalize()
            image = tx.normalize(image, self.mean, self.std)

        # prepare a shadow copy of composed data to avoid screwup cached data
        x = sample.copy()
        x['image'], x['label'], x['label_gt'] = image, label, label_gt
        return x
...
if __name__ == '__main__':
    ...
    train = NucleiDataset('/Users/winston/Datasets/Inference/data/20181017_predict/2018-01-30/')
