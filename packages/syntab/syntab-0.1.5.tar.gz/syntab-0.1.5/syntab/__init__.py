    ...
    def syn(self,source_dbname,targe_dbname,tab_name = None):
        ...
        if (tab_name == None):
            ...
            for row in cur.fetchall():
                self.syn_tab(db1,source_dbname,db2,targe_dbname,row[0],diff_sql)
            cur.close()
        else:
            self.syn_tab(db1,source_dbname,db2,targe_dbname,db_tbname,diff_sql)
        
        db1.close()
        db2.close()
        return diff_sql

    def syn_tab(self,db1,db1_name,db2,db2_name,tab_name,diff_sql):
        ...
        if (t[0] == 0):
            ...
            for k,val1 in dct1.items():
                if (dct2.get(k,False) == False):#目标表没有对应字段要添加新字段
