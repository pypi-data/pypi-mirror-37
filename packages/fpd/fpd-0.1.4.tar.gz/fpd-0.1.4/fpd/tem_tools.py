...
def blob_log_detect(image, min_radius, max_radius, num_radii, threshold, overlap=0.3, log_scale=True, plot=False):
    '''
    Detect blobs in an image using the Laplacian of Gaussian.
    ...
    Notes
    -----
    This is a wrapper of skimage.feature.blob_log.
...
def synthetic_lattice(cyx, ab, angles, reps=None, shape=None, max_r=None, plot=False, max_reps=21):
    ...
    if d90 != 0:
        ...
        trans = (trans_rot + trans_shr) + trans_rot.inverse
        trans = trans + AffineTransform(rotation=-alpha)
...
def lattice_magnitudes(rt, angles, window_deg=5, bin_pix=1.0, hist_gaus=2.0, min_vector_mag=None, max_vector_mag=None, mode='peaks', peak_min_distance=None, plot=False):
    ...
    for a in angles:
        ...
        # form histogram
        hmax = rb.max() // bin_pix
        hmax = bin_pix * int(hmax+1)
        bins = int(hmax/bin_pix)
        
        hist, bin_edges = np.histogram(rb, bins=bins, range=(0, hmax))
        ...
        if plot:
            ...
            ax1.set_ylabel('vector magnetude (pix')
            plt.sca(ax1)
            plt.minorticks_on()
...
def optimise_lattice(yx, cyx, ab, angles, shape, constraints=None, plot=False):
    '''
    ...
    res = minimize(fun, x0, args=(yx, shape), tol=1e-3, constraints=constraints)
    x_opt = res.x
