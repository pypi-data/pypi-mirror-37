...
class VirtualAnnularImages(object):
    def __init__(self, data, nr=16, nc=16, cyx=None, crop_r=None, rebin=None,
                parallel=True, ncores=None, nrnc_are_chunks=False,
                print_stats=True,
                mask=None, spf=1):
        '''
        ...
        if isinstance(data, dict):
            ...
            self._calc_rdf(data, nr, nc, cyx, crop_r, mask, spf, rebin, parallel, ncores,
    ...
    def _calc_rdf(self, data, nr, nc, cyx, crop_r, mask, spf, rebin, parallel, ncores,
                  nrnc_are_chunks, print_stats):
    ...
    def plot(self, r1=None, r2=None, nav_im=None, norm='log', scroll_step=1, alpha=0.3, cmap=None, pct=0.1, mradpp=None):
        ...
        var = np.std(sls, (1,2)) / np.mean(sls, (1, 2))
        ax_cntrst.plot(rs-dw/2.0, var)
        ax_cntrst.minorticks_on()
        ax_cntrst.set_xlabel('Radius (pixels)')
        ax_cntrst.set_ylabel('Contrast (std/mean)')
