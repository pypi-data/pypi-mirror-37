...
class PipelineClient:
    ...
    def _reportvalidator(self, report):
        """
        Checks whether a job has exited in error, if so it raises exceptions as appropriate
        :param report: report originally returned by run_job
        :return: None
        """
        report_json = getattr(report, "json", None)
        pipeline_logger.debug("Process is in report validator")

        if report_json is None:
            pipeline_logger.debug("No json return from report")
            raise PipelineClientException("Process report returned no json")
        response_json = report_json()
        job_id = response_json['job_id']
        # assume job_id is valid if present as string
