...
import oath
import base64
from vipaccess2.patharg import PathType
from vipaccess2 import provision as vp
...
def show_master_token(display=None):
    with open( os.path.expanduser('~/.vipaccess'), "r") as dotfile:
        d = dict( l.strip().split(None, 1) for l in dotfile )
        if 'id' in d:
            if None == display:
                return d['id']
