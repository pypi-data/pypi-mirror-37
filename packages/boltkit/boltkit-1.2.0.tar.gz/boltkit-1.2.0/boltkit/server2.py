...
class BoltMessageHandler(object):

    pass


class Forwarder(BoltMessageHandler):

    pass


class AbstractBoltServer(Thread):

    running = False

    def __init__(self, bind_address):
        super(AbstractBoltServer, self).__init__()
        self.bind_socket = socket()
        self.bind_socket.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
        self.bind_socket.bind(bind_address)
        self.bind_socket.listen(0)
    ...
    def run(self):
        self.running = True
        while self.running:
            s, client_address = self.bind_socket.accept()
            server_address = s.getsockname()
            print("C: <CONNECT> {} -> {}".format(client_address, server_address))
            self.handshake(s)

    def handshake(self, s):
        magic = s.recv(4)
        print("C: <BOLT> {}".format(h(magic)))
        versions = s.recv(16)
        print("C: <HANDSHAKE> {}".format(h(versions)))
...
def run():
    server = AbstractBoltServer(("localhost", 7687))
    server.start()
