...
from .forms import CoinTypeForm, UserCoinRecordForm, OtherCoinCreateForm
...
from .exports import get_user_coin, get_user_class_name
...
class OtherRecordCreate(CreateView):
    ...
    def get_class_users(self, user_class_id):
        try:
            from bee_django_user.models import UserClass
            other = UserClass.objects.get(id=user_class_id)
            return other.get_students()
        except:
            return None
    ...
    def get_context_data(self, **kwargs):
        ...
        # 班级剩余m币
        if coin_identity == "user_class":
            user_list = self.get_class_users(coin_content_id)
