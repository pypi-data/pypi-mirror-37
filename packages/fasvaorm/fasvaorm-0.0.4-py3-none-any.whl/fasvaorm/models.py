...
from sqlalchemy import Column, Integer, String, ForeignKey, BigInteger, DateTime, Text, FetchedValue, Index, Table, \
...
class ConfigurationSensorMapping(Base):
    ...
    idconfiguration = Column(ForeignKey('configuration.idconfiguration'), nullable=False, index=True)
...
class Measurement(Base):
    ...
    idvehicle = Column(ForeignKey('vehicle.idvehicle'), nullable=False, index=True)
...
class RecordTransferredMapping(Base):
    ...
    idrecord = Column(ForeignKey('record.idrecord'), nullable=False, index=True)
    idtransferred = Column(ForeignKey('transferred.idtransferred'), nullable=False, index=True)
...
class Signal(Base):
    ...
    idvaluetype = Column(ForeignKey('valuetype.idvaluetype'), nullable=False, index=True)
...
class SignalsignalgroupMapping(Base):
    ...
    idsignal = Column(ForeignKey('signal.idsignal'), nullable=False, index=True)
    idsignalgroup = Column(ForeignKey('signalgroup.idsignalgroup'), nullable=False, index=True)
...
class ProcessedDrive(Base, Serializer):
    ...
    idavailable_drive = Column(ForeignKey('available_drive.idavailable_drive'), nullable=False, index=True)

    # id of the module
    idmodule = Column(ForeignKey('module.idmodule'), nullable=False, index=True)
...
class FailedDrive(Base, Serializer):
    ...
    drive = relationship('AvailableDrive',
                         ...
                         backref='failed_drive')

    # and the module
    module = relationship('Module', primaryjoin='FailedDrive.idmodule == Module.idmodule', backref='failed_drive')
...
class SignalAggregatedBaseModel(object):
    ...
    def idaggregation(cls):
        return Column(BigInteger, ForeignKey(Aggregation.idaggregation), nullable=False)
...
class SignalBaseModel(object):
    ...
    @declared_attr
    def idvehicle(cls):
        return Column(ForeignKey('vehicle.idvehicle'), nullable=False, index=True)
