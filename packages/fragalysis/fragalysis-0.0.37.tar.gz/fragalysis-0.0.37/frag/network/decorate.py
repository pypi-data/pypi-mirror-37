...
def del_link_coord(input_mol, get_indices=False, iso_labels=False):
    ...
    for i,x in enumerate(linkers):
        ret_ans = get_atom_coords(x, Chem.MolFromMolBlock(input_mol), get_indices, linkers_iso[i])
        out_d["linkers"][ret_ans[0]] = ret_ans[1:]
    for i,x in enumerate(deletions):
        ret_ans = get_atom_coords(x, Chem.MolFromMolBlock(input_mol), get_indices, deletions_iso[i])
        out_d["deletions"][ret_ans[0]] = ret_ans[1:]
    for i,x in enumerate(ring):
        ret_ans = get_atom_coords(x, Chem.MolFromMolBlock(input_mol), get_indices, ring_iso[i])
