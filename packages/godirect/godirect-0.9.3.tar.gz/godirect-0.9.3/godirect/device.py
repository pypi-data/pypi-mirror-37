...
class GoDirectDevice(ABC):
	...
	CMD_ID_GET_SENSOR_CHANNEL_INFO             = 0x50
	CMD_ID_GET_SENSOR_CHANNELS_AVAILABLE_MASK  = 0x51
	CMD_ID_DISCONNECT                          = 0x54
	CMD_ID_GET_DEVICE_INFO                     = 0x55
	CMD_ID_GET_SENSOR_CHANNELS_DEFAULT_MASK    = 0x56
	...
	def __str__(self):
		...
		"""
		return self._name+" "+self._order_code+self._serial_number+" "+self._type+" "+str(self._rssi)
	...
	def enable_default_sensors(self):
		...
		"""
		availableMask = self._GDX_get_available_channels()
		defaultMask = self._GDX_get_default_channels()
		testMask = 1
		...
		if i == 32:
			return False
		self._logger.info("Autoset channel mask: %i", i)

		for i in sensors:
			if self._GDX_get_channel_info(i):
	...
	def start(self, period=None):
		...
		if not self._GDX_set_measurement_period(self._sample_period_in_milliseconds):
			return False
		if not self._GDX_start_measurements(self._get_channel_mask()):
			return False
		return True
	...
	def read(self, timeout=5000):
		...
		Returns:
			bool: True on success or False on failure
		"""
		if self._GDX_read_measurement(timeout):
			return True
		return False
	...
	def stop(self):
		...
		Returns:
			bool: True on success or False on failure
		"""
		if not self._GDX_stop_measurements(self._get_channel_mask()):
			return False
		return True
	...
	def list_sensors(self):
		...
		"""
		if not self._GDX_get_channel_info_all():
			return False
	...
	def enable_sensors(self, sensors):
		...
		"""
		for i in sensors:
			if self._GDX_get_channel_info(int(i)):
	...
	def set_name_from_advertisement(self, name):
		...
		if len(parts) == 2:
			self._name = parts[0]
			self._order_code = parts[1][0:3]
			self._serial_number = parts[1][3:]

	###########################################################################
	# GDX internal
	###########################################################################

	def _get_sensors_with_mask(self, channel_mask):
		sensors = {}
		mask = 1
		for i in range(0,32):
	...
	def _get_channel_mask(self):
		...
		for i in self._sensors:
			if self._sensors[i].enabled:
				channel_mask += (1 << self._sensors[i].sensor_number)
		return channel_mask
	...
	def _GDX_write_and_check_response(self, buff):
		if not self._GDX_write(buff):
			return False
		response = self._GDX_read_blocking()
		if len(response) < 2:
			return False
		return True
	...
	def _GDX_read_measurement(self, timeout):
		...
		if measurement_type == self.MEASUREMENT_TYPE_NORMAL_REAL32:
			...
			channel_mask = struct.unpack("<H",response[5:7])[0]
			sensors = self._get_sensors_with_mask(channel_mask)
			start = 9
			for i in sensors:
				measurement = struct.unpack("<f",response[start:start+4])[0]
				sensors[i].values.append(measurement)
				start += 4
		elif measurement_type == self.MEASUREMENT_TYPE_WIDE_REAL32:
			self._GDX_dump("WIDE REAL32: ", response[9:])
			channel_mask = struct.unpack("<H",response[5:7])[0]
			sensors = self._get_sensors_with_mask(channel_mask)
			start = 11
			for i in sensors:
				...
				sensors[i].values.append(measurement)
				start += 4
		elif measurement_type == self.MEASUREMENT_TYPE_APERIODIC_REAL32:
			self._GDX_dump("APERIODIC REAL32: ", response[8:])
			channel = struct.unpack("<H",response[6:8])[0]
			measurement = struct.unpack("<f",response[8:13])[0]
			sensors[channel].values.append(measurement)
		elif measurement_type == self.MEASUREMENT_TYPE_SINGLE_CHANNEL_INT32:
			self._GDX_dump("INT32: ", response[8:])
			channel = struct.unpack("<H",response[6:8])[0]
			measurement = struct.unpack("<i",response[8:])[0]
			sensors[channel].values.append(measurement)
		elif measurement_type == self.MEASUREMENT_TYPE_APERIODIC_INT32:
			self._GDX_dump("APERIODIC INT32: ", response[8:])
			channel = struct.unpack("<H",response[6:8])[0]
			measurement = struct.unpack("<i",response[8:])[0]
			sensors[channel].values.append(measurement)
		else:
	...
	def _GDX_get_channel_info_all(self):
		availableMask = self._GDX_get_available_channels()
		testMask = 1
		...
		for i in range(0,32):
			if testMask & availableMask:
				if not self._GDX_get_channel_info(i):
					return False
			testMask = testMask << 1;
		return True
	...
	def _GDX_get_available_channels(self):
		command = [0x58, 0x00, 0x00, 0x00, self.CMD_ID_GET_SENSOR_CHANNELS_AVAILABLE_MASK]
		...
		if not self._GDX_write(command):
			return False
		response = self._GDX_read_blocking(5000)
		if len(response) < 2:
			return False

		# Extract the channel mask from the packet
		mask = struct.unpack("<I", response[6:])[0]
		self._channels = str(format(mask,'032b')).count('1')
		self._logger.info("Available channels: %i", self._channels)
		return mask

	def _GDX_get_default_channels(self):
		command = [0x58, 0x00, 0x00, 0x00, self.CMD_ID_GET_SENSOR_CHANNELS_DEFAULT_MASK]
		...
		self._logger.info("Default channel: %i", default)
		return mask

	def _GDX_get_channel_info(self, channel_number):
		command = [0x58, 0x00, 0x00, 0x00, self.CMD_ID_GET_SENSOR_CHANNEL_INFO, 0x00]

		# Specify the channel number parameter
		command[5] = channel_number
		...
		(channelNumber,spare,sensorId,numericMeasType,samplingMode,
		...
		self._logger.info("Sensor[%i] info:",channelNumber)
	...
	def _GDX_start_measurements(self, channel_mask):
		command = [
		...
		command[7]  = (channel_mask >> 0)  & 0xFF
		command[8]  = (channel_mask >> 8)  & 0xFF
		command[9]  = (channel_mask >> 16) & 0xFF
		command[10] = (channel_mask >> 24) & 0xFF
	...
	def _GDX_stop_measurements(self, channel_mask):
		command = [
