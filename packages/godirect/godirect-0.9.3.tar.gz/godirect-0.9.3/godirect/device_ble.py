...
class GoDirectDeviceBLE(GoDirectDevice):
	...
	def _read(self, timeout):
		...
		while not self._response_read:
			timeoutCounter += 1000
			if timeoutCounter > timeout:
				self._logger.error("ERROR: BLE read timeout!")
				self._clear_response()
				return bytearray()
			time.sleep(0.25)

			str = "BLE READ: <<<"
			str += " ".join( [ "%02X " % c for c in self._response_buffer ] ).strip()
			self._logger.debug(str)
		tmp = self._response_buffer
		self._clear_response()
		return tmp
	...
	def _notify_callback(self, handle, value):
		...
		if self._response_read == True:
			self._logger.debug("Already have a packet waiting for pickup, clearing")
			self._response_read = False
			self._response_buffer = bytearray()
		self._response_buffer += value

		# Check if we have received the complete packet
		self._logger.debug("Recieve buffer length: %i Expected packet length: %i",self._response_length(),int(self._response_buffer[1]))

		if self._response_length() >= 1 and self._response_length() == int(self._response_buffer[1]):
