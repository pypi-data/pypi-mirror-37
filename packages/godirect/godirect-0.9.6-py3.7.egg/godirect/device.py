...
class GoDirectDevice(ABC):
	...
	def _GDX_handle_measurement(self, response):
		...
		if measurement_type == self.MEASUREMENT_TYPE_NORMAL_REAL32:
			...
			if value_count > 1:
				self._logger.info("More than one value: %i", value_count)
			...
			sensor_mask = struct.unpack("<H",response[5:9])[0]
			...
			if value_count > 1:
				self._logger.info("More than one value: %i", value_count)
			measurement = struct.unpack("<f",response[8:12])[0]
			self._sensors[sensor_number].values.append(measurement)
