        ...
        html.Div(className='container row', children=[
            ...
            html.Div(className='two columns', children=[
                ...
                dsi.RadioItemRows(
                    id='enhance',
                    className='card dash-styled-input',
                    options=[[{'label': i, 'value': i} for i in ['Origin', 'Enhanced']]],
                    value='Origin'
                ),
                dsi.RadioItemRows(
                    id='lock-zoom',
...
@page.callback(
    Output('graph', 'figure'),
    [Input('tile', 'value'), Input('img-mode', 'value'), 
...
def update_graph(tile, mode, nearby, enhance, ds, uid, zoom, relayout):
    if not ds or not uid or tile is None:
        return {}
    path = os.path.join(ds, uid, tile)
    res = b64img(path, mode, (enhance == 'Enhanced'))
    if not res:
...
@app.route('/image/<mode>/<path:subpath>')
def serve_image(mode, subpath):
    res = rawimg(subpath, mode)
    if not res:
        return
    buff, _, _ = res
    buff.seek(0)
...
@cache.memoize()
def b64img(path, mode, enhance):
    res = rawimg(path, mode, enhance)
    if not res:
        return
    buff, w, h = res
...
def rawimg(path, mode, enhance=False, quality=75):
