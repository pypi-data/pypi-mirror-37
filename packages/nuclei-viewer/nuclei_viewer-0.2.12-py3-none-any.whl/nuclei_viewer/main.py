...
@page.callback(
    ...
    [State('dataset', 'value'), State('uid', 'value'), State('url', 'href'),
     State('lock-zoom', 'value'), State('graph', 'relayoutData')]
)
def update_graph(tile, mode, nearby, ds, uid, href, zoom, relayout):
    ...
    images = [dict(
        ...
        source='{}image/{}/{}'.format(href, mode, path)
    )] if mode else []
    # figure grid lines
    xaxis = dict(
        autorange=False,
...
def serve_image(mode, subpath, quality=75):
    fp = image_picker(dataset_path, subpath, mode)
    if not fp:
        return None
