...
class ModelServer(Model):
    compare_fields = ["name", "tag", "run_id", "model_type", "path", "state", "created_at",
                      "state_changed_at", "urls", "auth_token"]
    ...
    def get_age(self):
        if self.status != "running":
            return "--"
        return prettify_timespan(dateutil.parser.parse(self.updated_at), datetime.now())
...
class RandomHyperParamRequest:
    def __init__(self, values=None, min=None, max=None, scaling=None, type=None):
        self.values = values
        self.min = min
        self.max = max
