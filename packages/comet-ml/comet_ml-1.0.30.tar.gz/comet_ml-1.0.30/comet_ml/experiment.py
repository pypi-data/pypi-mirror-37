...
class BaseExperiment(object):
    ...
    def _get_source_code(self):
        ...
        if self._in_jupyter_environment():
            source = ""
            cells = set()
            lines_of_code = self.config["comet.jupyter.lines_of_code_per_frame"]
            for frame in inspect.stack(
                context=lines_of_code
            ):  ## lines of code to collect
                module = inspect.getmodule(frame[0])
                if module is None:
                    frame_info = inspect.getframeinfo(frame[0], context=lines_of_code)
                    if frame_info.filename in cells:
                        continue
                    lines = frame_info.code_context
                    if lines:
                        cells.add(frame_info.filename)
                        source += (
                            ("## Code from cell %s:\n" % (frame_info.filename,))
                            + ("".join(lines))
                            + "\n"
