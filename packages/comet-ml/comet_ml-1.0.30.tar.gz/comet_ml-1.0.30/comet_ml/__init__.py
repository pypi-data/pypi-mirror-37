...
class Experiment(BaseExperiment):
    ...
    def _on_end(self, wait=True):
        ...
        """
        super(Experiment, self)._on_end(wait=wait)

        if self.ws_connection is not None:
            LOGGER.debug("Waiting for WS connection to close")
            if wait is True:
                timeout = 10
                self.ws_connection.join(timeout)
                if self.ws_connection.is_alive():
                    LOGGER.debug(
                        "WS connection wasn't closed after %d seconds", timeout
                    )
                else:
                    LOGGER.debug("WS connection correctly closed")
