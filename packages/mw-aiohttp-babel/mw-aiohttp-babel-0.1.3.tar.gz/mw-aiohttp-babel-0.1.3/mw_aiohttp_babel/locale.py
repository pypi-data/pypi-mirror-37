...
_default_locale = "zh_TW"
_translations = {}
_supported_locales = frozenset([_default_locale])
_use_gettext = False

def get_best_match_lang(acc_lang):
    langs = [l.split(';q=') for  l in acc_lang.split(',') if l]
    if not langs:
    ...
    for lang in langs:
        if len(lang) == 1:
            ...
            l, q = lang[0], float(lang[1])
        q_lang.append((q, l))
    q_lang.sort(reverse=True)
    return q_lang[0][1]
...
def _default_locale_detector(request):
    ...
    if not _code:
        ...
        locale_code = get_best_match_lang(request.headers.get('ACCEPT-LANGUAGE',''))
        try:
