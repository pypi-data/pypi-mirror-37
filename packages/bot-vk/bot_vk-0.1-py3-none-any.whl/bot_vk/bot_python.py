...
def vk_bot(szenarii1,szenarii2,message_neznayu="я не знаю, как ответить(((",attachment_neznayu = "doc302808715_475715032",attachments=None,keyboard=None,vk = None,login = None,password = None,token = None):
    ...
    keyboard - или передавать None, или передавать list, в котором каждое значение соответствует
        клавиатуре для данного сценария. Если клавиатуры для какого-то сценария нет, то передавать inet.keyboard.keyboard_none()
        
    vk - параметр inet.auth_vk() Если не передан, то должн быть передан token или login и password
    ...
    if keyboard!= None:
        for i in range(len(keyboard)):
            if keyboard[i] == None:
                keyboard[i] = inet.keyboard.keyboard_none()
    if keyboard==None:
            
            sinus = inet.keyboard.keyboard_none()
                    ...
                    if info["items"][0]["last_message"]["from_id"]>0:# and id_own != info["items"][0]["last_message"]["from_id"]:
                        ...
                        for i in range(len(szenarii1)):
                            ...
                            if mes.lower() == message.lower():
                                s = 0
                                string = inet.parse(szenarii2[i], pol).format(firstname=firstname,lastname=lastname,city=city)
                        ...
                        if s == 1:
                            string = inet.parse(message_neznayu, pol).format(firstname=firstname,lastname=lastname,city=city)
