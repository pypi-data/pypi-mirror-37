...
class Address:
    ...
    def to_dict(self):
        ...
        if self.street_address:
            rv['street_name'] = self.street_address
...
class UserService:
    ...
    def create_profile(self, user_id: UUID, profile: UserProfile) -> UserProfile:
        ...
        response = requests.post(f'{self._profile_url}/api/v1/profiles', json={
            'user_id': str(user_id),
    ...
    def update_profile(self, user_id: str, profile: dict) -> UserProfile:
        response = requests.patch(url=f'{self._profile_url}/api/v1/profiles/{user_id}', json=profile)
    ...
    def get_user_profile(self, id: str) -> UserProfile:
        response = requests.get(f'{self._profile_url}/api/v1/profiles/%s' % str(id))
        if response.status_code == HTTPStatus.OK:
            return self._dict_to_user_profile(response.json().get('data'))
        if response.status_code == HTTPStatus.NOT_FOUND:
            raise NoSuchProfile
    ...
    def create_user(
        ...
        if response.status_code == HTTPStatus.CONFLICT:
            return UserAlreadyExist(response.json().get('error'))
        if response.status_code == HTTPStatus.BAD_REQUEST:
            raise UserCreationFailed

    def generate_otp(self, credential_type, identity):
        response = requests.post(f'{self._auth_url}/api/v1/sessions/otp', json={
            'credential_type': credential_type.name,
            'identity': identity
        })
        if response.status_code != HTTPStatus.ACCEPTED:
            raise OtpGenerationFailed
