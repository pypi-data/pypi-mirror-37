    ...
    def vault_info(dk_api, global_secrets, kitchen):
        ...
        def display_vault_info(item, payload):
            ...
            if item in payload['config']:
                ...
                message += ' token\t\t\t%s%s' % (payload['config'][item]['token'], os.linesep)
                ...
                if not item == DKConstants.VAULT_GLOBAL:
                    ...
                    message += ' source-kitchen\t\t%s%s' % (payload['config'][item]['sourceKitchen'], os.linesep)
        ...
        if kitchen:
            ...
            if kitchen_output_message == '':
                kitchen_output_message = '%sVault is not configured for kitchen %s.' % (os.linesep, kitchen)
            ...
            if item in payload:
                ...
                if 'list' in payload[item]:
                    ...
                    for list_item in sorted_list:
                        override_message = ''
                        if opposite_item and list_item in payload[opposite_item]['list']:
