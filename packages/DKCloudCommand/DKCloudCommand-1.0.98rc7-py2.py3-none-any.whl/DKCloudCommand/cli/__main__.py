...
@click.option('--yes', '-y', default=False, is_flag=True, required=False, help='Force yes')
@click.pass_obj
def vault_config(backend, kitchen, service, url, token, prefix, private, inheritable, yes):
    """
    ...
    Examples:
        ...
        \b
        3) Kitchen vault config with custom service
    ...
    try:
        ...
        param_dict = {
            ...
            'inheritable': inheritable if inheritable else False
        }
        ...
        if kitchen:
            if service == 'default':
                if any([url, token, prefix, private, inheritable]):
