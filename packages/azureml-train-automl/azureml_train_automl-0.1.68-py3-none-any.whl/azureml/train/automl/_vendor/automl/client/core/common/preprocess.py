...
from automl.client.core.common import _engineered_feature_names as efn
...
class DateTimeFeaturesTransformer(
        BaseEstimator, TransformerMixin, TransformerLogger):
    """
...
class RawFeatureStats:
    ...
    def __init__(self, raw_column):
        ...
        self.cardinality_ratio = (
            1.0 * self.num_unique_vals) / \
            self.total_number_vals
...
class DataTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def transform(self, df):
        ...
        if self._engineered_feature_names_class is None:
            ...
            self._engineered_feature_names_class = \
                efn.GenerateEngineeredFeatureNames(
                    self.mapper.transformed_names_,
                    self.logger)

            # Generate engineered feature names
            self._engineered_feature_names_class._parse_raw_feature_names()

        return transformed_data

    def get_engineered_feature_names(self):
        """
        :return: The list of engineered fearure names
        """
        return self._engineered_feature_names_class._engineered_feature_names

    def get_json_str_for_engineered_feature_name(
            self, engineered_feature_name):
        """
    ...
    def get_json_strs_for_engineered_feature_names(
            self, engineered_feature_name_list):
        """
        :param engineered_feature_name_list: Engineered feature names for
        whom JSON strings
                                             are required
        ...
        # Walk engineering feature name list and get the corresponding JSON
        # string
        for engineered_feature_name in engineered_feature_name_list:

            json_str = self.get_json_str_for_engineered_feature_name(
                engineered_feature_name)
    ...
    def _get_transforms(self, df):
        ...
        # append the column type with a counter to generate a
        # raw feature name
        num_numeric_detected = 0
        num_datetime_detected = 0
        num_categorical_detected = 0
        num_categorical_hash_detected = 0
        num_text_detected = 0
        ...
        for dtype, column in zip(df.dtypes, df.columns):
            ...
            if feature_type_detected == efn.FeatureTypeRecognizer.Numeric:

                if isinstance(column, int):
                    # If raw data column type is integer, then compose a
                    # meaningful column name
                    num_numeric_detected = num_numeric_detected + 1
                    numeric_column_name =\
                        efn.FeatureTypeRecognizer.Numeric + \
                        str(num_numeric_detected)
                else:
                    numeric_column_name = column

                # Create transformed numeric column name for the data
                # transformer
                transformed_numeric_column_name = \
                    efn.FeatureNamesHelper\
                    .get_feature_names_for_transformations(
                        str(numeric_column_name),
                        efn.FeatureTypeRecognizer.Numeric,
                        efn.ImputerNames.NumericImputer)

                # floats or ints go as they are, we only fix NaN
                transforms.append(
                    ([column], [Imputer()],
                     {"alias": str(transformed_numeric_column_name)}))

                # if there are lot of imputed values, add an imputation marker
                if raw_stats.num_na > 0.01 * raw_stats.total_number_vals:
                    # Create transformed numeric column name for the data
                    # transformer
                    transformed_numeric_impt_column_name = \
                        efn.FeatureNamesHelper\
                        .get_feature_names_for_transformations(
                            str(numeric_column_name),
                            efn.FeatureTypeRecognizer.Numeric,
                            efn.ImputerNames.ImpMarker)

                    transforms.append(
                        ([column], [ImputationMarker(logger=self.logger)],
                         {"alias": "{}_imputemarker".format(str(
                             transformed_numeric_impt_column_name))}))

            elif feature_type_detected == efn.FeatureTypeRecognizer.DateTime:

                if isinstance(column, int):
                    # If raw data column type is integer, then compose a
                    # meaningful column name
                    num_datetime_detected = num_datetime_detected + 1
                    datetime_column_name = \
                        efn.FeatureTypeRecognizer.DateTime + \
                        str(num_datetime_detected)
                else:
                    datetime_column_name = column

                # Create transformed datatime column name for the data
                # transformer
                transformed_datetime_column_name = \
                    efn.FeatureNamesHelper\
                    .get_feature_names_for_transformations(
                        str(datetime_column_name),
                        efn.FeatureTypeRecognizer.DateTime,
                        efn.ImputerNames.DateTimeImputer)
                ...
                transforms.append((
                    ...
                    {"alias": str(transformed_datetime_column_name)}))
            elif feature_type_detected == \
                    efn.FeatureTypeRecognizer.CategoricalHash:

                # use categorical hash
                if isinstance(column, int):
                    # If raw data column type is integer, then compose a
                    # meaningful column name
                    num_categorical_hash_detected = \
                        num_categorical_hash_detected + 1
                    categorical_hash_column_name = \
                        efn.FeatureTypeRecognizer.CategoricalHash + \
                        str(num_categorical_detected)
                else:
                    categorical_hash_column_name = column

                # Create transformed categorical column name for the data
                # transformer
                transformed_categorical_hash_column_name = \
                    efn.FeatureNamesHelper\
                    .get_feature_names_for_transformations(
                        str(categorical_hash_column_name),
                        efn.FeatureTypeRecognizer.CategoricalHash,
                        None, efn.TransformationType.HashOneHot)
                    ...
                    (column,
                        [
                            ...
                            HashOneHotVectorizerTransformer(
                                ...
                                num_cols=int(bits), logger=self.logger)
                        ],
                        {"alias": str(
                            transformed_categorical_hash_column_name)})
                )
            elif feature_type_detected == \
                    efn.FeatureTypeRecognizer.Categorical:

                # use categorical
                if isinstance(column, int):
                    # If raw data column type is integer, then compose a
                    # meaningful column name
                    num_categorical_detected = num_categorical_detected + 1
                ...
                if raw_stats.num_unique_vals <= 2:
                    ...
                    # Create transformed categorical column name for the data
                    #  transformer
                    transformed_two_categorical_column_name = \
                        efn.FeatureNamesHelper\
                        .get_feature_names_for_transformations(
                            str(categorical_column_name),
                            efn.FeatureTypeRecognizer.Categorical,
                            efn.ImputerNames.CategoricalImputer,
                            efn.TransformationType.LabelEncode)
                    ...
                    # Create transformed categorical column name for the data
                    #  transformer
                    transformed_categorical_column_name = \
                        efn.FeatureNamesHelper\
                        .get_feature_names_for_transformations(
                            str(categorical_column_name),
                            efn.FeatureTypeRecognizer.Categorical,
                            None, efn.TransformationType.CountVec)

                    # use CountVectorizer for both Hash and CategoricalHash
                    # for now
                    transforms.append(
                        (column,
                            [
                            ...
                            {"alias": str(
                                transformed_categorical_column_name)}))
            elif feature_type_detected == efn.FeatureTypeRecognizer.Text:
                if isinstance(column, int):
                    # If raw data column type is integer, then compose a
                    # meaningful column name
                    num_text_detected = num_text_detected + 1
                    text_column_name = efn.FeatureTypeRecognizer.Text + str(
                        num_text_detected)
                else:
    ...
    def _detect_feature_type(self, column, df):
        ...
        if raw_stats.num_unique_vals == 1:
            ...
            feature_type_detected = efn.FeatureTypeRecognizer.Ignore
            ...
            if utilities._check_if_column_data_type_is_numerical(
                ...
                feature_type_detected = efn.FeatureTypeRecognizer.Numeric
            else:
                ...
                if num_dates == non_na.shape[0] and non_na.shape[0] > 0:
                    feature_type_detected = efn.FeatureTypeRecognizer.DateTime
                elif raw_stats.cardinality_ratio > 0.85 and \
                        raw_stats.average_number_spaces > 1.0:
                    feature_type_detected = efn.FeatureTypeRecognizer.Text
                    ...
                    feature_type_detected = efn.FeatureTypeRecognizer.Hashes
                else:
                    feature_type_detected = efn.FeatureTypeRecognizer.Ignore
        else:
            if utilities._check_if_column_data_type_is_int(
                ...
                feature_type_detected = efn.FeatureTypeRecognizer.Categorical
    ...
    def _get_string_transforms(self, column, column_name, ngram_len):
        """
        ...
        # Create trasnformed text tf idf column name for the data transformer
        tfidf_word_column_name = \
            efn.FeatureNamesHelper.get_feature_names_for_transformations(
                str(column_name), efn.FeatureTypeRecognizer.Text, None,
                efn.TransformationType.TfIdfWordBi)

        tfidf_char_column_name = \
            efn.FeatureNamesHelper.get_feature_names_for_transformations(
                str(column_name), efn.FeatureTypeRecognizer.Text, None,
                efn.TransformationType.TfIdfCharTri)
        ...
        tr = [(column,
               ...
               {
                   "alias": str(tfidf_char_column_name)
               }
               ),
              (column,
               [
               ...
               {
                   "alias": str(tfidf_word_column_name)
               }
               )]

        if self._task_type == constants.Tasks.CLASSIFICATION:
            # Create transformed text count vectorizer column name for the data
            # transformer
            count_vec_column_name = \
                efn.FeatureNamesHelper.get_feature_names_for_transformations(
                    str(column_name),
                    efn.FeatureTypeRecognizer.Text,
                    efn.ImputerNames.TextImputer,
                    efn.TransformationType.CountVec)
