...
import json
from marketorestclientpython.helper.http_lib import HttpLib
from marketorestclientpython.helper.exceptions import MarketoException, MarketoAuthenticationException
...
class MarketoClient:
    ...
    def authenticate(self):
        ...
        if data is None:
            raise MarketoAuthenticationException({'message': 'Empty response', 'code': 400})
        if 'error' in data:
            if data['error'] in ['unauthorized', 'invalid_client']:
                raise Exception({'message': data['error_description'], 'code': 401})
