...
class PySparkHelper:
	...
	def toPySparkDf(df_in):
		ist_of_cols = []

	  	for i in df_in.columns:
	    	new_struct_field = StructField(i, convertTypes(str(df_in[i].dtype)), True)
	    	list_of_cols.append(new_struct_field)
	    
	  	DfSchema = StructType(list_of_cols)
	  
		return spark.createDataFrame(df_in,schema=DfSchema)


	def convertTypes(type_in):
