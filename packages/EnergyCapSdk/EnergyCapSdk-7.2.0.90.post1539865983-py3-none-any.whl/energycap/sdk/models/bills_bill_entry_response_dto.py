# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class BillsBillEntryResponseDTO(Model):
    """BillsBillEntryResponseDTO.

    :param bill_id:
    :type bill_id: int
    :param account_id:
    :type account_id: int
    :param vendor_id:
    :type vendor_id: int
    :param needs_to_open_batch:
    :type needs_to_open_batch: bool
    :param begin_date:
    :type begin_date: datetime
    :param end_date:
    :type end_date: datetime
    :param billing_period:
    :type billing_period: int
    :param days:
    :type days: int
    :param total_cost:
    :type total_cost: float
    :param due_date:
    :type due_date: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param statement_date:
    :type statement_date: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param invoice_number:
    :type invoice_number: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param control_code:
    :type control_code: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param next_reading:
    :type next_reading: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param account_period_name:
    :type account_period_name: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param account_period_number:
    :type account_period_number: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param account_period_year:
    :type account_period_year: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param estimated:
    :type estimated: ~energycap.sdk.models.BillsBillHeaderChildDTO
    :param bill_note:
    :type bill_note: str
    :param void:
    :type void: bool
    :param from_vendor:
    :type from_vendor: bool
    :param observation_method:
    :type observation_method:
     ~energycap.sdk.models.CommoditiesObservationMethodChildDTO
    :param approved:
    :type approved: bool
    :param has_been_split:
    :type has_been_split: bool
    :param export_hold:
    :type export_hold: bool
    :param ap_exported:
    :type ap_exported: bool
    :param gl_exported:
    :type gl_exported: bool
    :param check_number:
    :type check_number: str
    :param check_date:
    :type check_date: datetime
    :param pay_status:
    :type pay_status: str
    :param cleared_date:
    :type cleared_date: datetime
    :param bill_image_url:
    :type bill_image_url: str
    :param general_ledger_code:
    :type general_ledger_code: str
    :param meters:
    :type meters: list[~energycap.sdk.models.BillsBillEntryMeterChildDTO]
    :param account_body_lines:
    :type account_body_lines:
     list[~energycap.sdk.models.BillsBillEntryBodyLineDTO]
    """

    _attribute_map = {
        'bill_id': {'key': 'billId', 'type': 'int'},
        'account_id': {'key': 'accountId', 'type': 'int'},
        'vendor_id': {'key': 'vendorId', 'type': 'int'},
        'needs_to_open_batch': {'key': 'needsToOpenBatch', 'type': 'bool'},
        'begin_date': {'key': 'beginDate', 'type': 'iso-8601'},
        'end_date': {'key': 'endDate', 'type': 'iso-8601'},
        'billing_period': {'key': 'billingPeriod', 'type': 'int'},
        'days': {'key': 'days', 'type': 'int'},
        'total_cost': {'key': 'totalCost', 'type': 'float'},
        'due_date': {'key': 'dueDate', 'type': 'BillsBillHeaderChildDTO'},
        'statement_date': {'key': 'statementDate', 'type': 'BillsBillHeaderChildDTO'},
        'invoice_number': {'key': 'invoiceNumber', 'type': 'BillsBillHeaderChildDTO'},
        'control_code': {'key': 'controlCode', 'type': 'BillsBillHeaderChildDTO'},
        'next_reading': {'key': 'nextReading', 'type': 'BillsBillHeaderChildDTO'},
        'account_period_name': {'key': 'accountPeriodName', 'type': 'BillsBillHeaderChildDTO'},
        'account_period_number': {'key': 'accountPeriodNumber', 'type': 'BillsBillHeaderChildDTO'},
        'account_period_year': {'key': 'accountPeriodYear', 'type': 'BillsBillHeaderChildDTO'},
        'estimated': {'key': 'estimated', 'type': 'BillsBillHeaderChildDTO'},
        'bill_note': {'key': 'billNote', 'type': 'str'},
        'void': {'key': 'void', 'type': 'bool'},
        'from_vendor': {'key': 'fromVendor', 'type': 'bool'},
        'observation_method': {'key': 'observationMethod', 'type': 'CommoditiesObservationMethodChildDTO'},
        'approved': {'key': 'approved', 'type': 'bool'},
        'has_been_split': {'key': 'hasBeenSplit', 'type': 'bool'},
        'export_hold': {'key': 'exportHold', 'type': 'bool'},
        'ap_exported': {'key': 'apExported', 'type': 'bool'},
        'gl_exported': {'key': 'glExported', 'type': 'bool'},
        'check_number': {'key': 'checkNumber', 'type': 'str'},
        'check_date': {'key': 'checkDate', 'type': 'iso-8601'},
        'pay_status': {'key': 'payStatus', 'type': 'str'},
        'cleared_date': {'key': 'clearedDate', 'type': 'iso-8601'},
        'bill_image_url': {'key': 'billImageUrl', 'type': 'str'},
        'general_ledger_code': {'key': 'generalLedgerCode', 'type': 'str'},
        'meters': {'key': 'meters', 'type': '[BillsBillEntryMeterChildDTO]'},
        'account_body_lines': {'key': 'accountBodyLines', 'type': '[BillsBillEntryBodyLineDTO]'},
    }

    def __init__(self, bill_id=None, account_id=None, vendor_id=None, needs_to_open_batch=None, begin_date=None, end_date=None, billing_period=None, days=None, total_cost=None, due_date=None, statement_date=None, invoice_number=None, control_code=None, next_reading=None, account_period_name=None, account_period_number=None, account_period_year=None, estimated=None, bill_note=None, void=None, from_vendor=None, observation_method=None, approved=None, has_been_split=None, export_hold=None, ap_exported=None, gl_exported=None, check_number=None, check_date=None, pay_status=None, cleared_date=None, bill_image_url=None, general_ledger_code=None, meters=None, account_body_lines=None):
        super(BillsBillEntryResponseDTO, self).__init__()
        self.bill_id = bill_id
        self.account_id = account_id
        self.vendor_id = vendor_id
        self.needs_to_open_batch = needs_to_open_batch
        self.begin_date = begin_date
        self.end_date = end_date
        self.billing_period = billing_period
        self.days = days
        self.total_cost = total_cost
        self.due_date = due_date
        self.statement_date = statement_date
        self.invoice_number = invoice_number
        self.control_code = control_code
        self.next_reading = next_reading
        self.account_period_name = account_period_name
        self.account_period_number = account_period_number
        self.account_period_year = account_period_year
        self.estimated = estimated
        self.bill_note = bill_note
        self.void = void
        self.from_vendor = from_vendor
        self.observation_method = observation_method
        self.approved = approved
        self.has_been_split = has_been_split
        self.export_hold = export_hold
        self.ap_exported = ap_exported
        self.gl_exported = gl_exported
        self.check_number = check_number
        self.check_date = check_date
        self.pay_status = pay_status
        self.cleared_date = cleared_date
        self.bill_image_url = bill_image_url
        self.general_ledger_code = general_ledger_code
        self.meters = meters
        self.account_body_lines = account_body_lines
