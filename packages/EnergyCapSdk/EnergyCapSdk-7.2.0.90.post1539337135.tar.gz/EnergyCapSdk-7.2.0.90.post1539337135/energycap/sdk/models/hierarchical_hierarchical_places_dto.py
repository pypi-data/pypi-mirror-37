# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class HierarchicalHierarchicalPlacesDto(Model):
    """HierarchicalHierarchicalPlacesDto.

    :param parent_id: The place's parent
    :type parent_id: int
    :param place_id: The place's primary ID
    :type place_id: int
    :param place_info: PlaceInfo
    :type place_info: str
    :param place_code: PlaceCode
    :type place_code: str
    :param place_type_id: PlaceTypeID
    :type place_type_id: int
    :param has_children: If this place has any children, either meters or
     places
    :type has_children: bool
    :param meter_children: List of meter children
    :type meter_children:
     list[~energycap.sdk.models.LogicalDeviceMeterChildDTO]
    :param place_children: List of place children
    :type place_children:
     list[~energycap.sdk.models.HierarchicalHierarchicalPlaces2Dto]
    """

    _attribute_map = {
        'parent_id': {'key': 'parentId', 'type': 'int'},
        'place_id': {'key': 'placeId', 'type': 'int'},
        'place_info': {'key': 'placeInfo', 'type': 'str'},
        'place_code': {'key': 'placeCode', 'type': 'str'},
        'place_type_id': {'key': 'placeTypeId', 'type': 'int'},
        'has_children': {'key': 'hasChildren', 'type': 'bool'},
        'meter_children': {'key': 'meterChildren', 'type': '[LogicalDeviceMeterChildDTO]'},
        'place_children': {'key': 'placeChildren', 'type': '[HierarchicalHierarchicalPlaces2Dto]'},
    }

    def __init__(self, parent_id=None, place_id=None, place_info=None, place_code=None, place_type_id=None, has_children=None, meter_children=None, place_children=None):
        super(HierarchicalHierarchicalPlacesDto, self).__init__()
        self.parent_id = parent_id
        self.place_id = place_id
        self.place_info = place_info
        self.place_code = place_code
        self.place_type_id = place_type_id
        self.has_children = has_children
        self.meter_children = meter_children
        self.place_children = place_children
