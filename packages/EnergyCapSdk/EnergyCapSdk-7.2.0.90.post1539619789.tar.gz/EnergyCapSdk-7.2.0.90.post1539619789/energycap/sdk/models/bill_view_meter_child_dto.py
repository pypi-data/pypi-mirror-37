# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class BillViewMeterChildDTO(Model):
    """BillViewMeterChildDTO.

    :param meter_id: The meter identifier
    :type meter_id: int
    :param meter_code: The meter code
    :type meter_code: str
    :param meter_info: The meter info
    :type meter_info: str
    :param meter_type: The type of the meter
    :type meter_type: ~energycap.sdk.models.LogicalDeviceMeterTypeChildDTO
    :param commodity: The meter's commodity
    :type commodity: ~energycap.sdk.models.CommoditiesCommodityChildDTO
    :param serial_number: The meter's serial number
    :type serial_number: str
    :param rate: The meter's rate
    :type rate: ~energycap.sdk.models.AccountsRateChildDTO
    :param place: The place the meter assigned to
    :type place: ~energycap.sdk.models.PlacePlaceChildDTO
    :param general_ledger: The meter's general ledger
    :type general_ledger: ~energycap.sdk.models.BillsGeneralLedgerChildDTO
    """

    _attribute_map = {
        'meter_id': {'key': 'meterId', 'type': 'int'},
        'meter_code': {'key': 'meterCode', 'type': 'str'},
        'meter_info': {'key': 'meterInfo', 'type': 'str'},
        'meter_type': {'key': 'meterType', 'type': 'LogicalDeviceMeterTypeChildDTO'},
        'commodity': {'key': 'commodity', 'type': 'CommoditiesCommodityChildDTO'},
        'serial_number': {'key': 'serialNumber', 'type': 'str'},
        'rate': {'key': 'rate', 'type': 'AccountsRateChildDTO'},
        'place': {'key': 'place', 'type': 'PlacePlaceChildDTO'},
        'general_ledger': {'key': 'generalLedger', 'type': 'BillsGeneralLedgerChildDTO'},
    }

    def __init__(self, meter_id=None, meter_code=None, meter_info=None, meter_type=None, commodity=None, serial_number=None, rate=None, place=None, general_ledger=None):
        super(BillViewMeterChildDTO, self).__init__()
        self.meter_id = meter_id
        self.meter_code = meter_code
        self.meter_info = meter_info
        self.meter_type = meter_type
        self.commodity = commodity
        self.serial_number = serial_number
        self.rate = rate
        self.place = place
        self.general_ledger = general_ledger
