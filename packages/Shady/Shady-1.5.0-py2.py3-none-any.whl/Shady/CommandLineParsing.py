...
If I had known about `argparse` I probably wouldn't have written this,
but it has many nice additional features.
	...
	def Resolve( self, args_p, args_k, optsOut=None ):
		...
		if value is Unspecified:
			if self.position is not Unspecified and 0 <= self.position < len( args_p ):
				...
				matchedArg = repr( value ) + ' when interpreted as ' + self.name
				matchedType = 'p'
				sources.append( ( originalPosition, matchedType ) )
		...
		if self.type:
			for t in self.type:
				...
				if t in [ str ]:
					...
					for permittedSet in [ self.values, self.strings ]:
						if permittedSet:
							...
							else: raise CommandLineValueError( matchedArg, 'must be ' + AlternativesString( permittedSet ) )
					break
...
class CommandLine( object ):
	...
	`c.Option(...)`
		...
		as key and value in `c.opts`. If you specify another `container` it will be
		stored there instead (and if you say `container=None`, it will not be stored
	...
	`c.Delegate()`
		...
		options removed, so you can pass it to the next `CommandLine`.
	"""
...
if __name__ == '__main__':
	...
	cmdline.Option( 'subcommand', default='help', type=str, position=0 )
	cmdline.Option( 'ipython',    default=True,   type=bool )
