...
def makeform(root, fields):
	...
	for field in fields:
		...
		if not field == 'Modeling' and not field =='Preprocessing' and not field =='':
			ent = Entry(row)
			if field == 'Number of representative cell shapes':
...
def build(entries,progress_bar):
	...
	clnum = entries['Number of representative cell shapes'].get()
...
def interface():
	...
	fields = ('Preprocessing','Cell Profiler output subfolder','','Modeling','Cell Profiler output folder', 'Number of representative cell shapes','Model name','','','Status')
	...
	b2_1 = Checkbutton(rows[2],text='Generate Properties',variable=propgen)
	...
	#function 2
	b5 = Button(rows[7],text='build model',width=12,command=(lambda e=ents: build(e,progress_bar)))
	b5.pack(side=RIGHT,padx=5,pady=5)
	b6 = Button(rows[7],text='apply model',width=12,command=(lambda e=ents: apply(e,progress_bar)))
