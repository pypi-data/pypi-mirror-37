...
.. glossary::
    ...
    remote_head_name
        :class:`str` - The host name of the remote head node (to prevent ssh when already on the head node). The corresponding environment variable is ``MAVIS_REMOTE_HEAD_NAME`` and the default value is ``''``
