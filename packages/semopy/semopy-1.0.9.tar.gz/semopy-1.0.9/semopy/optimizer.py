...
class Optimizer:
    ...
    def optimize(self, objective='MLW', regu=None, a=1.0, params_to_pen=list(),
                 method='SLSQP', bounds=True, degenerate_vars=None, **args):
        """Optimize model.
        ...
        degenerate_vars -- A list of observed variables to exclude from both
                           empirical and model-induced covariance matrices.
...
#           self.params = portmin(lf, self.params, **portmin_methods[method])
        ...
        ret =  lf(self.params) if rf is None else (lf(self.params),
                                                   rf(self.params))
        if degenerate_vars:
