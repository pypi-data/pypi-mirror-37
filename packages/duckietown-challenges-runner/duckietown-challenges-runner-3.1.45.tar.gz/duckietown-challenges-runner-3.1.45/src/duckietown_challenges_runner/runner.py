...
def run_single(wd, aws_config, steps2artefacts, challenge_parameters, solution_container, challenge_name,
    ...
    if valid_config:
        cr = run_one(wd, project, do_pull, monitor=tomonitor[0])
...
def run_one(wd, project, do_pull, monitor, timeout=600):
    ...
    try:
        ...
        container_id = get_id_for_service(wd, project, monitor)
        client = check_docker_environment()
        ...
        if True:
            ...
            while True:
                container = client.containers.get(container_id)
                x = container.logs(since=last)
                last = datetime.datetime.now()
                if x:
                    sys.stdout.write(x)
                if container.status != 'running':
                    break
                print('Waiting for container %s: %s' % (container_id, container.status))
                time.sleep(1)
...
def run_docker(cwd, project, cmd0, get_output=False):
    ...
    try:
        if get_output:
            ...
            subprocess.check_call(cmd0, cwd=cwd, stdout=sys.stdout, stderr=subprocess.STDOUT)
