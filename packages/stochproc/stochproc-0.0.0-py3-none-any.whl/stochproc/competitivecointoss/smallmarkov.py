...
def plot_probs():
    ...
    p_n = np.array([np.dot(start1, np.linalg.matrix_power\
                            (m_4,i))[0,0]\
                         for i in range(40)])
    #plt.plot(np.arange(40), p_n, color='red')

    p_n = np.array([np.dot(start1, np.linalg.matrix_power\
                            (m_4,i))[0,1]\
                         for i in range(40)])
    #plt.plot(np.arange(40), p_n, color='yellow')
    p_n = np.array([np.dot(start1, np.linalg.matrix_power\
                            (m_4,i))[0,2]\
                         for i in range(40)])
    #plt.plot(np.arange(40), p_n, color='orange')
    p_n = np.array([np.dot(start1, np.linalg.matrix_power\
                            (m_4,i))[0,3]\
                         for i in range(40)])
    plt.plot(np.arange(40), p_n, color='green')

    q_n = np.array([np.dot(start2, np.linalg.matrix_power\
                            (m_3,i))[0,2]\
                         for i in range(40)])
    plt.plot(np.arange(40), q_n, color='red')
    plt.show()
...
def plot_nHb4npl1H():
    ...
    for n in ns:
        ...
        win_probs.append(win_seq*lose_seq)
        lose_probs.append(lose_seq*win_seq)
