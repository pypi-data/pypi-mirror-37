...
def imgwithtext(img,text):
    ...
    if len(text)<=15 and bgimg.size[0]>=15*textinfo[0]:
        #获得图片长和宽 bgimg.size
        draw = ImageDraw.Draw(bgimg)
        #bgimg.size[0]-textinfo[0])//2将文字居中，bgimg.size[1]-textinfo[1]将文字置底
        draw.text(((bgimg.size[0]-textinfo[0])//2, bgimg.size[1]-textinfo[1]), text, fill=(0, 0, 0), font=ttfront)
        bgimg.show()
        bgimg.save("./new.jpg")
    else:
        ...
        draw.text(((resizebgimg.size[0] - textinfo[0]) // 2, resizebgimg.size[1] - textinfo[1]),
                  text, fill=(0, 0, 0), font=resizebgimgront)
        resizebgimg.show()
        resizebgimg.save("./resizeimg.jpg")

# imgwithtext(img,text)