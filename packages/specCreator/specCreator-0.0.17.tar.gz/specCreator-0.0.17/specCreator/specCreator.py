...
def clean_tmp_podspec(projectPath):
    ...
    repoPath = userPath + "/.cocoapods/repos/" + repoName
    if not fileExist(repoPath):
        excommandUntilDone("pod repo add lianjia-mobile_ios-LJComponentPodSpecs http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs")
...
def autoPackage(projectPath, podName, branch, version, subPackage=False, commitId="", dependencyJSON="", framework=True,
    ...
    if autoPushRepo:
        ...
        repoName = "lianjia-mobile_ios-" + repoSource.strip(" ").strip(".git").split("/")[-1]
        # 获取用户目录
        userPath = os.path.expanduser('~')
