...
def clean_tmp_podspec(projectPath):
    ...
    repoPath = userPath + "/.cocoapods/repos/" + repoName
    if not fileExist(repoPath):
        excommandUntilDone("pod repo add LJComponentPodSpecs http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs")
        excommandUntilDone("pod repo update LJComponentPodSpecs")
    os.chdir(repoPath)
...
def package(projectPath, podName, subSpecName, version, commitId, debug, dependencyJSON, subspecs, isFrameWork=True):
    ...
    descFileName = ''
    if isFrameWork:
        cmd += "pod package " + podspecName + " --force --no-mangle --verbose --exclude-deps --spec-sources=http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs,http://git.lianjia.com/mobile_ios/Lianjia_component_Podspec.git,https://github.com/CocoaPods/Specs.git "
        suffixString = ".framework"
        productDir = podName + "/Framework"
    else:
        cmd += "pod package " + podspecName + " --library --force --verbose --no-mangle --exclude-deps --spec-sources=http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs,http://git.lianjia.com/mobile_ios/Lianjia_component_Podspec.git,https://github.com/CocoaPods/Specs.git "
...
def changePodSpec(podName, version, commitId=""):
    ...
    for lineStr in readFileLines(originSpecPath):
        ...
        if str(tmpLine).startswith("s.version"):
            ...
            lineStr = lineStr.replace("git.lianjia.com", "gerrit.lianjia.com")
...
def canPackage(projectPath, podName):
    sourcePath = os.path.join(projectPath, podName) + "/Classes"
    if not fileExist(sourcePath):
        formatPrint(sourcePath + "没有找到源码目录。", "*")
        return False
    os.chdir(sourcePath)
    returnCode, content = excommandUntilDone("ls -R |awk '{print i$0}' i=`pwd`'/'| grep \"\.[m|mm|c]$\"")
    os.chdir(projectPath)
    if returnCode > 0:
        formatPrint("源码中没有找到要打包的资源文件")
        return False
    return True
...
def autoPackage(projectPath, podName, branch, version, subPackage=False, commitId="", dependencyJSON="", framework=True,
    ...
    dependencyDict = {}
    # 看有没有源码~
    haveSourceFile = canPackage(projectPath, podName)
...
def checkParams(podName, subPackage, whiteListPath, version, subspecs=[], autoPushRepo=False):
    ...
    if autoPushRepo:
        ...
        if content:
            ...
            for tag in tagList:
                if tagString == tag or version == tag:
...
def cleanEnv():
    ...
    excommandUntilDone("git add . ; git reset --hard")
    clean_tmp_podspec
    # excommandUntilDone("pod repo update")
