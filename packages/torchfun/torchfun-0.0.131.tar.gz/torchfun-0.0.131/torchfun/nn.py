...
def add_noise(in_tensor,noise_type='normal',noise_param=(0,1),range_limit=(-1,1)):
    ...
    Noise type can be either `normal` or `uniform`
        * for normal, (mean,variance) is required as noise_param
    ...
    if noise_type=='uniform':
        ...
        noise = torch.rand(in_tensor.shape)*(maxv-minv) - minv
    elif noise_type=='normal':
        mean,var = noise_param
        noise = (torch.randn(in_tensor.shape)+mean)*var
