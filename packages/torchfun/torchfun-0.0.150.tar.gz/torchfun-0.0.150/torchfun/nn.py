...
def instance_mean_std(x,num_features):
    ...
    print('input shape is:',x.shape,'num_features:',num_features)
    x = x.view(b,nc_groups,num_features,-1)
    x = x.transpose(1,2) # b, numfeatu, ncgroup, h*w
    x = x.contiguous()
    x = x.view(b*num_features,-1)

    mean = x.mean(1)
    std = x.std(1)

    mean = mean.view(b,num_features) # n,feature
    std = std.view(b,num_features) # n,feature
    return mean,std
...
def instance_renorm(x,mean,std):
    '''Argument:
    x: NCHW
    mean: N * num-features
    std: N * num-features
...
class InstanceReNorm(torch.nn.Module):
    ...
    def forward(self,x,mean,std):
        return instance_renorm(x,mean,std)
    forward.__doc__ = __doc__