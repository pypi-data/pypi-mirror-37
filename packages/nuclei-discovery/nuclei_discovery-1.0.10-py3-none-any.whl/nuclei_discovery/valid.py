...
from .helper import load_ckpt, prob_to_rles, prob_to_instances, iou_metric, clahe

INFERENCE_BATCH_SIZE = 100
POST_PROCESSING_WORKER_LIMIT = 10
...
def main(ckpt, action='csv', save=False, target='test'):
    ...
    if batchjob_info is not None: # Non-GUI, parallelized processing
        ...
        for batch_data in split_every(INFERENCE_BATCH_SIZE, dataset):
            print('Inference....')
            preds = []
            ...
            if action=='csv': # one output file for all inferences
                for result in parallel_processing_with_progress(collect_csv_job, preds):
                    for r in result:
                        writer.writerow(r)
            elif action=='iou': # one output file for all inferences
                ious = []
                for result in parallel_processing_with_progress(collect_iou_job, preds):
                ...
                for _ in parallel_processing_with_progress(save_rle_mask_job, preds): pass
            elif action == 'png_mask': # one output file per inference
                for _ in parallel_processing_with_progress(save_png_mask_job, preds): pass
...
def parallel_processing_with_progress(func, data):
    with Pool(POST_PROCESSING_WORKER_LIMIT) as p:
        with tqdm(total=len(data)) as pbar:
            for result in tqdm(p.imap(func, data)):
                yield result
                pbar.update()
...
def save_rle_mask_job(pred):
    ...
    with open(rle_csv, 'w') as fp:
        ...
        for idx, rle in enumerate(prob_to_rles(y, y_c, y_m)):
            writer.writerow([f'mask_{idx+1}', ' '.join([str(i) for i in rle])])

def save_png_mask_job(pred):
    uid, y, y_c, y_m, _ = pred
    instances, _ = prob_to_instances(y, y_c, y_m)
...
def show(uid, x, y, y_c, y_m, save=False):
    ...
    y, markers = prob_to_instances(y, y_c, y_m)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--action', choices=['csv', 'iou', 'rle_mask', 'png_mask', 'show'], help='Specify action to do')
    ...
    parser.set_defaults(action='csv', save=False, dataset='test')
