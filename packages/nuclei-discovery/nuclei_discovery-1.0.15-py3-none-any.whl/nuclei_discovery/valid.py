...
def main(ckpt, action='[csv]', save=False, target='test'):
    ...
    if (len(action) > 1) and ('rle_mask' in action) and ('png_preds' in action):
        action = 'rle_mask'
        save_png_preds = True
    elif (len(action) == 1):
        action = action[0]
    ...
    if batchjob_info is not None: # Non-GUI, parallelized processing
        ...
        for batch_data in split_every(infr_batchsize, dataset):
            ...
            if action == 'csv': # one output file for all inferences
                ...
                for _ in parallel_processing_with_progress(partial(save_rle_mask_job, save_png=save_png_preds), preds): pass
                ...
                for _ in parallel_processing_with_progress(save_png_preds_job, preds): pass
...
def save_png_preds_job(pred):
    uid, y, y_c, y_m, _ = pred
    dir = os.path.join(predict_save_folder(), uid, 'infr')
    if not os.path.exists(dir):
        os.makedirs(dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--action', nargs='+', choices=['csv', 'iou', 'rle_mask', 'png_mask', 'png_preds','show'], help='Specify action to do, multiple actions only for rle_mask & png_mask')
