...
def exclude():
    ...
    return func 
'''
inject request/response context for method, valid for the last parameter!
'''
def context():
    def func(fn): 
        fn.__ctx = True 
...
class RpcProcessor:
    ...
    @staticmethod
    def _get_class_filter(obj): 
        t = inspect.getmembers(obj) 
        for kv in t:
            if kv[0] == '__class_filter': 
                f = kv[1] 
                return f[1] 
    ...
    def mount(self, prefix, service, doc_enabled=True): 
        ...
        class_filter = RpcProcessor._get_class_filter(service)
        ...
        for method in methods:
            ...
            req_filter_array = []
            if class_filter:
                req_filter_array.append(class_filter)
            ctx_inject = False
            exclude = False
            ...
            if hasattr(method[1], '__filter'):
                ...
                req_filter_array.append(req_filter)
            if hasattr(method[1], '__ctx'):
                ctx_inject = getattr(method[1], '__ctx')  
            ...
            method_instance.filter_array = req_filter_array
            method_instance.ctx_inject = ctx_inject
    ...
    def process(self, req, res):   
        ...
        try:    
            kvargs = {}
            ctx_inject = target_method.ctx_inject
            if ctx_inject:
                ctx = Dict()
                ctx.request = req
                ctx.response = res
                params.append(ctx) #last parameter as request context 
