...
class DatafileFunctions(object):
    ...
    def delete_datafile(self, datafile_id):
        ...
        datafile = self._get_datafile(datafile_id)
        if not datafile:
            raise CatalogueEntityNotFoundException('Datafile', id=datafile_id)
    ...
    def add_files_to_datafile(self, datafile_id, s3_prefix, files):
        ...
        dataset = self.get_dataset(datafile.dataset_id)
        if not dataset:
            raise CatalogueEntityNotFoundException('Dataset', id=datafile.dataset_id)
