...
class NginxWithPumaWebserver(Webserver):
    ...
    def _nginx_project_configuration(self):
        ...
        configuration += 'server {\n' + \
                         ...
                         '\troot %s;\n' % os.path.join(self._webapp_root_path(), 'public')
    ...
    def _configure_puma(self):
        ...
        with self._machine.cd(self._webapp_root_path()):
    ...
    def _puma_already_configured(self):
        config_file = os.path.join(self._webapp_root_path(), self._puma_config_file())
        return self._machine.path_exists(config_file)

    def _puma_project_configuration(self):
        log_path = "%s/log" % self._webapp_root_path()
        pids_path = "%s/tmp/pids" % self._webapp_root_path()
    ...
    def _socket_path(self):
        return "%s/tmp/sockets/%s.sock" % (self._webapp_root_path(), self._webapp_name)
