...
class XnatUploadTool:
    ...
    def start_upload(self):
        ...
        if self.ingestconfig is not None:
            self.logger.info("Starting upload of %s to %s" % (self.target, self.host))
    ...
    def start_ingestion(self):
        ...
        for mysubject, mysubjectdata in self.ingestdata.items():
            self.check_subject(create=True, subject=mysubject, fields=mysubjectdata['fields'])
            for mysession, mysessiondata in mysubjectdata['sessions'].items():
                self.check_session(create=True, subject=mysubject, session=mysession,
                                   datatype=mysessiondata['datatype'], xmlnamespace=mysessiondata['xmlnamespace'],
                                   fields=mysessiondata['fields'])
    ...
    def analyze_ingestion_targets(self):
        ...
        if os.path.exists(self.target):
            ...
            self.logger.info("Analyzed ingestion target: %s files @ %s" %
                              (totalfiles, self.bytes_format(totalsize)))
            return True
        else:
    ...
    def pull_ingestion_field(self, pattern, filename):
        ...
        if 'separator' in pattern:
            ...
            if pattern['source'] == 'filename':
                parts = os.path.basename(filename).split(pattern['separator'])
            elif pattern['source'] == 'filename-no-ext':
                parts = os.path.basename(filename.rsplit(".", 1)[0]).split(pattern['separator'])
            ...
            if 'fields' in pattern:
                ...
                if 'join' in pattern:
                    myjoin = pattern['join']
                for thisfield in pattern['fields']:
                    myoutput += parts[int(thisfield)-1] + myjoin
                if len(myjoin) > 0:
                    myoutput = myoutput[:-(len(myjoin))]
                # range logic
            if 'range' in pattern:
                ranges = pattern['range'].split(',')
                myoutput = myoutput[ranges[0]:ranges[1]]
        else:
    ...
    def upload_ingestedfile(self, subject=None, session=None, scan=None, scandata=None, resource=None):
        ...
        self.logger.info('[%s] Uploading data file (%s) to [Proj %s Sub %s Sess %s Resource %s]' %
                         (scan, self.bytes_format(sumsize), self.project, subject, session, resource))
