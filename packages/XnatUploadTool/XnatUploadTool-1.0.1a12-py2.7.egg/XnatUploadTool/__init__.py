...
class XnatUploadTool:
    ...
    def setup_upload(self):
        # Check for project
        self.logger.info('Preparing for upload to %s from %s as %s: declared type %s' %
                         (self.host, self.directory, self.username, self.datatype))
    ...
    def start_upload(self):
        ...
        if self.scan is not True:
            self.analyze_dir(self.directory + '/' + self.scan)
        else:
        ...
        if os.path.isfile(self.directory):
            ...
            if self.scan:
                ...
                results.append(self.process_scan(self.directory, self.filetree))
            else:
    ...
    def process_scan(self, scan, filetree):
        ...
        if self.raw is True:
            ...
            if os.path.isfile(self.directory):
                ...
                self.upload_datafile(targetdir=self.directory, scan=scan)
            else:
                ...
                elif len(self.filetree[self.directory][scan]['otherfiles']) > 0:
                    self.renew_session()
                    # Upload all files in scan dir
                    self.upload_datafiles(targetdir=os.path.join(self.directory, scan), scan=scan)
        ...
        for thisfile in targetfiles:
            ...
            try:
                myurl = ("%s/resources/%s/files/%s?inbody=true&update-stats=true&event_reason=batch-upload"
                         "&event_action=Uploaded-%s" % (myurl, self.folder, thisfile, self.datatype))
    ...
    def upload_singlefile(self, scan):
        ...
        self.logger.debug('[%s] Analyzing datafile in %s' % (scan, self.directory))
        sumsize = os.path.getsize(self.directory)
        ...
        mydata = open(self.directory, 'rb')

        try:
            myurl = ("%s/resources/%s/files/%s?inbody=true&update-stats=true&event_reason=batch-upload"
                     "&event_action=Uploaded-%s" %
                     (myurl, self.folder, scan, self.datatype))
    ...
    def check_project(self):
        ...
        try:
            ...
            if response.status_code == 200:
                    self.logger.info('Project %s found' % self.project)
                    return True
        except Exception as e:
    ...
    def check_session(self, create=False):
        ...
        try:
            response = self.httpsess.get(self.host + '/data/projects/%s/subjects/%s/experiments/%s?format=json' %
                                         (self.project, self.subject, self.session))
            if response.status_code == 200:
                self.logger.info('Session %s found' % self.session)
                return True
        except Exception as e:
    ...
    def check_scan(self, create=False):
        ...
        try:
            ...
            if response.status_code == 200:
                self.logger.info('Scan %s found' % self.scan)
                return True
        except Exception as e:
