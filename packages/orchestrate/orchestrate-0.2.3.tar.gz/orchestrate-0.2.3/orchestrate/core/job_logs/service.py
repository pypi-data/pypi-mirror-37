...
class FollowPodsRunner(object):
  def __init__(self, kubectl_service, kube_config, experiment_id, color_off=False, max_pods=10):
    self.kubectl_service = kubectl_service
  ...
  def wait_for_pod_logs(self, pod_name):
    while self.kubectl_service.kubectl_get(['pod', pod_name])['status']['phase'] == 'Pending':
      sleep(random.uniform(5, 6))

  def execute_follow(self, pod_name, color, kube_config):
    pargs = self.kubectl_service.kubectl_pargs(['logs', '-f', pod_name], decode_json=False)
    env = self.kubectl_service.kubectl_env()
    try:
...
class JobLogsService(Service):
  def get_logs(self, experiment_id):
    ...
    pod_names = self.services.kubectl_service.pod_names(job_name)
    for name in pod_names:
      ...
      try:
        print(self.services.kubectl_service.logs(name))
      except Exception:
        print(safe_format("""
  ...
  def follow_logs(self, experiment_id, color_off=False):
    job_name = self.services.job_runner_service.job_name(experiment_id)
    kube_config = self.services.kubectl_service.kube_config
    follow_pods_runner = FollowPodsRunner(self.services.kubectl_service, kube_config, experiment_id, color_off)
    try:
      while True:
        current_pods = self.services.kubectl_service.pod_names(job_name)
        for pod in current_pods:
          follow_pods_runner.follow(pod, self.services.kubectl_service.logs)
