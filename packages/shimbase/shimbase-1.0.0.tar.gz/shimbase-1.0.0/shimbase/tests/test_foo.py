...
from database import Database, AdhocKeys
...
class TestFoo(TestCase):
    ...
    def test_createMulti(self):
        rows = [('foo name TD', 'foo desc TD', 98),
                ('foo name TD2', 'foo desc TD2', 99)]
        objs = Foo.createMulti(rows)
    ...
    def test_select(self):
        ...
        objs = TestFoo.db.select(Foo.createAdhoc(AdhocKeys({'desc': 'foo desc TD', 'bar_id': 98})))
        self.assertEqual(len(objs), 1)
        self.assertEqual(objs[0].getName(), 'foo name TD')
