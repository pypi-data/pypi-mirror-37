...
class Preprocessor(BasePreprocessor):
    ...
    def _process_jinja(self,
        ...
        if self._additional:
            if not os.path.exists(self._additional):
                print(f'Additional json {self._additional} is missing. Skipping')
            else:
                add = json.load(open(self._additional, encoding="utf8"))
                data = {**add, **data}
    ...
    def process_pgsqldoc_blocks(self, content: str) -> str:
        def _sub(block: str) -> str:
            ...
            if type(json_url) is str:
                json_url = [json_url]
            json_path = tag_options.get('json_path') or self.options['json_path']
            self._additional = tag_options.get('additional_json_path') or \
