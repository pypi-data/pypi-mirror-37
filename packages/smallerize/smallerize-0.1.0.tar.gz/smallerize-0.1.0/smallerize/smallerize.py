...
class Minimizer:
    ...
    def _check_probability_method(self, method: str, method_args: dict) -> str:
        ...
        """
        valid_methods = ['best_only', 'rank_all', 'pure_random']
        if method not in valid_methods:
            raise ValueError(
                "Probability method must be one of 'best_only', "
                " 'rank_all' or 'pure_random', {m} given".format(m=method)
            )
    ...
    def assign_participant(self, factor_levels: dict) -> str:
        ...
        if self._probability_method == 'pure_random':
            arm_probs = self.get_arm_probability(imbalances=None)
        else:
    ...
    def get_assignment_info(self, factor_levels: dict) -> dict:
        """
        Alternative to assign_participant(), takes factor levels
        for a new participant, chooses an arm (without actually
        adding to the count table), and returns the arm along with extra
        details about whether the most-favoured arm was chosen.

        :return: dict with keys: ``'arm'``: chosen arm, ``'prob'``: probability
          that was assigned to that arm before the selection,
    ...
    def get_arm_probability(
            self,
            imbalances: typing.Optional[dict]) -> dict:
        """
