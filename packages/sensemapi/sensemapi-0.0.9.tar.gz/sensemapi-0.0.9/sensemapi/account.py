...
class SenseMapAccount(SenseMapClient):
    ...
    def cache(self, new):
        if new:
            if all(map(lambda x: hasattr(new, x), ("get","sync"))):
                ...
                if self.application:
                    if self.application.cache:
                        self._cache = self.application.cache
                    else:
                        raise ValueError(
                            "{} object is not suitable as cache and "
                            "this account's associated application does not "
                            "have a cache"
                            .format(type(new).__name__).format(new))
                else:
                    raise ValueError(
                        "{} object is not suitable as cache and "
                        "this account does not have an associated application"
                        .format(type(new).__name__).format(new))
        self._cache = new
    ...
    def authenticache(self):
        ...
        for attr in ("token", "refresh_token",
            "token_time", "refresh_token_time", "password", "username",
            "email", "api"):
    ...
    def get_box(self, id):
        ...
        try:
            ...
            logger.debug("We already have box '{}', updating it!"
                .format(box.id))
        ...
        for account in self:
            if account.username:
                if account.username in d:
                    ...
                    d["{} Nr. {}".format(account.username,
                        duplicates[account.username] + 1)] = account
                else:
                    d[account.username] = account
            else:
                d["account without username {}".format(next(unnamed))]=account
        return d
    ...
    def __getattr__(self, attr):
        ...
        try:
            ...
            raise AttributeError(
                ...
                "nor an account with that title"
