...
class ReprObject(object): # pragma: no cover
    ...
    def to_json(self):
        return json.loads(json.dumps(self.init_arguments,
            default = self.to_json_fallback))

    @classmethod
    def from_json(cls, d):
        init_argspec = inspect.getfullargspec(self.__init__)
        init_args = {arg:default for arg, default \
            in zip(init_argspec.args[1:], init_argspec.defaults[1:])}
        for k,v in d.items():
            if k in init_args:
                init_args[k] = v
        obj = cls(**init_args)
