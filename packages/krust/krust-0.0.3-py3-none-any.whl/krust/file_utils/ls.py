...
def LS(root=None, recursive=False, pattern=None, with_root=None, ignore_hidden=True, use_glob=None, **kwargs):
    root = P(root)
    ...
    if pattern:
        filtered_result = []
        for full_fname in result:
            fname_to_match = os.path.basename(full_fname) if pattern_is_base_only else full_fname
            if re.search(pattern, fname_to_match):
                filtered_result.append(full_fname)
        return filtered_result
    else:
        return result


def LS_R(root=None, pattern=None, with_root=None, ignore_hidden=True, use_glob=None, **kwargs):
    kwargs.pop('recursive', None)
    return LS(root=root, recursive=True, pattern=pattern, with_root=with_root, ignore_hidden=ignore_hidden, use_glob=use_glob, **kwargs)
