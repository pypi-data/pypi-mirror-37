...
def nfs_add_export(conninfo, credentials, export_path, fs_path, description,
    ...
    method = "POST"
    allow_fs_path_create_ = "true" if allow_fs_path_create else "false"
    uri = "/v2/nfs/exports/?allow-fs-path-create=%s" % allow_fs_path_create_
...
@request.request
def nfs_get_export(conninfo, credentials, id_):
    id_ = unicode(id_)

    method = "GET"
    uri = "/v2/nfs/exports/%s" % id_

    return request.rest_request(conninfo, credentials, method, uri)

@request.request
def nfs_modify_export(conninfo, credentials, id_, export_path, fs_path,
                     description, restrictions, allow_fs_path_create=False,
                     if_match=None):
    id_ = unicode(id_)
    allow_fs_path_create_ = "true" if allow_fs_path_create else "false"
    ...
    method = "PUT"
    uri = "/v2/nfs/exports/%s?allow-fs-path-create=%s" % \
        (id_, allow_fs_path_create_)
...
@request.request
def nfs_delete_export(conninfo, credentials, id_):
    id_ = unicode(id_)
