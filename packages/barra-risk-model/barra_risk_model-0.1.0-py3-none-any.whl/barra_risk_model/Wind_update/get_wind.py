...
def update_wind(path,dt_range_dict,**kwargs):
    '''
    ...
    **kwargs:
    ...
    keys=['dt','rf','totalA','growth','index','descriptor']
    w.start()
    for k in keys:
        if k not in dt_range_dict.keys():
            ...
            if k=='dt':
                ...
                cprint('\t\tload growth',c='',f='l')
                dt_range2=fill_dt(dt_range_dict[k],path_dict['growth_path'])
                ...
                cprint('\t\tload descriptor',c='',f='l')
                dt_range2=fill_dt(dt_range_dict[k],path_dict['des_path'])
                load_descriptor(dt_range=dt_range2,stk_path=path_dict['stk_path'],des_path=path_dict['des_path'],**kwargs)
            else:
                continue
    w.close()