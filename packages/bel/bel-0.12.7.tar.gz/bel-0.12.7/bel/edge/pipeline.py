...
pipeline_errors_coll = edgestore_db.collection(arangodb.edgestore_pipeline_errors_name)
...
def get_edges_for_nanopub(nanopub_id):
    query = f'''
        FOR edge IN edges
            ...
            RETURN edge
    '''
    result = [edge for edge in edgestore_db.aql.execute(query)][0]
    return result
...
def process_nanopub(nanopub_url, db_name, collection_name: str = 'Nanopub', override: bool = False):
    ...
    results = bel.edge.edges.nanopub_to_edges(nanopub)
    ...
    if results['success']:
        ...
        return {'msg': 'Could not process nanopub into edges', "success": False, 'e': results['error']}


def save_pipeline_errors(nanopub_id: str, nanopub_url: str, type_: str, msg: str):
    """Save pipeline errors for review

    Args:
        type_: type of errors ['conversion', 'loading']
    """

    dt = utils.dt_utc_formatted()
    doc = {
    ...
    try:
        pipeline_errors_coll.insert(doc)
    except Exception as e:
        log.exception('Could not add pipeline error')
    # log.info(f'Save pipeline error: {r}')
...
def load_edges_into_db(nanopub_id: str, nanopub_url: str, edges: list = [], edges_coll_name: str = edges_coll_name, nodes_coll_name: str = nodes_coll_name):
    ...
    query = f'''
        FOR edge IN {edges_coll_name}
            FILTER edge.nanopub_id == "{nanopub_id}"
            REMOVE {{ _key: edge._key }} IN edges
    '''

    try:
        edgestore_db.aql.execute(query)
    except Exception as e:
        log.exception('Could not remove nanopub-related edges')
    ...
    query = f'''
        FOR error IN pipeline_errors
            FILTER error.nanopub_id == "{nanopub_id}"
            REMOVE error IN pipeline_errors
    '''
    try:
        edgestore_db.aql.execute(query)
    except Exception as e:
        log.exception('Could not remove nanopub-related errors')
    ...
    try:
        ...
        if results['error']:
            save_pipeline_errors(nanopub_id, nanopub_url, 'loading', f'Loading edges {json.dumps(results)}')
    except Exception as e:
        log.exception('Could not load edges')
    ...
    try:
        ...
        if results['error']:
            save_pipeline_errors(nanopub_id, nanopub_url, 'loading', f'Loading nodes {json.dumps(results)}')
    except Exception as e:
        log.exception('Could not load nodes')
