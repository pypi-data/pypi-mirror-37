...
def get_kline_data(symbol='EOS/USDT', timeframe='1m', since='', limit=1000):
...
def bfx_sopt_data(db_conf, exchange, symbol='EOS/USDT', timeframe='1m',sleeptimes=10):
    ...
    if max_recount != None:
        ...
        print(r'[%s]%s为空表，正尝试从2017-7-1获取数据！' % (dt.fmt_now(),table_name))
    ...
    while since - now_data_stamp <= 0:
        df = get_kline_data(symbol=symbol, timeframe=timeframe, since=since)
        if not df.empty:
...
def save_all_data():
    symbls = ['BTC/USDT',
              ...
              'SAN/USDT',
              'ETP/USDT']
    db_client = db_conf(host="127.0.0.1", port='3306', database='trading', user='coin_user1', password='coinuser1')
    timeframes = ['1D', '12h', '6h', '3h', '30m', '15m']
    for timeframe in timeframes:
        for symbol in symbls:
            bfx_sopt_data(db_client, 'bfx', symbol=symbol, timeframe=timeframe)
            time.sleep(5)
