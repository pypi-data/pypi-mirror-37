...
def _filter_by_date(dobjs, studydate='all'):
    ...
    Selects closest study to specified (ISO) date if multiple dates are present.
...
def fetch_series(patid, stuid='1', sernos=1, server=None, aettable=None, localaet=None, studydate='all'):
    ...
    # TODO: if we have a lot of studies may need to filter earlier without holding them all in memory
    dobjs = _filter_by_date(dobjs, studydate=studydate)
    return sorted(dobjs, key=lambda d: (int(d.SeriesNumber), int(d.InstanceNumber)))
...
def read_series(fileordirname, key=None, numeric=False, reverse=False, globspec='*.dcm'):
    ...
    ----------
    fileordirname:
        List of files, name of directory containing dicom files, a zip file or a single dicom file.
    key:
...
def main():
    ...
    def series_numbers(string):
        ...
        Defined by a single integer, an integer range or
