...
class CmdUpload(object):
    ...
    def _check_recipe_date(self, conan_ref, remote):
        ...
        if (remote_recipe_manifest != local_manifest and
            ...
            self._print_debug_information(remote_recipe_manifest, local_manifest, conan_ref, remote)
    ...
    def _print_debug_information(self, remote_recipe_manifest, local_manifest, conan_ref, remote):
        try:
            self._print_file("Remote manifest", "conanmanifest.txt", remote_recipe_manifest)
            self._print_file("Local manifest", "conanmanifest.txt", local_manifest)
            self._user_io.out.info("Different local files:\n")
            folder = self._client_cache.conan(conan_ref)
            difference = remote_recipe_manifest.difference(local_manifest)
            for the_file in difference:
                contents = tools.load(os.path.join(folder, the_file))
                self._print_file("Local: ", the_file, contents, line_ends=True)
                try:
                    # only unzipped files available
                    contents = self._remote_manager.get_path(conan_ref, package_id=None,
                                                             path=the_file, remote=remote)
                except NotFoundException:
                    pass
                else:
                    self._print_file("Remote: ", the_file, contents, line_ends=True)
        except Exception as e:
            self._user_io.out.info("Error printing information about the diff: %s" % str(e))

    def _print_file(self, prefix, filename, contents, line_ends=False):
        self._user_io.out.info("%s '%s':\n%s" % (prefix, filename, ("=" * 50)))
        if line_ends:
            contents = contents.replace("\r", "[CR]").replace("\n", "[LF]\n")
        self._user_io.out.info(contents)
        self._user_io.out.info("%s\n" % ("=" * 50))
