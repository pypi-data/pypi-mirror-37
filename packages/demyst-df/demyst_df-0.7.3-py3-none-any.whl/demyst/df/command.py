...
def watch():
    ...
    c = load_volatile_config(env="stg", region="us")
...
def channel(config_file, channel_id, inputs_file, sample_mode, pipes_only):
    spinner = Halo(text='Connecting to Demyst API ...', spinner='dots', stream=sys.stderr)
    spinner.start()
    c = load_volatile_config(config_file=config_file, env="stg", region="us")
...
def list_pipes(config_file, pipe_id, spinner=None):
    if spinner == None:
...
@click.pass_context
def attach_pipe(ctx, config_file, pipe_id, data_function_id, spinner=None):
    if spinner == None:
    ...
    ctx.invoke(list_pipes, config_file=config_file, pipe_id=pipe_id, spinner=spinner)
    return 0
...
@click.pass_context
def detach_pipe(ctx, config_file, pipe_id, spinner=None):
    if spinner == None:
...
@click.option('--config_file', default=None, required=False, help='Config file for Data Function or ./df.config')
@click.option('--inputs_file', required=True, help='JSON inputs file (see inputs.example.json)')
def execute(config_file, inputs_file):
