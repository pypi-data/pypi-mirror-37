...
class CheckGmsh(Command):
    ...
    def finalize_options(self):
        ...
        import subprocess
        import platform
        #If the command-line option is not present, use the path specified in the setup.py file. If
        # setup.py does not specify a gmsh_bin_path, try to locate gmsh via `which gmsh`.
        if self.gmsh_bin_path is None:
            if self.distribution.gmsh_bin_path is None:
                ...
                try:
                    ...
                    msg = 'Host system does not appear to have a gmsh installation.'
