...
def listcontainers(accountname=None, accountkey=None, nmax=None):
    ...
    table_header = ("Name", "Date")
    table_rows = [(container.name,) for i, container in enumerate(containers) if i < nmax]
    print(tabulate.tabulate(table_rows, headers=table_header))
...
def cli():
    ...
    # azblob list
    parser_get = subparsers.add_parser("list", help="list containers and blob in containers")
    parser_get.add_argument("container", nargs="?", help="container name, list blobs in it", default=None)
    parser_get.add_argument(
