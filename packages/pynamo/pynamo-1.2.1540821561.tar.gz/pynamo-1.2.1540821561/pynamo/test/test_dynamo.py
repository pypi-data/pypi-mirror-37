...
def test_get_keys():
    ...
    table_keys = pyn.get_table_keys('test_table')
...
def test_rewrite_same_key():
    ...
    pyn.rewrite_item('test_table', TEST_KEY, new_item)
    get_response = pyn.get_item('test_table', TEST_KEY)

    assert get_response['data'] == new_return
    assert len(pyn.scan('test_table')['data']) == 1
...
def test_rewrite_diff_prim_key():
    ...
    pyn.rewrite_item('test_table', TEST_KEY, new_item)
    get_response = pyn.get_item('test_table', new_key)
...
def test_rewrite_diff_both_key():
    ...
    pyn.rewrite_item('test_table', TEST_KEY, new_return)
    get_response = pyn.get_item('test_table', new_key)
